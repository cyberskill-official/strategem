...
- `strategem-implement` - build the next eligible FR (or a named task) to a reviewable state: load context, run the language gates and the engine oracle checks, record evidence in the ledger, hand off. See `.claude/skills/strategem-implement/SKILL.md`.
