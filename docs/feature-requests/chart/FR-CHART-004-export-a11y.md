...
related_frs: [FR-CHART-001, FR-WEB-001, FR-CHART-002, FR-CHART-003, FR-REPORT-002, FR-WEB-005, FR-WEB-007]
depends_on: [FR-CHART-001]
blocks: []
new_paths:
...
This FR completes the chart export seam FR-CHART-001 exposed and delivers the full accessibility pass for every chart type. It is the shared, cross-cutting slice: it takes the SVG serialization hook each chart provides (QiMen FR-CHART-001, LiuRen FR-CHART-002, TaiYi FR-CHART-003) and turns it into PNG, SVG, and print export, and it makes every chart pass the Vietnamese stacked-diacritics (dau chong) clip test and be fully screen-reader navigable. It owns export and accessibility for charts; it does NOT render the chart internals (each chart FR does) and does NOT re-cast or read the envelope beyond what the chart already read.

The module SHALL provide, for all chart types via their common export seam: (a) SVG export - direct serialization of the rendered chart, crisp at any zoom; (b) PNG export - a raster of the same at a specified scale; and (c) print - a print stylesheet (`chart-print.css`) that lays the chart to a page, collapses any opt-in glass to a solid surface (FR-WEB-001), and preserves every marker. The clip test SHALL run over each chart's palace/lesson/than labels (Han plus stacked Vietnamese diacritics) at 100% / 200% / 400% zoom on both light and dark themes, and no diacritic or descender SHALL be clipped (FR-WEB-001). Every chart component - each palace, lesson, transmission, than, and general - SHALL carry a screen-reader label naming its content and role (`a11y-labels.ts`), so the chart is navigable and announced without sight of the visual. cat/hung / trung polarity SHALL never be encoded by color alone - every polarity marker SHALL pair color with an icon and text - and this SHALL hold in every export: a PNG, an SVG, and a print output SHALL each carry the icon and text, not color only, so the marker survives grayscale, print, and low vision.
...
```ts
// consumes the FR-CHART-001 export seam each chart type exposes (QiMen / LiuRen / TaiYi)
...
A `@media print` stylesheet that fits the chart to the page, collapses opt-in glass to a solid surface (FR-WEB-001), preserves the icon+text polarity markers, and keeps Han plus diacritics unclipped at print scale.
...
| keyboard | every component is focusable and its detail is keyboard-reachable (baseline from each chart FR; completed here) |
...
1. SVG, PNG, and print export are available for all chart types (QiMen/LiuRen/TaiYi) through the common FR-CHART-001 export seam; a chart type is exportable via the seam with no per-type export code.
...
1. `lib/chart/export-svg.ts`: serialize the chart's SVG through the FR-CHART-001 seam, inlining fonts and the icon+text markers.
...
Depends on FR-CHART-001 (the export seam and the baseline accessibility it exposes; this FR completes both). Applies to FR-CHART-002 (LiuRen) and FR-CHART-003 (TaiYi) through the same seam, so all three chart types export and pass the a11y gate uniformly. Uses FR-WEB-001 (the tokens, the glass-collapses-on-print rule, the cat/hung color+icon+text convention, and the shared clip-test harness). The export it completes is what the report view (FR-WEB-005), the PDF export (FR-REPORT-002), and the management-flow export (FR-WEB-007) build on for chart images. It re-reads no envelope and re-casts nothing (strategy 4.3) - it serializes the rendered chart.

## §8 - Example payloads

```ts
// export any chart type through the common FR-CHART-001 seam
...
- Whether print is per-chart or whole-report. Default: this FR owns the chart print stylesheet; the whole-report PDF is FR-REPORT-002, which embeds the chart export - one chart-export source of truth, two consumers.
...
| Glass survives print | opt-in glass renders on the printed page | forbidden; print collapses glass to a solid surface (FR-WEB-001) |
