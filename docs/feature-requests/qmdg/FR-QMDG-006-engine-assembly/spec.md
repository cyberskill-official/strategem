...
related_frs: [FR-QMDG-005, FR-CORE-006, FR-PLAT-002, FR-CHART-001, FR-STRAT-001]
depends_on: [FR-QMDG-005, FR-CORE-006]
blocks: [FR-CHART-001, FR-STRAT-001, FR-QMDG-007]
new_paths:
...
- `tu_tru` and `dinh_cuc.tiet_khi` / `tam_nguyen` -> envelope `lich_phap` (from FR-CORE-005); the engine-native Han tam nguyen (上元/中元/下元) maps to the romanized `lich_phap.tiet_khi.tam_nguyen` (thuong/trung/ha).
...
- `cach_cuc` -> envelope `cach_cuc` as `CachCuc` objects (FR-QMDG-005 `CachCucHit`), not bare strings.
...
`cache_key` follows FR-PLAT-002: a stable hash of `(he, dau_vao rounded to the casting granularity, co_truong_phai sorted, lich_phap.co_lich_phap sorted)`; `co_truong_phai` is a sorted map so the key is identical in Rust and Python.
...
- Gates: `cargo fmt --check`, `cargo clippy -p cyberos-qimen -- -D warnings`, `cargo test -p cyberos-qimen`; the CORE oracle harness (FR-CORE-006) must be green first, since QiMen inherits the calendar context.

## §6 - Implementation skeleton

1. Add `ban.rs` with `KyMonBan` (serde) and its mapping to the envelope `ban` slot; add `engine.rs` with `cast`.
2. Wire the pipeline: call FR-QMDG-001..005 in order, assemble `KyMonBan`, collect `CachCucHit`s.
3. Build the envelope: set `he`, `dau_vao`, `lich_phap` (from FR-CORE-005), `ban`, `cach_cuc`, `co_truong_phai` (all five flags), `provenance`.
...
Depends on FR-QMDG-005 (the last pipeline stage) and FR-CORE-006 (the calendar oracle harness must be green, since QiMen stands on the CORE context). Emits the FR-PLAT-002 envelope. Blocks FR-CHART-001 (the 9-palace UI renders this chart), FR-STRAT-001 (the Timing Optimizer scans this engine), and FR-QMDG-007 (dung than reads the assembled chart). This is the QiMen node on the P0 critical path: `CORE-006 + QMDG-006 -> CHART-001 / STRAT-001`.
