...
related_frs: [FR-QMDG-002, FR-QMDG-004, FR-QMDG-006]
depends_on: [FR-QMDG-002]
blocks: [FR-QMDG-004, FR-QMDG-006]
new_paths:
...
This FR implements the pivot that makes a QiMen chart specific to the hour: find the truc phu (值符, the leading star) and truc su (值使, the leading door), then rotate the thien ban (sky plate) so the truc phu and its nghi sit over the palace that holds the hour stem. It is the step that binds the still earth plate (FR-QMDG-002) to the moment of the question, and it is one of the sharpest school-fork points in the whole engine (s5.2).
...
2. Find that nghi's palace on the `DiaBan` from FR-QMDG-002 - this is the tuan-thu palace.
3. The resting cuu tinh star at that palace is the truc phu; the resting bat mon door at that palace is the truc su. (The resting rings - each star's and door's home palace - are the fixtures formalized in FR-QMDG-004; this FR consumes them.)
...
pub struct TrucPhuSu {
    pub truc_phu: CuuTinh,        // leading star (type from FR-QMDG-004)
    ...
    pub xoay: i8,                 // rotation offset consumed by FR-QMDG-004
}
...
2. The tuan-thu palace equals the palace of the hidden nghi on the earth plate; a unit test cross-checks against FR-QMDG-002 output.
...
2. Locate the hidden nghi on the `DiaBan`; read the resting star and door (rings from FR-QMDG-004, imported as consts).
...
Depends on FR-QMDG-002 (reads the earth plate). Blocks FR-QMDG-004 (the star / door / god rings are placed using the `xoay` offset and the truc phu / truc su this FR identifies) and FR-QMDG-006 (assembly). The resting cuu tinh and bat mon rings are formalized in FR-QMDG-004 and imported here; within one crate they are shared consts, so there is no cyclic build dependency.
...
| Resting-ring drift | rings differ from FR-QMDG-004 | shared const import prevents divergence; test cross-checks |

## §11 - Notes

This FR and FR-QMDG-004 are tightly coupled: this one identifies the pivot and rotates the stems and exposes the `xoay` offset; the next one paints the three dynamic rings using that offset. Keep the split clean - rotation math lives here, ring placement lives there - so each has a focused oracle gate. The chuyen / phi fork is the RISK-2 exemplar for QiMen: never let a default become an implicit "only correct" answer.
