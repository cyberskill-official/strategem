...
related_frs: [FR-WEB-001, FR-WEB-002, FR-WEB-003, FR-WEB-007, FR-API-004, FR-AUTH-001, FR-LEGAL-001]
...
```
+----------------------------- dashboard -----------------------------+
| top bar (Umber, Ochre mark, system tabs)            (FR-WEB-001)    |
...
| disclaimer (FR-LEGAL-001)                                           |
+--------------------------------------------------------------------+
```

### Data (`lib/api/dashboard.ts`, reads FR-API-004 persistence)
...
// GET the persisted references for the authed principal (FR-API-004); sends the JWT Bearer;
// read-only; never fetches or mutates the ban.
```

### Routing

| Affordance | Routes to | FR |
|---|---|---|
| quick-cast (Ochre primary) | `/cast` (optionally `?system=<last>`) | FR-WEB-002 |
| a recent / saved chart card | `/results/{query_id}` | FR-WEB-003 |
| flow card: lookup | `/cast` | FR-WEB-002 |
| flow card: learning | the EDU entry | FR-EDU-001 (later) |
| flow card: management | `/manage` | FR-WEB-007 (later) |
...
2. There is exactly one Ochre primary on the view - the quick-cast entry at md `44px` (FR-WEB-001) - routing to `/cast`; the three flow cards are secondary, not primary.
...
5. The in-product disclaimer (FR-LEGAL-001) is present on the screen.
...
- Accessibility: `jest-axe` clean; keyboard-reachable cards and CTA; focus-visible Ochre ring; the stacked-diacritics clip test (FR-WEB-001) over question-type labels and system names.
- Contract: `ChartRef` is checked against the FR-API-004 persisted-reference shape (shared fixture); a drift fails the test.
- Gates: `pnpm --filter web lint`, `pnpm --filter web test`, `next build`.

## §6 - Implementation skeleton

1. `lib/api/dashboard.ts`: the typed `getDashboard()` read against FR-API-004 with the auth token; read-only.
...
5. `dashboard.tsx` + `app/dashboard/page.tsx`: compose the sections in the FR-WEB-001 shell; wire the disclaimer slot (FR-LEGAL-001).
6. `tests/dashboard.test.tsx` + the shared FR-API-004 reference fixture.

## §7 - Dependencies

Depends on FR-WEB-001 (the shell, tokens, Button, top-bar tabs, disclaimer slot). Reads FR-API-004 (the persisted query/chart references) for the recent and saved lists, and is gated by FR-AUTH-001 (post-login). Routes into FR-WEB-002 (quick-cast and the lookup card), FR-WEB-003 (a chart card opens its results), FR-WEB-007 (the management card), and the FR-EDU-001 learning surface when it lands. Uses FR-LEGAL-001 for the disclaimer. It never casts a chart itself (strategy 4.2) and reads any envelope read-only (strategy 4.3).
...
// a recent card -> router.push(`/results/${ref.query_id}`)   (FR-WEB-003)
// quick-cast    -> router.push(`/cast?system=${lastSystem}`) (FR-WEB-002)
```

## §9 - Open questions

- What "saved" means at P1: an explicit user pin vs an automatic keep of the last N. Default: an explicit saved flag on the chart (toggled from results / management, FR-WEB-007), with recent being the automatic last-N; this keeps saved a deliberate act and recent a convenience.
...
- Where the learning flow lands before FR-EDU-001 ships. Default: the learning card renders disabled with a "coming soon" state so the three-flow information architecture is visible without a dead link.

## §10 - Failure modes inventory

| Mode | Trigger | Required behavior |
|---|---|---|
| Second primary | a flow card styled as an Ochre primary | forbidden; the single primary is quick-cast; flow cards are secondary (FR-WEB-001) |
...
| Dashboard casts | the landing tries to compute a chart | forbidden; it reads references and routes; casting is FR-WEB-002 / the gateway |
| Envelope mutated on preview | a chart preview writes an engine field | forbidden; read-only; a byte-equality test asserts it |
| Missing disclaimer | framing only in a footer elsewhere | the FR-LEGAL-001 disclaimer is present on the dashboard |

## §11 - Notes

The dashboard is the hub that makes the three-flow information architecture real (Claude-07 s6): one front door, built on the FR-WEB-001 shell, that leads to casting, to past charts, and to each flow. Keep it a thin navigator - one Ochre primary (quick-cast), read-only reference cards routing by `query_id`, and secondary flow cards - and let the screens it routes to do the casting, reading, and managing. It is a SHOULD at P1 because the P0 lookup spine (WEB-002 -> WEB-003) can stand without it, but it is the screen that turns a set of pages into a coherent product.
