...
related_frs: [FR-WEB-002, FR-CHART-001, FR-RAG-003, FR-RAG-004, FR-WEB-001, FR-WEB-005, FR-LEGAL-001, FR-API-001]
depends_on: [FR-WEB-002, FR-CHART-001, FR-RAG-003]
blocks: [FR-WEB-005, FR-WEB-007]
new_paths:
...
This FR builds the results screen - the right panel of the chart-casting layout (FR-WEB-001) - which presents the outcome of a cast: the chart, the deterministic patterns the engine and rule layer found, and the AI's cited interpretation, each clearly attributed to its source (strategy 4.4). It is the "read" step of the lookup flow. It owns the results presentation; it does NOT render the chart internals (FR-CHART-001) nor produce interpretation (FR-RAG-003) - it composes their outputs and is where the deterministic/AI boundary is made legible to the user.

The screen SHALL render, from the FR-API-001 `QueryResponse`: (a) the chart via FR-CHART-001, reading `charts[i]` (the la so envelope, `he = "ky_mon"` at P0); (b) the detected patterns from `charts[i].cach_cuc` / `patterns[]` as a list with each pattern's name, Han name, palace (`cung`), and cat/hung polarity shown with icon and text, never color alone (FR-WEB-001); and (c) the interpretation from `interpretation` (FR-RAG-003) with both persona readings (beginner/expert, toggleable), the recommendations, and the citation cards (Han + bach thoai + dich + locator).

The screen SHALL display an `AIDisclosureBadge` (FR-WEB-001) on the interpretation, always, fed by the `ai_disclosure` block - it is mandatory on any AI output (strategy 4.4). Where `interpretation.requires_human_review` is true (or `review_status` is pending), the screen SHALL render a `HumanReviewGate` (FR-WEB-001, FR-RAG-004) and SHALL clearly distinguish a not-yet-reviewed interpretation from an approved one. The screen SHALL visually separate the deterministic region (chart + patterns, sourced from the engine) from the interpreted region (AI reading, sourced from RAG and labeled), so a user can always tell computed fact from AI interpretation. It SHALL treat the la so envelope as read-only and SHALL NOT mutate `ban`, `cach_cuc`, `lich_phap`, or `co_truong_phai` (strategy 4.3).
...
### Layout (Claude-07 s6, the right panel of FR-WEB-001)

```
+-------------------- results panel (right) --------------------+
| [ chart region - deterministic ]                             |
|   <NinePalaceChart envelope={charts[0]} />   (FR-CHART-001)  |
...
### Data source (the FR-API-001 `QueryResponse`)

```ts
type QueryResponse = {
  query_id: string;
  charts: LaSo[];                 // FR-PLAT-002 envelope(s); read-only here
  patterns: CachCuc[];            // detected cach cuc
  interpretation: Interpretation; // FR-RAG-003: beginner/expert, recommendations, citations, confidence, requires_human_review, disclosure
  ai_disclosure: AIDisclosure;    // mandatory
};
```

- Chart region: passes `charts[0]` to FR-CHART-001 (`he = "ky_mon"` at P0); the component reads `ban` and marks `cach_cuc` palaces. Read-only.
...
`persona-toggle` switches between `beginner_interpretation` and `expert_interpretation` (both present in one response, FR-RAG-003). `citation-card` shows the three text layers (`han`, `bach_thoai`, `dich`) and the `locator`; in-text citation refs link to their card.

## §4 - Acceptance criteria

1. The screen renders the chart (FR-CHART-001) from `charts[0]`, the detected cach cuc list, and the interpretation from a `QueryResponse`, in the three regions above.
...
4. cat/hung polarity on each pattern is shown with icon and text, never color alone (FR-WEB-001).
...
- Accessibility: `jest-axe` clean; the `HumanReviewGate` state announced to screen readers (FR-WEB-001); the stacked-diacritics clip test over the interpretation text and pattern names on light+dark.
- Contract: the `Interpretation` / `AIDisclosure` types are checked against the FR-RAG-003 shapes (shared fixture) so the screen and the interpreter agree; a drift fails the test.
- Gates: `pnpm --filter web lint`, `pnpm --filter web test`, `next build`.

## §6 - Implementation skeleton

1. `app/results/[queryId]/page.tsx`: fetch/receive the `QueryResponse` (routed from FR-WEB-002 or fetched by `query_id`), mount the right panel in the FR-WEB-001 shell.
2. `results-panel.tsx`: the three regions and the visual boundary; pass `charts[0]` to FR-CHART-001.
...
6. `tests/results-panel.test.tsx` + the shared FR-RAG-003 contract fixture.

## §7 - Dependencies

Depends on FR-WEB-002 (routes here on a successful cast, carrying the response/`query_id`), FR-CHART-001 (the 9-palace chart it embeds), and FR-RAG-003 (the `Interpretation` and `AIDisclosure` it renders). Uses FR-WEB-001 (`AIDisclosureBadge`, `HumanReviewGate`, tokens, the cat/hung icon+text convention) and pairs with FR-RAG-004 behind the gate. Reads the FR-API-001 `QueryResponse`. Blocks FR-WEB-005 (the report view builds on this presentation) and FR-WEB-007 (the management flow lists past results). Reads the FR-PLAT-002 envelope read-only (strategy 4.3).
...
- Multi-chart layout for `/calculate/all` (P1+): tabs, stacked, or side-by-side per system. Default: render `charts[0]` at P0 (QiMen only); the panel already takes a list, so a per-system tab set is additive when LiuRen/TaiYi land (FR-CHART-002/003, FR-STRAT-004).
- Where the persona default comes from: the request's `persona_level` vs a user preference. Default: honor the request's persona as the initial toggle state; both readings are present in one response, so switching is local and free (FR-RAG-003 §9).
- How prominent the confidence value is. Default: show `confidence` alongside the AIDisclosureBadge as supporting context, not as a headline number, so it informs without implying false precision; the badge carries the limits copy (FR-LEGAL-001).
...
| Color-only polarity | cat/hung shown by color alone | forbidden; polarity is icon + text + color (FR-WEB-001) |
...
| Uncited claim rendered as sourced | interpretation text without a resolvable citation | citations come from FR-RAG-003 (guarded there); the card/ref link must resolve, or the claim is not shown as sourced |
