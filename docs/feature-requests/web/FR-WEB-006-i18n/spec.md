...
related_frs: [FR-WEB-001, FR-WEB-002, FR-WEB-003, FR-WEB-008, FR-KB-003, FR-RAG-003, FR-LEGAL-001]
...
export const routing = defineRouting({
  locales: ["vi", "en"],     // FR-WEB-008 adds "zh"
  defaultLocale: "vi",
  localePrefix: "as-needed"
});
```

The middleware resolves the active locale (path prefix, then `Accept-Language`, then default `vi`) and sets it on the request; `request.ts` loads the matching message catalog. The active locale is hosted in the FR-WEB-001 shell's locale slot.

### The two planes

| Plane | Source | Translated by | Rendered |
|---|---|---|---|
| UI labels / chrome | `messages/{locale}.json` | the frontend (this FR) | via next-intl `t("...")` |
| domain content (pattern names, interpretation, recommendations, classical excerpts, Han/bach thoai/dich) | backend KB (FR-KB-003) + RAG (FR-RAG-003) | domain experts, upstream | displayed as-is, never re-translated |
...
1. next-intl is configured with locales `["vi", "en"]`, default `vi`, locale-prefixed routing, and a middleware that resolves the active locale; the locale plugs into the FR-WEB-001 shell.
...
4. Domain content is fetched in the active locale from the backend (FR-KB-003 / FR-RAG-003) and rendered as-is; the resolver has no machine-translation path (asserted - there is no translate call).
...
- Accessibility: `lang` attribute reflects the active locale; the stacked-diacritics clip test (FR-WEB-001) passes in `vi` and over Han-bearing domain terms in both locales.
- Gates: `pnpm --filter web lint`, `pnpm --filter web test`, `next build`.

## §6 - Implementation skeleton

1. `i18n/routing.ts` + `i18n/request.ts` + `middleware.ts`: next-intl with `["vi","en"]`, default `vi`, locale resolution; host the locale in the FR-WEB-001 shell slot.
2. `messages/vi.json` + `messages/en.json`: the UI chrome catalogs (buttons, field labels, nav, empty states, error strings from the FR-API-001 envelope mapping, the FR-LEGAL-001 disclaimer keys).
3. `lib/i18n/domain-content.ts`: the resolver that fetches active-locale domain content from FR-KB-003 / FR-RAG-003 and returns it verbatim, Han preserved, with no translate path.
4. `components/i18n/locale-switcher.tsx`: the vi/en switch (Ochre focus ring, FR-WEB-001).
5. Migrate the existing chrome (FR-WEB-001/002/003) to `t("...")` keys; keep domain content on the resolver.
6. `tests/i18n.test.tsx` + the hard-coded-string audit.

## §7 - Dependencies

Depends on FR-WEB-001 (the shell hosts the locale, and the components whose chrome moves into the catalogs). Reads domain content from FR-KB-003 (the classical three-layer store, with expert translations) and FR-RAG-003 (the interpretation text and recommendations in the requested locale). Provides the label i18n FR-WEB-002 and FR-WEB-003 consume for their chrome, and the disclaimer keys for FR-LEGAL-001. Blocks FR-WEB-008 (Chinese + RTL-ready extends this foundation). The split - frontend owns chrome, backend owns domain content - is the technical form of the cultural-respect guardrail (strategy 7).
...
// -> { han: "青龍返首", text: "Green Dragon Returns... (expert rendering, from FR-KB-003)", locale: "en", source: "Yen Ba Dieu Tau Ca" }
...
- Whether numbers/dates are localized. Default: localize UI-facing dates/numbers via next-intl formatters, but never localize a `datetime` that is an astronomical input (the cast uses explicit ISO + tz, FR-WEB-002) - display formatting only, never the input value.
