...
related_frs: [FR-TAT-001, FR-TAT-002, FR-TAT-004, FR-TAT-005, FR-TAT-006, FR-PLAT-002]
depends_on: [FR-TAT-002]
blocks: [FR-TAT-005, FR-TAT-006]
new_paths:
...
This FR computes the most distinctive part of a Thai At chart: the eight tuong (八將, eight generals) and the toan (算, counts) that measure the strength of host and guest. It extends the `cyberos-thaiat` crate, reading the sixteen-than ring and the Thai At seat from FR-TAT-002 and the `nhap_cuc` / don from FR-TAT-001.
...
```
# Van Xuong: placed by nhap cuc; counts on the FR-TAT-002 ring
...
3. `tinh_toan` adds a chinh cung's palace number and a gian than's one (tags read from FR-TAT-002), and stops before Thai At under `truoc_thai_at`; switching to `sau_thai_at` changes both toan; the flag value is stamped into `co_truong_phai`.
...
6. The emitted `ban.bat_tuong` and `ban.cac_toan` round-trip through the la so envelope (FR-PLAT-002) under `he = "thai_at"`.
...
- Property: over 1..=72 x {duong, am}, `van_xuong`, `ke_than`, `thuy_kich` are total and land on valid ring indices; `tinh_toan` gives identical results whether the tag is read from FR-TAT-002 or recomputed (guards against tag drift); switching `dem_toan` changes the toan by exactly the before/after mark's contribution.
- Oracle: `tests/battuong_oracle.rs` loads `fixtures/battuong_kintaiyi.csv` (generated once from kintaiyi, per epoch, both dons, both `dem_toan` values, many years) and asserts every tuong ring index and both toan match exactly. Central to the FR-TAT-006 100% gate.
...
1. `battuong.rs`: `van_xuong` (the reduce-by-eighteen count with the double-count marks), `ke_than` (forward/backward over the twelve chi), `thuy_kich` (derive through ke than), and the two dai/tham tuong placements reusing FR-TAT-002's chinh-cung-mark correspondence.
2. `toan.rs`: `tinh_toan` reading the chinh-cung / gian-than tag from FR-TAT-002, the `DemToan` flag, `dai_tuong_cung`, `tham_tuong_cung`, `truong_doan`.
3. Wire the `dem_toan` flag into the FR-TAT-001 flag set (default `truoc_thai_at`); FR-TAT-006 stamps it.
4. Extend the `he = "thai_at"` `ban` with `bat_tuong` and `cac_toan`; keep FR-TAT-002's `thap_luc_than` and seat intact.
...
Depends on FR-TAT-002 (the sixteen-than ring, the chinh-cung / gian-than tags, and the Thai At seat) and transitively on FR-TAT-001 (`nhap_cuc`, don, the flag set). Blocks FR-TAT-005 (cach cuc reads the tuong positions and the toan; the truong/doan label is one of its four criteria) and FR-TAT-006 (assembly + the oracle gate). Emits into the FR-PLAT-002 envelope.
...
|---|---|---|
| Chinh cung / gian than mixed in the toan | loop re-decides the tag instead of reading FR-TAT-002 | property test: reading the tag vs recomputing gives identical toan; oracle diverges otherwise |
...
This is the heaviest computational slice of the TaiYi engine and the one most exposed to the classic counting bug, which is exactly why FR-TAT-002 pre-tagged the ring: the toan loop here reads the chinh-cung / gian-than tag rather than re-deriving it (Claude-04 s3.3, s4.3). Two flags meet here - the epoch from FR-TAT-001 and `dem_toan` from this FR - and both are stamped by FR-TAT-006; a one-mark stop difference re-casts the victory reading, so `dem_toan` is load-bearing (RISK-2). The truong/doan label lives with the toan because it is a pure function of it; FR-TAT-005 consumes it as one of the four victory criteria. Faithful transcription against the s4 pseudocode plus the kintaiyi oracle, over both dons and both `dem_toan` values, is the acceptance bar.
