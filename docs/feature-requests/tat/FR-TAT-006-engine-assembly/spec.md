...
related_frs: [FR-TAT-001, FR-TAT-002, FR-TAT-003, FR-TAT-004, FR-TAT-005, FR-PLAT-002, FR-CORE-006, FR-CHART-003]
depends_on: [FR-TAT-003, FR-CORE-006]
blocks: [FR-CHART-003]
new_paths:
...
This FR assembles the whole Thai At engine from the slices FR-TAT-001..005 into one deterministic pipeline, emits the la so envelope for `he = "thai_at"`, stamps the full flag set, and gates the engine 100% against the kintaiyi oracle. It is the terminal MUST of the TAT module and the culmination of the `cyberos-thaiat` crate.

The module SHALL run the pipeline in order (Claude-04 s7.1): compute the tich for the chosen level and reduce to the cuc (FR-TAT-001, and FR-TAT-004 for nguyet / nhat / thoi ke), seat Thai At and lay the sixteen than (FR-TAT-002), place the eight tuong and compute the toan (FR-TAT-003), then recognize the cach and compute tam tai and the four criteria (FR-TAT-005). Every step is deterministic, so the assembled chart SHALL be reproducible from `dau_vao` plus the stamped flags alone.

The module SHALL emit the s7.1 engine JSON mapped into the FR-PLAT-002 envelope: `he = "thai_at"` at the top level; `dau_vao`; `lich_phap` from FR-CORE-005; the engine plates (`tich`, `thai_at_cung`, `thai_at_ring`, `thap_luc_than`, `bat_tuong`, `cac_toan`, `tam_tai`) inside `ban`; each recognized cach as an envelope-level `cach_cuc` entry; and the flag set in `co_truong_phai`. The module SHALL NOT invent an engine-local envelope shape - it fills the shared PLAT-002 envelope.

The module SHALL stamp the full flag set into `co_truong_phai` (Claude-04 s7.2): `epoch` (default `kim_kinh`, the ~60-year-gap school flag from FR-TAT-001), `dem_toan` (default `truoc_thai_at`, the count-stop flag from FR-TAT-003), the time-level `cap` (nien / nguyet / nhat / thoi), and the than-name variant handling flag (FR-TAT-002). Every flag that changed the chart SHALL be stamped, or it is a reproduction defect (RISK-2). The module SHALL compute the chart cache key per FR-PLAT-002 (a stable hash keyed by year + cap + epoch and the sorted flag set), since the whole pipeline is deterministic and cacheable (Claude-04 s7.1).
...
```
def cast_thai_at(dau_vao, flags, lich):            # lich = FR-CORE-005 output (Dong Chi / Ha Chi anchor)
    tich  = tich_theo_cap(flags.cap, dau_vao, flags.epoch, lich)     # FR-TAT-001 / FR-TAT-004
    seat  = an_thai_at(tich.nhap_cuc, tich.duong_don)                # FR-TAT-002
    ring  = THAP_LUC_THAN                                            # FR-TAT-002
    tuong = bat_tuong(tich, seat, dau_vao.nam_chi, flags.dem_toan)   # FR-TAT-003
    toan  = cac_toan(tuong, seat, flags.dem_toan)                    # FR-TAT-003
    cach  = nhan_dien_cach_cuc(tuong, seat.thai_at_ring)             # FR-TAT-005 (SHOULD; else empty)
    tt    = tinh_tam_tai(ban)                                        # FR-TAT-005 (SHOULD; else None)
    ban   = ThaiAtBan { tich, seat, ring, tuong, toan, tam_tai: tt }
    return to_envelope(dau_vao, lich, ban, cach, flags)             # map into FR-PLAT-002
```
...
The flat s7.1 shape maps into the shared envelope thus: `he`, `dau_vao`, `lich_phap` (FR-CORE-005), and `provenance` at the top level; `tich`, `thai_at_cung`, `thai_at_ring`, `thap_luc_than`, `bat_tuong`, `cac_toan`, `tam_tai` inside `ban`; the s7.1 `cach_cuc` string list becomes the envelope-level `cach_cuc` array of FR-PLAT-002 `CachCuc` objects (FR-TAT-005); `co_truong_phai` is the full flag set below.
...
pub struct CoTruongPhaiTat {
    pub epoch: Epoch,                    // default KimKinh (FR-TAT-001); re-casts the whole chart
    pub dem_toan: DemToan,               // default TruocThaiAt (FR-TAT-003); shifts both toan by a mark
    pub cap: Cap,                        // nien / nguyet / nhat / thoi (FR-TAT-004)
    pub bien_the_ten_than: BienTheTenThan,   // than-name variant handling (FR-TAT-002)
}
...
pub struct ThaiAtBan {
    ...
    pub tam_tai: Option<TamTai>,         // filled by FR-TAT-005; None if that SHOULD is not built
}
...
1. `cast_thai_at` runs the full pipeline and emits a valid FR-PLAT-002 envelope with `he = "thai_at"`; the golden 2004 nien ke chart matches the s5.1 / s7.1 numbers (tich 10,155,921, cuc 33, Giap Than, duong don) and round-trips through the envelope.
...
4. `cache_key` is a stable hash of year + cap + epoch + the sorted flag set (FR-PLAT-002); two casts with identical inputs and flags produce identical keys, and changing any stamped flag changes the key.
...
1. `ban.rs`: the `ThaiAtBan` type and its serialization into the FR-PLAT-002 `ban` slot for `he = "thai_at"`; the s7.1 -> envelope mapping.
2. `engine.rs`: `cast_thai_at` running the FR-TAT-001..005 pipeline in order; the `CoTruongPhaiTat` flag set with the s7.2 defaults; `cache_key` per FR-PLAT-002 (year + cap + epoch + sorted flags).
3. Wire `lich_phap` from FR-CORE-005 (the Dong Chi / Ha Chi anchor, shared with QiMen / LiuRen) into the envelope and into FR-TAT-004's nhat / thoi ke.
4. Fill the envelope `cach_cuc` array from FR-TAT-005's `map_to_envelope_cach_cuc`; set `tam_tai` in `ban`; leave both empty / None if the SHOULD FR-TAT-005 / FR-TAT-004 levels are not yet built (the MUST nien ke path still emits a valid envelope).
...
Hard depends on FR-TAT-003 (the tuong and toan - the MUST spine that makes a chart meaningful) and FR-CORE-006 (the oracle cross-check harness and the shared solstice anchors). Soft-consumes FR-TAT-004 (the nguyet / nhat / thoi ke levels - without them the assembly still emits and gates nien ke; with them the gate extends to all four levels) and FR-TAT-005 (the cach set and tam tai - without them the envelope `cach_cuc` is empty and `tam_tai` is None, still valid). Transitively stands on FR-TAT-001 (tich / reductions / epoch), FR-TAT-002 (ring / seat), and FR-PLAT-002 (the envelope, the version rule, the cache-key rule). Blocks FR-CHART-003 (the TaiYi chart view renders this envelope: cuu cung, sixteen than, tuong). Soft-feeds STRAT-004 (cross-system validate can add TaiYi as a third opinion once this lands) and EDU-002 (the TaiYi engine as an auto-grader at curriculum level 3).
...
{ "envelope_version": 1, "he": "thai_at",
  ...
  "ban": {
    ...
    "thap_luc_than": { "...": "the 16-than ring, FR-TAT-002" },
...
- Whether `ban` should become a tagged union in the shared `laso-envelope` crate now that all three engines exist (the deferral in FR-PLAT-002 §9 named TAT-006 as the revisit point). Default: keep `ban` opaque (`serde_json::Value` in the shared crate, strongly typed `ThaiAtBan` in this crate); revisit only if the interpretation branch needs a cross-engine typed reader.
- The cache-key granularity for nhat / thoi ke: nien / nguyet key cleanly by year + cap + epoch, but hour-level casts vary within a day. Default: include the casting instant at the cap's granularity in the key (per FR-PLAT-002's "dau_vao rounded to the casting granularity"); confirm the rounding with PLAT-006.
