...
related_frs: [FR-TAT-002, FR-TAT-003, FR-TAT-004, FR-TAT-006, FR-CORE-005, FR-PLAT-002]
depends_on: [FR-CORE-005]
blocks: [FR-TAT-002, FR-TAT-004, FR-TAT-006]
new_paths:
...
This FR builds the base of every Thai At Than So chart: tich nien and the reductions that turn it into the numbers the rest of the engine reads. It owns the birth of the `cyberos-thaiat` crate; FR-TAT-002..006 extend it.
...
The module SHALL emit its result (tich nien, the three reductions, the don direction, the Thai At palace) into the `ban` slot of the la so envelope (FR-PLAT-002) under `he = "thai_at"`, and SHALL stamp the TAT flag set into `co_truong_phai`. The oracle for the whole engine is kintaiyi.
...
6. The emitted `ban` (tich, Thai At palace) round-trips through the la so envelope (FR-PLAT-002) under `he = "thai_at"`.
...
1. Create the `cyberos-thaiat` crate (this FR owns its birth; FR-TAT-002..006 add modules).
...
Depends on FR-CORE-005 (the calendar module API). Nien ke needs only the civil year, but the don direction keys off the Dong Chi / Ha Chi instants from FR-CORE-001, and the nhat/thoi ke tich (FR-TAT-004) anchor on Dong Chi - so TaiYi and the calendar core must agree on the solstice instants. Blocks FR-TAT-002 (an Thai At + the sixteen than read `nhap_cuc` and the Thai At palace), FR-TAT-004 (the four time levels reuse this epoch / reduction machinery), and FR-TAT-006 (assembly + oracle gate). Emits into the FR-PLAT-002 envelope.
...
- Nien ke needs only the year; nguyet/nhat/thoi ke (FR-TAT-004) need tich nguyet/nhat/thoi and the Dong Chi anchor. The tich formulas are TAT-004's, but the epoch + reduction machinery lives here. Confirm the boundary when TAT-004 lands.
