...
related_frs: [FR-REPORT-001, FR-REPORT-002, FR-QMDG-007, FR-STRAT-003, FR-WEB-005]
depends_on: [FR-REPORT-001]
blocks: []
new_paths:
...
This FR provides sample report templates per question type: trach thoi 擇時 (timing), phuong vi 方位 (direction), chu-khach 主客 (competitor / risk), and vi mo (macro outlook). Each template SHALL shape how a FR-REPORT-001 `StructuredReport` is presented - the section order, which dung than 用神 to foreground, and the emphasis - and SHALL document the mapping from question type to the right system and dung than per Claude-07 s1: trach thoi -> Ky Mon (Luc Nham for a specific act); phuong vi -> Ky Mon (cuu cung, eight directions); chu-khach -> Thai At / Ky Mon / Luc Nham (host-guest); vi mo -> Thai At (macro, long-range).
...
6. The templates feed FR-REPORT-002 (PDF) and FR-WEB-005 (report view) without changing the assembled report.
...
- Samples: each shipped sample report validates against the FR-REPORT-001 schema.
...
Depends on FR-REPORT-001 (the `StructuredReport` whose sections it reorders; read-only). Aligns with FR-QMDG-007 (dung than by question type - the same mapping the templates encode) and FR-STRAT-003 (the chu-khach template mirrors the chu-khach decision frame). Consumed by FR-REPORT-002 (PDF) and FR-WEB-005 (report view). Nothing depends on it (blocks empty).
...
- Overlap with FR-QMDG-007 (the dung than mapping). Default: QMDG-007 owns the engine-side dung than palace; REPORT-003 owns the presentation mapping; both cite the same Claude-07 s1 source.
...
| Sample drifts from schema | a shipped sample is invalid | the sample validates against the FR-REPORT-001 schema in CI |
| Layout hides the AI boundary | a template interleaves engine + AI content | the section order keeps the deterministic and AI sections distinct (FR-REPORT-002 renders them separated) |

## §11 - Notes

Package `tamthuc_report` (Python, DEC-2). Report templates give each question type the report shape that fits it - trach thoi leads with the window, chu-khach with the two-sided posture, macro with the long-range trend - while keeping REPORT-001 as the single assembler and staying read-only over the assembled report (Grok-08, strategy 4.3). Each template also encodes the Claude-07 s1 mapping from question type to system and dung than as a shipped, documented artifact (timing -> Ky Mon, direction -> Ky Mon cuu cung, competitor / risk -> Thai At / Ky Mon / Luc Nham, macro -> Thai At), so the presentation reasons are written down. Feeds the PDF export (FR-REPORT-002) and the report-view screen (FR-WEB-005).
