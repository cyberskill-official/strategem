...
related_frs: [FR-KB-002, FR-KB-003, FR-RAG-004, FR-RAG-006, FR-AUTH-002, FR-PLAT-003, FR-API-004]
depends_on: [FR-KB-002]
blocks: []
new_paths:
...
This FR is the human curation workflow over knowledge-base content: an expert (master) reviews the interpretation patterns (FR-KB-002) and the classical excerpts (FR-KB-003) through a review queue, accepts or rejects each with a required reason, and every accepted change bumps a monotonic version and appends to a retained history. It is the authoring-time human-in-the-loop for the knowledge base, and it encodes the RISK-9 "expert review each release" process as a gate. It does NOT author the patterns (FR-KB-002) or the corpus (FR-KB-003); it reviews, versions, and signs off on exactly those rows.

A reviewable object SHALL be either a `pattern` (a FR-KB-002 row) or a `classical_unit` (a FR-KB-003 unit). A submission SHALL create a `ReviewItem` in state `in_review` carrying a snapshot of the object version under review; the queue SHALL surface pending items to a reviewer holding the master/expert role (FR-AUTH-002). A decision SHALL be `accept` or `reject`, SHALL carry a non-empty `reason` in both cases, and SHALL be recorded as an immutable `ReviewDecision` with the reviewer id and timestamp (an audit trail). On `accept` the object's version SHALL increment and the object SHALL become eligible for the active set (a pattern becomes loadable into `knowledge_patterns`; a unit becomes loadable into the corpus); on `reject` the object SHALL stay out of the active set with the reason retained. A reviewer SHALL NOT decide their own submission where the role model distinguishes author and reviewer.
...
class ReviewDecision(BaseModel):
    ...
    object_type: ReviewObjectType
    object_id: str
    reviewer_id: str            # must hold the master/expert role (FR-AUTH-002)
...
`GateReport` lists `signed`, `unsigned`, and `unchanged` objects; a non-empty `unsigned` fails the gate. The gate is run in CI at release time (FR-PLAT-004) and is the checkable form of the RISK-9 per-release expert review.
...
6. Wire the role check to FR-AUTH-002 (master/expert), and the active-set effects to FR-KB-002 (`knowledge_patterns` load) and FR-KB-003 (corpus load).

## §7 - Dependencies

Depends on FR-KB-002 (the 150-200 patterns this workflow reviews and versions; FR-KB-002's own open question defers the sign-off workflow to this FR). Reviews FR-KB-003 classical units through the same queue. Uses FR-AUTH-002 for the master/expert reviewer role and FR-PLAT-003 for the `review_queue` / `review_decision` tables and audit rows (soft edge; the physical tables are applied through the PLAT runner). Blocks nothing in the catalog, but gates releases (the RISK-9 per-release expert review) and feeds FR-RAG-006, which stamps the object versions it evaluated. Sibling to FR-RAG-004: same human-in-the-loop principle, different object (knowledge content here, AI interpretations there).
...
- Whether the release gate blocks the deploy or only warns at MVP. Default: warn-with-named-unsigned in P2, harden to a blocking gate before the public heritage-education launch (align with FR-LEGAL-004 sign-off).
...
| Unauthorized reviewer | a non-master role decides | rejected; only the master/expert role (FR-AUTH-002) may decide |
...
This FR turns the knowledge base from a set of committed files into reviewed, versioned, signed scholarship, which is what a heritage-education product must be able to claim. It is marked SHOULD at P2 because the P0/P1 seed can ship with a lighter "cited and curator-checked" bar (FR-KB-002's default), but the release gate is the piece that makes "expert review each release" (RISK-9) real rather than aspirational, so wire it into CI as soon as it exists. Keep it a clean sibling of FR-RAG-004 - do not merge the two queues; they review different objects for different reasons even though both express the human-in-the-loop rule. The package `tamthuc_kb` is shared with FR-KB-001/002/003/005; this FR adds the `curation/` module, so the knowledge base stays one installable, mypy-clean unit.
