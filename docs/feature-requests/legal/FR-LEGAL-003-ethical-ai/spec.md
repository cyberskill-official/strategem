...
related_frs: [FR-RAG-003, FR-LEGAL-001, FR-LEGAL-004, FR-PLAT-002, FR-KB-003, FR-WEB-003]
...
Present schools evenhandedly; state the convention (co_truong_phai) a reading used; never assert a school is uniquely correct; when schools diverge, name the divergence rather than hide it. The co_truong_phai flag discipline (FR-PLAT-002) is the technical form of this rule.
...
def check_ethics(interp: "Interpretation") -> list[EthicsFinding]:
    ...
    #     over beginner + expert + recommendations; reuse the FR-LEGAL-001 copy-policy lexicon
...
Policy (`ethics-checks.md`): high-severity findings (advice verdict, certain-future) block the output; medium-severity findings (fairness, attribution, fear/dependency) route to the FR-RAG-004 HumanReviewGate.

## §4 - Acceptance criteria

1. The three rule docs exist: language do/don't (extending FR-LEGAL-001), school fairness (naming co_truong_phai as the technical form), and source attribution (Han beside transliteration + translation).
2. `check_ethics` runs over a RAG-003 `Interpretation` and returns findings per family; a clean output yields no findings.
3. Language check: a certain-future / medical / legal-financial / fear-dependency phrasing is flagged, aligned with the FR-LEGAL-001 copy-policy and the RAG-003 framing guard.
...
- Alignment: the language check shares the FR-LEGAL-001 forbidden-lexicon spec (one source of forbidden phrasing); a test asserts parity with the RAG-003 framing-guard policy set.
- Attribution: a citation card missing `han` while its source unit has Han fails; the three-layer card shape is checked against FR-KB-003.
...
1. `ethical-ai-rules.md`: the four forbidden categories restated for AI output; reference FR-LEGAL-001.
2. `school-fairness.md`: the evenhandedness rules; co_truong_phai as the technical form (FR-PLAT-002).
...
Depends on FR-RAG-003 (the `Interpretation` output the checks run over; extends its framing guard). Extends FR-LEGAL-001 (the forbidden-lexicon copy-policy and positioning rules). Reads the FR-PLAT-002 co_truong_phai discipline as the technical form of school fairness. Aligns with FR-KB-003 and FR-WEB-003 (the three-layer citation card the attribution rule checks). Reviewed at FR-LEGAL-004. Nothing hard-depends on this FR (blocks empty); it is an enforcement layer the RAG path and the review gate call.
...
|---|---|---|
| Certain-future / advice verdict | interpretation asserts a fixed future or a medical/legal/financial ruling | high-severity, blocked (strategy 7, FR-LEGAL-001) |
...
| Uncited claim | a claim with no citation | flagged (defense in depth with FR-RAG-003) |
...
Package hook in `tamthuc_rag` (Python, DEC-2). This FR is the cultural-and-ethical layer stated as content rules plus automated checks, sitting on top of the RAG-003 framing guard and the FR-LEGAL-001 lexicon. Its throughline is that respect-the-source and cite-the-source are one rule (Claude-07 s4.3): the Han beside transliteration and translation is both scholarship and honesty. School fairness is not a slogan but the co_truong_phai flag discipline seen in prose (strategy 4.4, RISK-2). Reviewed at FR-LEGAL-004 before launch. refs Claude-07 s4.1, s4.3, s2.3.
