...
related_frs: [FR-RAG-003, FR-PLAT-002, FR-STRAT-001, FR-STRAT-004, FR-WEB-003, FR-LEGAL-003]
...
Imports `AIDisclosure` from the FR-RAG-003 schema so the shapes never drift.
...
3. Step four is a handoff, not a recommendation: it contains no imperative verdict and no certain-future / medical / legal / financial claim (aligned with FR-LEGAL-003).
...
- Framing guard: an adversarial interpretation with a verdict phrase does not leak into step four; findings align with the FR-LEGAL-003 checks.
...
- Dung than selection: from the question type (the FR-QMDG-007 mapping) or user-specified parties? Default: seed from the question type + interpretation, let the user relabel parties (`role_label` is free text).
...
|---|---|---|
| Verdict in step four | the handoff carries a recommendation / certain-future | forbidden; step four is a handoff; framing guard + FR-LEGAL-003 |
