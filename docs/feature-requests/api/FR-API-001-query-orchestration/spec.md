...
related_frs: [FR-AUTH-001, FR-AUTH-002, FR-CORE-005, FR-QMDG-006, FR-RULE-003, FR-RAG-003, FR-REPORT-001, FR-API-003, FR-API-004, FR-PLAT-002]
depends_on: [FR-AUTH-001, FR-CORE-005]
blocks: [FR-API-002, FR-API-003, FR-API-004, FR-WEB-002]
new_paths:
...
| 403 | `FORBIDDEN_TIER` | tier lacks the capability (FR-AUTH-002) |
| 404 | `NOT_FOUND` | unknown report id |
| 422 | `UNPROCESSABLE` | valid shape, un-castable input |
| 429 | `RATE_LIMITED` | quota exceeded (FR-API-003) |
| 500 | `INTERNAL` | unexpected |
| 501 | `NOT_IMPLEMENTED` | strategic endpoint pending STRAT |
| 502 / 503 | `UPSTREAM_ENGINE` / `UPSTREAM_LLM` | engine or LLM failure (FR-PLAT-008 degradation) |
...
`calculate.py` maps `/calculate/{system}` to `run_query`; `/calculate/all` fans out to every engine and returns the multi-chart response. `knowledge.py` serves `GET /knowledge/patterns?system=` from FR-RULE-003 / the seeded set. `reports.py` serves `POST /reports/generate` and `GET /reports/{id}/download` (FR-REPORT-001/002). `timing.py` exposes `/timing/optimize` and `/scenario/compare` contracts, delegating to STRAT or returning `NOT_IMPLEMENTED`.
...
4. `/calculate/all` returns one chart per system and a single fused response; `/calculate/qimen` for a Free principal is allowed, `/calculate/all` for a Free principal is 403 `FORBIDDEN_TIER` (FR-AUTH-002 capability).
...
1. Create the `tamthuc_api` package (`pyproject.toml`, `uv`-managed per FR-PLAT-001); this FR owns its birth, FR-API-002/003/004 add modules.
2. `app.py`: the FastAPI app, router mount, the FR-AUTH-001 auth dependency, the exception handlers that render the error envelope, and the `request_id` middleware.
...
- Sync vs async report generation for `/reports/generate`. Default: synchronous inline assembly at P0 (REPORT-001 is P1; the orchestrator returns an inline summary until then); an async job with `GET /reports/{id}` polling is the P1 shape. Coordinate with FR-REPORT-001.
- Whether `/calculate/all` interpretation is one fused reading or per-system readings plus an agreement view. Default: per-system charts now, with cross-system agreement deferred to FR-STRAT-004; the response shape already carries a list of charts so adding the agreement view is additive.
...
| Upstream failure leaks 500 | engine/LLM error surfaced as opaque 500 | mapped to 502/503 `UPSTREAM_*` with degradation per FR-PLAT-008, not a generic 500 |
| Tier bypass | a capability-gated endpoint reached without the tier | FR-AUTH-002 dependency on every gated route; 403 `FORBIDDEN_TIER` |

## §11 - Notes

This is the P0 keystone: the end-to-end demo (ask -> cast QiMen -> detect -> cited interpretation -> chart) runs through this orchestrator, and its one inviolable rule is that the gateway orchestrates but never computes. Keep the la so envelope read-only here, keep the nine steps explicit and in order, and keep every error in one envelope. The package `tamthuc_api` is shared with FR-API-002/003/004; they add versioning, rate limiting, and persistence/audit to the same app, so the gateway is one installable, mypy-clean unit. The strategic endpoints ship their contract now and their implementation in P1, so the frontend never has to change its calls when STRAT lands.
