...
related_frs: [FR-RULE-001, FR-RULE-003, FR-RULE-004, FR-PLAT-002, FR-QMDG-005]
depends_on: [FR-RULE-001]
blocks: [FR-RULE-003, FR-RULE-004, FR-QMDG-005]
new_paths:
...
A missing field path SHALL evaluate to a defined result, never an error or panic: for `eq`, `in`, `gte`, `lte`, and `contains` a missing or null path yields `false`; `exists` is the explicit presence test. Scoring: a matched pattern's score SHALL default to the pattern `confidence` (FR-RULE-001); the DSL MAY carry an optional per-node `weight` reserved for FR-RULE-004, ignored at MVP. This FR provides the parser, the path resolver, the evaluator, and the scorer; FR-RULE-003 wraps them with loading, caching, and per-system filtering.
...
```rust
use laso_envelope::{LaSo, CachCuc};   // FR-PLAT-002 types
...
`as_addressing_root()` presents the envelope so paths resolve per the root rule above. The emitted `CachCuc` is exactly the envelope shape from FR-PLAT-002 so the interpretation branch consumes detected patterns with no adapter.
...
5. `score_match` emits a `CachCuc` with `score = confidence` and the pattern citations when the tree matches, and `None` when it does not; the emitted type is byte-identical to an FR-PLAT-002 `CachCuc`.
...
- `tests/dsl_eval.rs`: a truth-table suite over the six operators; nested-tree cases; the missing-path matrix; the QiMen worked example evaluated against `fixtures/laso_qimen_sample.json` (a real la so envelope emitted by the QMDG engine or hand-built to the FR-PLAT-002 schema).
...
- Schema: `docs/contracts/condition-dsl.schema.json` validates every `conditions` tree in the FR-RULE-001 seed and rejects malformed trees; a test cross-checks that `Cond::from_json` accepts exactly what the schema accepts.
- Cross-type: `score_match` output deserializes as a `laso_envelope::CachCuc` (contract with FR-PLAT-002).
...
4. `score.rs`: `score_match`, `resolve_cung`, the RULE-to-envelope `Polarity` conversion; depend on the `laso-envelope` crate (FR-PLAT-002).
5. Build `fixtures/laso_qimen_sample.json` and wire the truth-table, property, and schema-parity tests.

## §7 - Dependencies

Depends on FR-RULE-001 (the `Pattern` type and the `conditions` field this FR parses) and, for the emitted `CachCuc`, on FR-PLAT-002 (the la so envelope crate). Blocks FR-RULE-003 (the loader/match API wraps this evaluator), FR-RULE-004 (cross-system nesting extends this grammar and the reserved `weight`), and FR-QMDG-005, whose cach cuc detection runs this evaluator over the QiMen plate through the FR-RULE-003 loader.
...
```json
// emitted CachCuc (FR-PLAT-002 shape) when the tree matches
...
- Where does `cung` on a multi-palace pattern come from - a fixed hint field on the pattern, or resolved from a field path at match time? Default: an optional `cung` hint on the pattern row, else `None`; FR-RULE-004 may compute it for multi-palace cross-system patterns.
...
| Empty connective | `{ "type": "and", "rules": [] }` | `from_json` is a `DslError`; the pattern fails FR-RULE-001 review before load |
