...
related_frs: [FR-PLAT-003, FR-PLAT-004, FR-PLAT-010, FR-LEGAL-002]
...
This FR is backup and disaster recovery for the data tier: automated backups, point-in-time recovery, a multi-AZ deployment with cross-region failover, and a recurring restore drill that proves the backups are actually restorable. It protects the FR-PLAT-003 schema (users, queries, charts, knowledge_patterns, reports, audit_logs). It owns the backup policy, the recovery targets, and the DR runbooks; it does NOT own the schema (FR-PLAT-003) nor the infrastructure provisioning (FR-PLAT-010), though it constrains both (backups run against the PLAT-003 tables; the multi-AZ/cross-region topology is realized in the PLAT-010 IaC).
...
Daily automated logical + physical backups of the FR-PLAT-003 database; continuous WAL archiving for PITR; a documented retention window; encryption at rest; access restricted to the DR role. `cron/pg-backup.sh` is the scriptable path when not using a managed backup service; a managed Postgres (per the strategy 4.1 note that any managed Postgres works) satisfies this via its native automated backups + PITR.
...
A recurring drill restores a backup into an isolated environment, applies PITR to a target instant, and asserts: the FR-PLAT-003 tables/indexes exist, RLS is enabled and fail-closed (the no-GUC probe sees zero rows), and a data-integrity sample matches; it fails if the restore exceeds the RTO or the assertions fail.
...
2. A restore drill restores a backup + PITR into an isolated environment and verifies the FR-PLAT-003 schema, fail-closed RLS (the no-GUC probe sees zero rows), and a data-integrity sample; the drill fails loudly if any check fails.
...
6. Backups are within the FR-LEGAL-002 erasure scope: a right-to-erasure is applied across backups on the documented cadence, so an erased subject does not persist indefinitely in backup copies.
...
- Gates: the restore-drill workflow is required in ops CI; the DR runbooks are reviewed; `actionlint`/`hadolint` where applicable via the FR-PLAT-004 pipeline.
...
5. Wire the erasure-across-backups cadence to FR-LEGAL-002; confirm backup encryption + separate-region storage in the FR-PLAT-010 topology.

## §7 - Dependencies

Depends on FR-PLAT-003 (the schema and the fail-closed RLS the backups protect and the drill verifies; the restore drill asserts the same no-GUC probe PLAT-003 defines). Realized on the topology FR-PLAT-010 provisions (multi-AZ + cross-region), so the two coordinate: PLAT-010 builds the infrastructure, PLAT-009 sets the backup/recovery policy on it. Coordinates with FR-PLAT-004 (the drill runs as a scheduled ops workflow alongside the pipeline) and FR-LEGAL-002 (erasure must reach into backups on a documented cadence so RISK-5 data does not outlive a right-to-erasure).
...
- Retention window length. Default: 30 days PITR + a longer cold-snapshot tail for the legally required minimum; the exact numbers are set with FR-LEGAL-002's retention policy, not hardcoded here.
- Erasure-across-backups mechanism: crypto-shred (drop the per-record wrapped key so ciphertext in backups is unreadable) vs scheduled backup rewrite. Default: crypto-shred for birth_data (the wrapped DEK is dropped, so all backup copies become unreadable at once), aligned with the FR-AUTH-001 envelope-encryption design and FR-AUTH-004's erasure.
...
| Backup privacy hole | backups readable or outside erasure scope | backups carry the live protection posture and are in the FR-LEGAL-002 erasure scope (RISK-5) |
...
The point of this FR is the drill, not the backup: a backup nobody has restored into a working, RLS-enforced, schema-correct database within the RTO is an assumption, so the restore drill is scheduled, asserted, and allowed to fail loudly. Hold the two numbers (RPO 1h, RTO 4h) as real commitments the drill measures, keep backups in a separate region/account with the same protection as live data, and keep them inside the FR-LEGAL-002 erasure scope so RISK-5 data does not outlive a right-to-erasure. This FR is IaC/ops: it sets policy and runbooks on the topology FR-PLAT-010 provisions and the schema FR-PLAT-003 owns.
