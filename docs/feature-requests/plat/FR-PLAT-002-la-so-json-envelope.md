...
related_frs: [FR-CORE-005, FR-QMDG-006, FR-LN-006, FR-TAT-006, FR-RAG-002, FR-API-001]
depends_on: [FR-PLAT-001]
blocks: [FR-CORE-005, FR-QMDG-006, FR-LN-006, FR-TAT-006, FR-RULE-002, FR-RAG-002, FR-API-004]
new_paths:
...
The la so (chart) JSON envelope is the single boundary contract between the deterministic engine branch (Rust) and the interpretation branch (Python), per strategy section 4.3. This FR defines that envelope once and ships it as (a) a Rust crate `laso-envelope` of serde types, (b) a Python package `laso_envelope` of Pydantic models generated from or checked against the same JSON Schema, and (c) a canonical JSON Schema at `docs/contracts/laso-envelope.schema.json`.
...
pub struct LaSo {
    ...
    pub dau_vao: DauVao,
    pub lich_phap: LichPhap,             // the full CORE output (FR-CORE-005)
...
Depends on FR-PLAT-001 (workspace layout that hosts `crates/` and `packages/`). Blocks every engine assembly FR and the interpretation branch, because they all (de)serialize this type.
...
This FR is the contract that makes DEC-2 (hybrid stack) safe. Do it before any engine assembly. The `lich_phap` sub-object is defined in full by FR-CORE-005; this FR only fixes its slot and the version/stamp/cache-key rules. Keep the schema file under `docs/contracts/` so it is reviewable independently of either language's build.
