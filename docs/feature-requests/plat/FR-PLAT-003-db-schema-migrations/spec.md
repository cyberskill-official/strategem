...
related_frs: [FR-PLAT-001, FR-PLAT-006, FR-AUTH-001, FR-RULE-001, FR-API-004, FR-KB-002, FR-LEGAL-002]
depends_on: [FR-PLAT-001]
blocks: [FR-RULE-001, FR-API-004, FR-PLAT-006, FR-PLAT-009, FR-AUTH-001]
new_paths:
...
This FR defines the PostgreSQL data tier (strategy 4.1) as forward-only, checked-in migrations: the six tables the platform persists - `users`, `queries`, `charts`, `knowledge_patterns`, `reports`, `audit_logs` - plus GIN indexes on their JSONB columns and fail-closed row-level security (RLS) for tenant/user isolation. It owns the schema and its migration history; it does NOT own how rows are written (FR-API-004 persistence, FR-AUTH-001 user creation) nor the cache (FR-PLAT-006), though every writer targets this schema.

The migrations SHALL be forward-only and ordered (`NNNN_name.sql`), each idempotent enough to run once in sequence, with a documented, human-run apply path. The schema SHALL store the la so envelope (FR-PLAT-002) in `charts.envelope` as `jsonb`, the rule conditions (FR-RULE-001) in `knowledge_patterns.conditions` as `jsonb`, and the structured interpretation (FR-RAG-003) in `reports.interpretation` as `jsonb`. Every JSONB column that is queried by containment SHALL carry a GIN index (`0008`).

RLS SHALL be fail-closed: every table that holds user-scoped data SHALL have `ENABLE ROW LEVEL SECURITY` and `FORCE ROW LEVEL SECURITY`, and SHALL default to deny - no permissive policy means no row is visible. Access SHALL be granted only through explicit policies keyed on a per-request session variable (`app.current_user_id`, and `app.current_role` for the admin bypass), following the cyberos RLS pattern. A connection that has not set the session variable SHALL see zero rows, never all rows. Personal data (birth data, question text) is sensitive: `users.birth_data_encrypted` SHALL be stored as ciphertext (AES-256, encrypted by FR-AUTH-001, opaque to the DB), the schema SHALL support soft-delete / erasure for VN PDPD and GDPR (FR-LEGAL-002), and sensitive access is auditable via `audit_logs` (strategy 4.4, RISK-5).
...
-- 0005_knowledge_patterns.sql   (owned jointly with FR-RULE-001; this FR ships the table)
...
The app SHALL set `app.current_user_id` (and `app.current_role` where relevant) per request/transaction (`SET LOCAL`) from the authenticated principal (FR-AUTH-001). `current_setting(..., true)` returns NULL when the GUC is unset, and `NULL::uuid = user_id` is never true, so an unset session sees no rows - the fail-closed property. `knowledge_patterns` is world-readable (seeded classical knowledge, FR-KB-002) but admin/curator-write only.
...
7. `users.birth_data_encrypted` is `bytea` (ciphertext), and the schema supports soft-delete (`deleted_at`) so erasure (FR-LEGAL-002) is expressible.
...
- Gates: the migration + RLS tests run in the CI `python`/integration lane (FR-PLAT-001) against a service Postgres container; a failing isolation assertion fails the build.
...
5. `db/rls/session.md`: document the `SET LOCAL app.current_user_id` contract FR-AUTH-001 / FR-API-004 must honor per request.
6. `db/tests/test_rls_isolation.sql` + the CI wiring; `db/migrations/README.md` apply path.

## §7 - Dependencies

Depends on FR-PLAT-001 (the repo and CI that run the migration tests). Blocks FR-RULE-001 (the `knowledge_patterns` table and its `conditions` JSONB are this schema), FR-API-004 (query/chart/report/audit persistence writes these tables and sets the RLS session variable), FR-PLAT-006 (the Redis chart cache is keyed off `charts.cache_key`), FR-PLAT-009 (backup/PITR/restore drill runs against this schema), and FR-AUTH-001 (user rows and the encrypted birth-data column). Coordinates with FR-LEGAL-002 (erasure/export operate on this schema) and FR-PLAT-002 (the envelope stored in `charts.envelope`).

## §8 - Example payloads

```sql
-- per-request preamble the app runs inside the transaction (FR-API-004 / FR-AUTH-001)
...
- Vector storage location: pgvector in this Postgres vs a separate vector DB (strategy 4.1 lists Chroma/pgvector/Pinecone). Default: pgvector as an extension in this instance for MVP (FR-RAG-001 owns the embedding tables); this FR reserves the extension slot in `0001` if pgvector is chosen.
...
| Plaintext birth data | birth data stored unencrypted | forbidden; column is `bytea` ciphertext (AES-256 by FR-AUTH-001), DB never sees plaintext |
| Irreversible delete blocks erasure | hard-delete only | soft-delete `deleted_at` supports the erasure/export contracts (FR-LEGAL-002) |

## §11 - Notes

The RLS discipline is the point of this FR: fail-closed, forced, deny-by-default, admin as an explicit policy - the same pattern cyberos uses, chosen because the blast radius here is personal divination data (RISK-5). Keep migrations forward-only and the schema shapes that another module owns (`envelope`, `conditions`, `interpretation`) as `jsonb` under GIN, so those contracts evolve under their own versioning rather than under DDL. The per-request `SET LOCAL app.current_user_id` contract is the seam FR-AUTH-001 and FR-API-004 must honor on every connection; document it in `db/rls/session.md` and treat a code path that talks to Postgres without setting it as a defect.
