# Tam Thuc Strategem - feature request catalog

Master index of every feature request for the Tam Thuc Strategem product. Source of rationale: `../strategy/tam-thuc-unified-plan-2026-07-08.md` (read section refs in each FR). Implementation order, status, and the agent trigger live alongside this catalog: `IMPLEMENTATION_ORDER.md`, `backlog.yaml`, `PROMPT.md`, `LEDGER.md` (see the section at the end).

87 FRs across 16 modules, roughly 900 engineering-hours, sequenced into four phases (P0 and P1 = MVP). IDs never renumber.

## Conventions

- Path: `feature-requests/<module-lowercase>/FR-<CODE>-<NNN>-<slug>.md`.
- Priority is MoSCoW: MUST, SHOULD, COULD.
- Phase is P0..P3 (see strategy section 6). Effort is engineering-hours (rough).
- `depends_on` lists FR IDs that must be `done` (or `in_review` with reviewer waiver) before this FR is eligible.
...
- Every engine FR emits the la so JSON envelope (strategy 4.3) and stamps its full school-flag set. Every AI FR is retrieval-grounded, cited, and carries AIDisclosure.

## FR document template

Every FR file uses this shape (heavyweight - the contract must be complete enough for an agent to implement without re-deriving from the strategy report):
...
depends_on: [...]
blocks: [...]
new_paths: [...]                                  # files/dirs this FR creates
---
...
| Code | Module | Lang | FRs | Phase focus |
|---|---|---:|---|---|
...
| FR | Pri | Phase | h | depends_on | Title |
|---|---|---|--:|---|---|
...
At greenfield stage the FRs are the plan, so the build order and the agent trigger live here alongside the catalog rather than in a separate `improvement/` folder. `docs/improvement/` is reserved for the post-launch audit and evolution stage (the cyberos convention), and is not created yet.
...
Task id = FR id (task `QMDG-001` implements `FR-QMDG-001`). No second numbering scheme. The FR is the task spec; it carries its own acceptance criteria (section 4) and verification (section 5), so there are no separate task cards.
...
- Acceptance criteria in the FR are binding; a task is not `in_review` until every criterion has evidence (test output, oracle-diff number, screenshot path, commit hash), recorded in `LEDGER.md`.
- Gates by language (DEC-2):
  ...
  - Engines additionally run the oracle cross-check (kinqimen / kinliuren / kintaiyi / sxwnl / tyme4py) per the FR.
