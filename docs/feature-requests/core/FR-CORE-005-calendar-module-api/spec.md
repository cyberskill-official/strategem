...
related_frs: [FR-CORE-002, FR-CORE-003, FR-CORE-004, FR-CORE-006, FR-PLAT-002]
depends_on: [FR-CORE-002, FR-CORE-003, FR-CORE-004]
blocks: [FR-CORE-006, FR-QMDG-001, FR-LN-001, FR-TAT-001, FR-API-001]
new_paths:
...
This FR is the public face of the calendar core: one entry point that takes a civil moment, a timezone, a longitude, and the calendar flags, and returns the `lich_phap` object - the exact sub-object that the la so envelope (FR-PLAT-002) carries and that all three engines read as their calendar input (Claude-05 s6, strategy 4.3). It composes FR-CORE-001 (tiet khi), FR-CORE-002 (chan thai duong), FR-CORE-003 (tu tru), and FR-CORE-004 (phai sinh) into a single serializable result.

The module SHALL expose one primary function that accepts `(datetime, tz, longitude, LichFlags)` and returns a `LichPhap` value. It SHALL define the canonical `LichFlags` set (the single source of truth that FR-CORE-001..004 borrow fields from) and SHALL stamp the complete flag set into `lich_phap.co_lich_phap` on every call. The output SHALL contain: `tu_tru`, `tiet_khi { hien_hanh, bat_dau, tam_nguyen }`, `chan_thai_duong`, `phai_sinh { tuan_khong, vuong_suy, truong_sinh }`, and `co_lich_phap`. The serialized shape MUST be byte-identical to the `lich_phap` slot defined by FR-PLAT-002; a change to one without the other is a contract defect.
...
```rust
pub struct LichPhap {
    pub tu_tru: BonTru,                     // FR-CORE-003
    pub tiet_khi: TietKhiHt,                // hien_hanh + bat_dau + tam_nguyen (FR-CORE-001)
    pub chan_thai_duong: ChanThaiDuong,     // FR-CORE-002
    pub phai_sinh: PhaiSinh,                // FR-CORE-004
...
### Serialized shape (Claude-05 s6.1; MUST match the FR-PLAT-002 lich_phap slot)
...
1. `tinh_lich_phap` returns a `LichPhap` whose serialized JSON is byte-identical to the FR-PLAT-002 `lich_phap` golden slot for the reference input (2004-01-01T10:30:00 +07:00, longitude 106.7, default flags).
...
- `tests/lichphap_golden.rs` holds the reference-input golden JSON and asserts round-trip (serialize -> deserialize -> serialize) byte stability and equality to the committed `lich_phap` slot from FR-PLAT-002's fixtures.
...
- Schema conformance: validate the serialized object against `docs/contracts/laso-envelope.schema.json` (shared with FR-PLAT-002).
...
3. `api.rs`: `tinh_lich_phap` - call tiet khi (FR-CORE-001), chan thai duong (FR-CORE-002), bon tru (FR-CORE-003), phai sinh (FR-CORE-004) in order, then stamp `co_lich_phap`.
4. Add the golden fixture (kept in sync with FR-PLAT-002's `lich_phap` slot) and the flag-isolation matrix.

## §7 - Dependencies

Depends on FR-CORE-002, FR-CORE-003, FR-CORE-004 (the three producers it composes; FR-CORE-001 arrives transitively through them). Fills the `lich_phap` slot fixed by FR-PLAT-002 - the two MUST agree on the serialized shape, so change them together. Blocks FR-CORE-006 (the oracle harness runs against this API), FR-QMDG-001 / FR-LN-001 / FR-TAT-001 (every engine's first slice reads this object), and FR-API-001 (the orchestrator resolves the calendar context here).
...
- Should `tam_nguyen` (thuong/trung/ha) be computed here or left to QMDG? Decision (inherited from FR-CORE-001 §9): this object carries the raw current term + its instant and a coarse tam_nguyen marker; the QiMen-specific thuong/trung/ha for dinh cuc is computed in FR-QMDG-001 from the phu dau. Keep the two aligned when QMDG-001 lands.
...
This FR and FR-PLAT-002 are two views of one contract: PLAT-002 fixes the slot and the version/cache-key rules, this FR fills the slot and owns the `LichFlags` set. Keep the golden `lich_phap` JSON identical in both crates' fixtures so a drift is a failing test, not a production surprise. Same crate `cyberos-lichphap` - this FR adds `api.rs`, `flags.rs`, `output.rs` and turns the calendar core into one callable, testable unit.
