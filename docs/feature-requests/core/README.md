...
- Depends on: FR-PLAT-001 (the cargo + uv + Next.js workspace). CORE-001 and CORE-007 root here; everything else in the module chains off CORE-001.
- Fills a contract slot: FR-CORE-005's output IS the `lich_phap` sub-object of the la so JSON envelope (FR-PLAT-002). The two are one contract seen from two sides - PLAT-002 fixes the slot and the version / stamp / cache-key rules, CORE-005 fills it and owns the `LichFlags` set. Keep their `lich_phap` fixtures byte-identical.
- Blocks every engine: FR-QMDG-001, FR-LN-001, FR-TAT-001 each read the calendar object as their first input (via CORE-005), and FR-QMDG-006 / FR-LN-006 / FR-TAT-006 reuse the CORE-006 oracle harness as the calendar half of their own oracle gate. FR-API-001 resolves the calendar context here. The relation-consuming engine slices (QMDG-005, LN-002, TAT-003) build on CORE-007.

## Module notes

- The crate is `cyberos-lichphap`. All seven FRs live in and extend this one crate - CORE-002..007 add modules (`truesolar.rs`, `pillars.rs`, `derived.rs`, `api.rs`, `ganzhi.rs`, ...) rather than spawning new crates, so the calendar core is a single testable unit.
