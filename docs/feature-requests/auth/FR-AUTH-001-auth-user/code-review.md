...
- Persistence is protocol + `InMemoryUserStore` for unit tests; production Postgres wiring is FR-API-004. DDL lives under AUTH migrations + PLAT-003 coordination.
- Social OIDC uses JWT audience/issuer verify (Authlib-shaped); production JWKS adapter is FR-PLAT-007 / follow-on.
- `/auth/me` never returns `birth_data`.

## Recommendation

**Approve** review acceptance: `APPROVE review FR-AUTH-001`
