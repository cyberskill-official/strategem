...
| RAG-004 | MUST | P1 | 12 | RAG-003 | (planned) | HumanReviewGate pipeline (queue, approve/reject, audit) |
| RAG-005 | SHOULD | P2 | 10 | RAG-002 | (planned) | Term-sense expansion (ban nghia / dan than / gia ta / dien tich) |
| RAG-006 | MUST | P2 | 12 | RAG-003, KB-002 | (planned) | Interpretation eval loop (faithfulness/relevance/citation) + CI gate |
| RAG-007 | MUST | P1 | 8 | RAG-003 | (planned) | LLM fallback + circuit breaker + rule-based degradation |

Three P0 FRs are authored (RAG-001..003, the ingest -> retrieve -> interpret spine). Four are listed for the dependency picture and authored later: RAG-004 (HumanReviewGate, P1), RAG-005 (term-sense expansion of văn ngôn văn queries - bản nghĩa / dẫn thân / giả tá / điển tích, P2), RAG-006 (the interpretation eval loop + CI gate, P2), and RAG-007 (LLM fallback / circuit breaker / rule-based degradation, P1).
