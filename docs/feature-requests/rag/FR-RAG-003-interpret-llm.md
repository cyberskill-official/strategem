...
related_frs: [FR-RAG-002, FR-RAG-004, FR-RAG-006, FR-RAG-007, FR-PLAT-002, FR-WEB-003, FR-REPORT-001, FR-STRAT-003, FR-LEGAL-003]
depends_on: [FR-RAG-002]
blocks: [FR-RAG-004, FR-RAG-006, FR-RAG-007, FR-WEB-003, FR-REPORT-001, FR-STRAT-003]
new_paths:
...
This is the interpretation core: it reads a la so envelope plus the passages FR-RAG-002 retrieved, builds a grounded prompt, calls an LLM, and returns a structured, cited interpretation. It is where the "AI does not invent numbers, and does not assert beyond its sources" principle is enforced (Claude-06 s4.1, s4.3; strategy 4.4). The module SHALL read the la so envelope (FR-PLAT-002) read-only and SHALL NEVER write `ban`, `cach_cuc`, `lich_phap`, or `co_truong_phai`; it only interprets what the deterministic engine produced (strategy 4.3).
...
class StructuredLLM:
    ...
    # temperature; on invalid JSON, one repair retry, then raise. FR-RAG-007 wraps this with
...
def interpret(la_so: LaSo, request: RetrievalRequest, persona: str,
    ...
    retrieved = retriever.retrieve(request)          # FR-RAG-002 (read chart context)
...
7. No output contains a medical/legal/financial verdict on a policy test set of adversarial questions (framing guard, aligned with FR-LEGAL-003).

## §5 - Verification

- `tests/test_interpret.py`: full `interpret` with a stub `LLM` returning canned structured outputs (good, fabricated-citation, uncited-recommendation, empty-retrieval, and advice-verdict cases) plus the FR-RAG-002 stub retriever; asserts each guard fires; asserts the read-only envelope invariant; asserts AIDisclosure/citation-card completeness.
...
- The measurable-quality gate (faithfulness/relevance/citation accuracy) is FR-RAG-006's eval loop; this FR provides the hooks (`prompt_version`, `retrieved_citation_ids`) it scores against.
...
Depends on FR-RAG-002 (the grounded passages it interprets) and FR-PLAT-002 (the la so envelope it reads). Blocks the AI-facing product: FR-RAG-004 (HumanReviewGate consumes `requires_human_review` and the AIDisclosure), FR-RAG-006 (eval loop scores this output), FR-RAG-007 (fallback wraps the LLM caller), FR-WEB-003 (results screen renders the interpretation + citation cards + AIDisclosure), FR-REPORT-001 (report assembly), and FR-STRAT-003 (chu-khach framework builds on interpretation).
...
- Claim-to-citation mapping granularity: sentence-level attribution vs paragraph-level. Default: require each `recommendation` and each interpretation paragraph to carry >=1 citation; sentence-level attribution is a FR-RAG-006 quality refinement, not a P0 blocker.
- The `requires_human_review` policy (confidence threshold, high-stakes question types). Default: a conservative threshold plus a question-type allowlist; the actual gate and queue are FR-RAG-004. Tune from review outcomes.
...
- Term-sense expansion of the query (bản nghĩa / dẫn thân / giả tá / điển tích) is FR-RAG-005 and widens retrieval upstream of this FR; not in P0 scope here.
...
| Advice verdict | model gives medical/legal/financial ruling | framing guard + system rule 4; policy test set; blocked (FR-LEGAL-003) |
...
| Invalid JSON | model returns prose or bad JSON | one repair retry, then raise; FR-RAG-007 provides fallback/degradation |

## §11 - Notes

This FR is the responsible-AI heart of the product and the reason the deterministic/AI split exists at all. Two invariants are non-negotiable: it reads the la so and never writes it, and it makes no claim without a retrieved citation. The eval loop (FR-RAG-006) turns those invariants into a measured gate (faithfulness, relevance, citation accuracy - RISK-3, RISK-9); the HumanReviewGate (FR-RAG-004) puts a person behind the important calls; the fallback (FR-RAG-007) keeps a graceful degradation when the LLM is unavailable. Keep the prompt library versioned - a change to `system.md` is a change to the product's safety posture and must ship as a reviewed diff with a bumped `prompt_version`.
