...
related_frs: [FR-LN-002, FR-LN-003, FR-LN-004, FR-LN-006, FR-CORE-001, FR-CORE-005, FR-PLAT-002]
depends_on: [FR-CORE-005]
blocks: [FR-LN-002, FR-LN-003, FR-LN-004, FR-LN-006]
new_paths:
...
This FR builds the root state of every Dai Luc Nham chart: the two-layer thien dia ban and the nguyet tuong that rotates it. It owns the birth of the `cyberos-luchnham` crate; FR-LN-002..006 extend it.
...
The module SHALL emit its result (dia ban, thien ban, nguyet tuong, gio chiem, board state) into the `ban` slot of the la so JSON envelope (FR-PLAT-002) under `he = "luc_nham"`, and SHALL stamp the LN flag set into `co_truong_phai`. The oracle for the whole engine is kinliuren.
...
Nguyet tuong is called out separately because it is the classic silent bug. Nguyet tuong is the sun's apparent zodiacal position reduced to a chi, and it steps at the twelve trung khi (Vu Thuy, Xuan Phan, ...), whereas the can-chi month steps at the twelve jie (Lap Xuan, Kinh Trap, ...). Take the wrong marker and the nguyet tuong is off by one, the thien ban is off, and every khoa and truyen is wrong while still looking plausible. Reading the trung khi from CORE (which computes term instants from solar longitude, FR-CORE-001) keeps LiuRen and the calendar core on the same clock.
...
Two special rotations that FR-LN-003 keys off (detected here, resolved there): when `nguyet_tuong == gio_chiem` the offset is 0, thien ban coincides with dia ban - phuc ngam (伏吟). When they are xung (opposite, offset 6) the thien ban is the dia ban turned exactly six palaces - phan ngam (返吟). This FR returns the board state marker; the special tam truyen laws are LN-003's.
...
6. The emitted `ban.thien_dia_ban` round-trips through the la so envelope (FR-PLAT-002) under `he = "luc_nham"`, and `co_truong_phai` carries the stamped LN flag set.
...
1. Create the `cyberos-luchnham` crate (this FR owns its birth; FR-LN-002..006 add modules).
...
Depends on FR-CORE-005 (the calendar module API supplies the tu tru and the current trung khi that sets nguyet tuong; the TrungKhi/Jie distinction comes from FR-CORE-001). Blocks FR-LN-002 (tu khoa reads the thien ban and ky cung), FR-LN-003 (tam truyen reads the thien ban and the phuc/phan ngam state), and FR-LN-004 (thien tuong are placed on the thien ban). Emits into the FR-PLAT-002 envelope; FR-LN-006 assembles and gates the whole engine.
...
{ "envelope_version": 1, "he": "luc_nham",
  ...
  "lich_phap": { "...": "from FR-CORE-005; tiet_khi.hien_hanh is a TrungKhi term" },
  "ban": {
    "thien_dia_ban": {
