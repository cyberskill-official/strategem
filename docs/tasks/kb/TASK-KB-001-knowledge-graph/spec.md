...
related_frs: [TASK-KB-002, TASK-KB-003, TASK-KB-005, TASK-RULE-001, TASK-RAG-002, TASK-PLAT-003]
depends_on: [TASK-PLAT-001]
blocks: [TASK-KB-002, TASK-KB-005]
new_paths:
...
Storage SHALL be pluggable behind a `GraphStore` protocol. The MVP default SHALL be a relational edge table `l2_edge(src, rel, dst, attrs)` alongside an `l2_node(id, kind, ...)` table in the shared Postgres (TASK-PLAT-003), matching the cyberos house pattern - no separate graph database is introduced at MVP. Neo4j, an RDF triple store, and a native property-graph adapter SHALL be reserved behind the same protocol so the store can be swapped later without touching callers. Every write SHALL validate that `kind` and `rel` are members of the closed enums; an unknown kind or relation SHALL be rejected, not stored.

The graph SHALL serve two consumers through one shape (Claude-06 s3): it is the structured rule source the deterministic layer reads (ngũ hành and địa chi relations that patterns reference), and it is one half of hybrid RAG retrieval - the graph arm that TASK-KB-005 exposes and TASK-RAG-002 fuses with the vector arm.
...
1. Create the `tamthuc_kb` package (`pyproject.toml`, `uv`-managed per TASK-PLAT-001); this task owns its birth, TASK-KB-002/003/005 add modules.
...
Depends on TASK-PLAT-001 (the Python workspace). Soft edge to TASK-PLAT-003: the `l2_node` / `l2_edge` DDL is owned here (KB is the natural owner of the graph tables) but is applied through the PLAT migration runner and lives in the same shared Postgres; the hard `depends_on` stays PLAT-001 per the master catalog, and the coordination with PLAT-003 is a reconciliation note, not a build blocker. Blocks TASK-KB-002 (pattern conditions reference graph node ids and relations) and TASK-KB-005 (the hybrid-retrieval query API traverses this graph). Read at runtime by TASK-RAG-002 (the graph arm of hybrid retrieval, via KB-005) and referenced by the deterministic relation logic that TASK-CORE-007 and TASK-RULE-002 encode.
...
- Node id namespacing across systems. Default: primitives shared by all systems (can, chi, ngũ hành) are `system="all"` with a single id; system-specific nodes (a QiMen-only cách cục) carry the `system` tag and a system-prefixed id. Decide the exact prefix convention with TASK-KB-002.
...
This task is marked SHOULD, not MUST, at P0: the P0 end-to-end flow can run with the ngũ hành / địa chi relations that TASK-CORE-007 encodes directly, and RAG-002 degrades to vector-only until TASK-KB-005 activates the graph arm. But building the schema in P0 pays for itself immediately, because TASK-KB-002's pattern conditions and TASK-CORE-007's relation checks both want a single source of truth for "what relates to what". Keep the boundary clean: this task owns the graph shape and store; TASK-KB-002 fills the cách cục / khóa thể / thần sát nodes and the interpretation patterns; TASK-KB-003 owns the classical text the citations resolve into. The package `tamthuc_kb` is shared with TASK-KB-002/003/005; they extend it, so the knowledge base is one installable, mypy-clean unit.
