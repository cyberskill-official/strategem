...
The structured-knowledge layer of the interpretation branch: the graph of the three systems' primitives and relations, the 150-200 cited interpretation patterns, the three-layer classical-text store, the curation workflow, and the hybrid-retrieval query API. It is the source of truth the deterministic engines read for relations and the RAG branch retrieves from for meaning - one half of hybrid retrieval, joined to the vector arm in TASK-RAG-002. Language is Python (DEC-2); everything lives in one package, `tamthuc_kb`. Primary sources: Claude 06 s3 (knowledge graph, RAG on classical text), Grok 37 (pattern seeding), Grok 24 (curation). See the unified plan sections 4.1, 5, and RISK-9.

## tasks

| task | Pri | Phase | h | Title |
|---|---|---|--:|---|
| KB-001 | SHOULD | P0 | 12 | [Knowledge-graph schema (node + edge taxonomy) + storage](TASK-KB-001-knowledge-graph/spec.md) |
| KB-002 | MUST | P0 | 16 | [Pattern seeding (150-200 patterns, 3 systems, JSON conditions + citations)](TASK-KB-002-pattern-seeding/spec.md) |
...
Two P0 tasks are authored (KB-001 the graph schema, KB-002 the pattern seed). Three are authored: KB-003 (the three-layer classical-text store RAG-001 ingests, P1), KB-004 (the expert-review and versioning workflow over the seeded patterns, P2), and KB-005 (the graph query API that is RAG-002's graph arm, P2).
...
- Depends on RULE: KB-002 fills the pattern-as-data shape and seed-file format that TASK-RULE-001 owns, validated against the TASK-RULE-002 DSL. The seed is the ruleset TASK-RULE-003 serves to the engines.
- Blocks RAG: KB-003 is what TASK-RAG-001 ingests and embeds; KB-005 is the graph arm TASK-RAG-002 fuses with the vector arm; KB-002's cited patterns enrich the TASK-RAG-003 chart summary and are the TASK-RAG-006 eval corpus (RISK-9). Citations on every pattern resolve into the KB-003 corpus.
- Feeds the engines: the ngu hanh / dia chi relations in the KB-001 graph are the reference the deterministic relation logic (TASK-CORE-007, TASK-RULE-002) reads, so "what relates to what" has one source.

## Module notes

- One package, one installable unit. All five tasks live in and extend `tamthuc_kb` (graph, seed, corpus, curation, query modules), so the knowledge base is one mypy-clean, pytest-covered Python package, not five scattered pieces.
