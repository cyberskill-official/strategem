---
id: TASK-KB-003
title: "Classical-text three-layer store - per source the nguyen van Han / bach thoai / dich layers segmented into natural units (dieu/phap/khoa/cau), each unit carrying citation_id + system + source; relational store in shared Postgres; the canonical corpus TASK-RAG-001 ingests and every citation resolves into"
module: KB
priority: MUST
status: done
phase: P1
...
related_frs: [TASK-RAG-001, TASK-KB-002, TASK-KB-005, TASK-RAG-003, TASK-PLAT-003, TASK-EDU-003]
depends_on: [TASK-PLAT-001]
blocks: [TASK-RAG-001, TASK-KB-005]
new_paths:
...
This task is the source-of-truth store for the classical text of Tam Thuc: per source work it holds three parallel layers - nguyen van chu Han (original), ban bạch thoại (modern-Chinese rendering), and ban dịch (Vietnamese/English translation) - segmented into the natural bibliographic units of the thư tịch, each unit carrying a stable citation id. It is the corpus that TASK-RAG-001 ingests and embeds, and the corpus that every citation in a pattern (TASK-KB-002) or an interpretation (TASK-RAG-003) resolves into. This task owns the canonical text and its unit segmentation; it does NOT own the embeddings or the vector index (TASK-RAG-001) nor the knowledge graph (TASK-KB-001).

Text SHALL be segmented by the natural unit of the work - điều (條), pháp (法), khoá (課), or câu (句) - never by fixed token windows (Claude-06 s4.2). Every unit SHALL carry `unit_id`, `source`, `system` (qimen | liuren | taiyi | all), `unit_type`, `citation_id`, `ordinal`, and up to three layer texts (`han`, `bach_thoai`, `dich`), of which at least one SHALL be present. The three layers of a unit SHALL be aligned by construction (one unit row, three layer fields) so a retrieval can return the Han and its translation as the same passage. Segmentation SHALL keep a unit whole: a source is divided at natural-unit boundaries only, and a unit is never split mid-sentence to fit any downstream constraint (sub-unit splitting for embedding context is TASK-RAG-001's concern, not this store's).

The store SHALL be pluggable behind a `ClassicalStore` protocol whose MVP default is two relational tables - `classical_source` and `classical_unit` - in the shared Postgres (TASK-PLAT-003), matching the cyberos house pattern of keeping stateful data in the one managed Postgres. It SHALL expose a read API that yields units for TASK-RAG-001 to ingest and a `resolve_citation(citation_id)` that returns the unit(s) a citation points at, so TASK-RAG-003 can render citation cards (Han + bạch thoại + dịch + locator). Every `citation_id` a source declares SHALL be resolvable; a citation referenced by an active pattern (TASK-KB-002) that resolves to no unit is a dangling citation and SHALL be reported, because the chain source -> pattern -> cách cục -> cited interpretation must have no link where a citation has no text behind it (strategy 4.4).
...
Segmenting by natural unit rather than token window is a correctness choice that this store must get right because everything downstream inherits it. A điều or a khoá is a complete thought in these texts; a citation points at exactly such a unit, and TASK-RAG-001 embeds exactly such a unit. If the store split a unit arbitrarily, citations would point at fragments and retrieval would return half a rule. The store is relational at MVP for the same reason TASK-KB-001's graph is: any managed Postgres can host it, it needs no second stateful system to run and back up, and the queries this corpus needs (units of a source in order, a unit by id, a citation resolved to its unit) are trivial in SQL. The `citation_id` is deliberately a first-class field separate from `unit_id` so the citation-id prefixes that TASK-KB-002 agrees in `SOURCES.md` resolve here directly, keeping the pattern seed and the corpus aligned on one naming scheme rather than two.
...
# System (qimen | liuren | taiyi | all) is the shared enum defined in TASK-KB-001.
...
class ClassicalStore(Protocol):
    ...
    def iter_units(self, system: System | None = None) -> Iterator[ClassicalUnit]: ...  # TASK-RAG-001 ingest feed
    def resolve_citation(self, citation_id: str) -> list[ClassicalUnit]: ...  # citation cards (TASK-RAG-003)
...
`iter_units` is the feed TASK-RAG-001 consumes: it yields units as validated rows (dicts under the schema), which `ingest(source_rows: Iterable[dict], ...)` chunks per layer and embeds. The store emits JSON-shaped rows so no Python type crosses the package boundary; the contract is the schema, not an import.
...
5. `iter_units` yields rows valid under `docs/contracts/classical-corpus.schema.json`, and a round-trip through TASK-RAG-001's `ingest` chunks one unit into one chunk per present layer sharing the `unit_id` (the KB-003 -> RAG-001 seam works on the fixture).
6. A dangling-citation check reports any `citation_id` referenced by an active TASK-KB-002 pattern that resolves to no unit here; on the aligned fixture the report is empty.
...
- Schema conformance: every emitted row validates against `docs/contracts/classical-corpus.schema.json`, which is the same shape TASK-RAG-001 validates its ingest input against.
- Cross-FR integrity: the dangling-citation check runs against a small aligned TASK-KB-002 pattern fixture and asserts every active pattern's `citations` resolve here.
...
6. Author `data/corpus/qimen/yen_ba_dieu_tau_ca.json` (marked units, the P1 flagship source) and `data/corpus/SOURCES.md` aligned to the TASK-KB-002 citation-id prefixes; commit `fixtures/corpus_sample.json`.

## §7 - Dependencies

Depends on TASK-PLAT-001 (the Python workspace and, for the relational store, the shared Postgres from TASK-PLAT-003 - a soft edge like TASK-KB-001's graph tables). Blocks TASK-RAG-001 (which reads and embeds this corpus; its `Chunker` splits these units into per-layer chunks and its `citation_id`/`unit_id` come from here) and TASK-KB-005 (whose graph query collects citations that resolve into this store). Aligns with TASK-KB-002 on the citation-id prefixes: KB-002's `SOURCES.md` declares the prefixes, and this task's `data/corpus/SOURCES.md` provides the units those prefixes resolve into, so a cited pattern always has text behind it. Read at runtime by TASK-RAG-003 (citation cards) and TASK-EDU-003 (the bilingual classical library). Reconciliation note: TASK-RAG-001 declares `Layer` / `UnitType` enums in `tamthuc_rag`; both packages validate against `docs/contracts/classical-corpus.schema.json`, which this task owns, so the vocabularies are one contract enforced in CI, not two that can drift.
...
- How the Han text is normalized (traditional vs simplified, variant characters). Default: store the source's own form verbatim in `han`, keep a normalized field only if retrieval quality (TASK-RAG-006) shows variant-character misses; do not silently transcode the original.
- How much of a source ships before P1 vs stays `draft`. Default: only cited, curator-checked units load; the citation-id prefixes are fixed early (with TASK-KB-002) so patterns can reference units before every layer is translated. TASK-KB-004 owns the sign-off.
...
This task is the textual ground the whole anti-hallucination contract stands on: no citation card, no cited interpretation, and no dangling-citation check exists without a store that holds the classical text as cited, aligned, natural units. Keep the boundary clean - this task owns the canonical three-layer text and its unit segmentation; TASK-RAG-001 owns the embeddings and the vector index over it; TASK-KB-001 owns the graph. The one contract that binds this task to TASK-RAG-001 is `docs/contracts/classical-corpus.schema.json`; author it first and let both packages validate against it. The package `tamthuc_kb` is shared with TASK-KB-001/002/004/005; this task adds the `corpus/` module to it, so the knowledge base stays one installable, mypy-clean unit.
