...
related_frs: [TASK-RULE-001, TASK-RULE-002, TASK-RULE-003, TASK-KB-001, TASK-KB-003, TASK-KB-004, TASK-RAG-003, TASK-RAG-006]
depends_on: [TASK-RULE-001, TASK-KB-001]
blocks: [TASK-KB-004, TASK-RAG-006]
new_paths:
...
This task fills the pattern-as-data ruleset with real content: 150-200 named patterns across the three systems, each expressed as a JSON row conforming to the TASK-RULE-001 pattern shape (`docs/contracts/knowledge-pattern.schema.json`) and each carrying at least one citation into the classical corpus. It does NOT define the pattern schema (TASK-RULE-001) nor the condition DSL grammar (TASK-RULE-002); it authors data against them. The same 150-200 rows SHALL double as the interpretation-quality validation dataset named in strategy RISK-9.

The seed SHALL be language-neutral canonical data under `data/patterns/<system>.json`, one file per system, each a JSON array of pattern rows. Every row SHALL populate `id`, `system`, `name`, `name_han` where a Han name exists, `conditions` (a DSL tree valid under TASK-RULE-002), `polarity` (cát/hung/trung), `meaning_classical`, `meaning_modern`, `citations` (non-empty for any `active` row), `version` (>= 1), `confidence`, and `status`. No `active` row SHALL ship without a citation, because a pattern with no textual source cannot ground a claim under the anti-hallucination rule (strategy 4.4, TASK-RAG-003).
...
The build SHALL validate every row against the TASK-RULE-001 validator and the TASK-RULE-002 DSL checker before load, and SHALL load `active` rows into the `knowledge_patterns` table (TASK-PLAT-003). A malformed or uncited `active` row SHALL fail the whole build with the offending `id` named; nothing partial loads.
...
Each `citations` entry is a stable id that resolves into the TASK-KB-003 classical corpus. Provenance of the seed content:
...
`SOURCES.md` records, per source, the work, the citation-id prefix it owns, and the layer availability (Han / bạch thoại / dịch) so TASK-KB-003 can align the corpus to the citations these patterns reference.
...
def build(paths: list[Path]) -> BuildReport:
    ...
    # 2. validate every row with the TASK-RULE-001 validator (envelope + shallow conditions)
    # 3. deep-validate conditions with the TASK-RULE-002 DSL checker
...
def load(rows: list[Pattern], db) -> LoadReport:
    # upsert active rows into knowledge_patterns (TASK-PLAT-003), keyed by id; bump version on change
```
...
def validation_cases(system: System | None = None) -> list[ValidationCase]:
    ...
    # meaning_classical, citations} case; this is the fixed corpus TASK-RAG-006 scores
    # interpretation faithfulness and citation accuracy against (RISK-9).
```

## §4 - Acceptance criteria

1. `data/patterns/qimen.json`, `liuren.json`, and `taiyi.json` together contain 150-200 rows, every row valid under the TASK-RULE-001 validator and the TASK-RULE-002 DSL checker.
...
6. `validation_cases()` returns one case per seeded pattern with its `(id, version)` stamp, and TASK-RAG-006 can consume it as the RISK-9 corpus.
...
- Schema conformance: every row validates against `docs/contracts/knowledge-pattern.schema.json` (the TASK-RULE-001 contract), so the seed cannot drift from the row shape.
- DSL conformance: every `conditions` tree parses and deep-validates under the TASK-RULE-002 checker (fields, operators, depth), run as part of the build test.
...
5. `seed/validation.py`: the `validation_cases` projection for TASK-RAG-006.
...
Depends on TASK-RULE-001 (the pattern row shape, the validator, and the seed-file format this task fills) and TASK-KB-001 (pattern `conditions` reference graph node ids and relations, and the cách cục / khóa thể / thần sát nodes live in the graph taxonomy). Loads into the `knowledge_patterns` table that TASK-PLAT-003 migrates from the TASK-RULE-001 shape (soft edge; the physical table is PLAT-003's). Citations resolve into the TASK-KB-003 classical corpus (a parallel P1 build; the citation-id prefixes are agreed here in `SOURCES.md` so KB-003 can align to them). Blocks TASK-KB-004 (the curation workflow reviews and versions these rows) and TASK-RAG-006 (the eval loop scores interpretation against this seed as the RISK-9 validation set). Read at runtime by TASK-RULE-003 (the loader serving `active` patterns to the engines) and enriches the chart summary in TASK-RAG-003.
...
- How much of the seed is expert-signed before P0 ship vs marked `draft`. Default: rows ship `active` only once cited and reviewed; unreviewed rows stay `draft` and are excluded from the loaded ruleset but kept in the validation corpus for tracking. TASK-KB-004 owns the sign-off workflow.

## §10 - Failure modes inventory

| Mode | Trigger | Required behavior |
|---|---|---|
| Uncited active pattern | `status = active`, `citations = []` | build fails with the `id`; DB check constraint also rejects (TASK-RULE-001) |
...
| Invalid conditions | `conditions` tree malformed or uses an unknown field/operator | TASK-RULE-002 DSL checker fails the build at that row |
...
This is the content task that turns the pattern-as-data machinery into a working interpreter, and it is the single most leverage-heavy quality artifact in P0: the same 150-200 cited rows are the ruleset the engine detects, the answer key the eval loop grades, and the substrate the citation cards render. Keep it language-neutral data under `data/patterns/` so both the Rust engine (offline seed) and the Python DB loader consume one source; the small `crates/cyberos-rule/seed/qimen.json` from TASK-RULE-001 stays only as the format exemplar and fixture. Author QiMen first and cite everything - an uncited pattern is not shippable. TASK-KB-004 later adds the expert-review and versioning workflow over exactly these rows.
