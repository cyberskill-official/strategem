...
related_frs: [TASK-STRAT-002, TASK-STRAT-003, TASK-STRAT-004, TASK-QMDG-006, TASK-QMDG-007, TASK-RULE-003, TASK-PLAT-002, TASK-PLAT-006]
depends_on: [TASK-QMDG-006, TASK-RULE-003]
blocks: [TASK-STRAT-002]
new_paths:
...
The optimizer must call the engine and never cast its own chart, for the same reason the whole platform splits deterministic casting from everything else: if STRAT re-implements dinh cuc or plate placement, it will drift from the oracle-gated engine and quietly recommend the wrong hour. Scanning through TASK-QMDG-006 (with the TASK-PLAT-006 chart cache making repeat casts cheap) keeps every recommendation traceable to an oracle-exact chart.
...
2. For each candidate, cast via the TASK-QMDG-006 engine client (cache-keyed by TASK-PLAT-002 rules) -> la so envelope. Never compute plates in STRAT.
3. Select the relevant `cach_cuc` for `loai_cau_hoi` via TASK-RULE-003 (per-system filter + match); default all-palace until TASK-QMDG-007 (dung than by question type) narrows to the dung than palace.
...
2. Each candidate is cast via the TASK-QMDG-006 engine (a repeat scan hits the cache); a test asserts STRAT calls the engine and does not compute plates itself.
...
3. `timing_optimizer.py`: enumerate candidates by granularity; call the TASK-QMDG-006 engine client per candidate (cache-keyed); collect envelopes.
4. `scoring.py`: the documented weighting over `cach_cuc` polarity/score; TASK-RULE-003 selection for the question type; the optional phuong vi term.
...
Depends on TASK-QMDG-006 (casts each candidate chart and emits the `cach_cuc`) and TASK-RULE-003 (selects and weights the patterns that matter for the question type). Reads the TASK-PLAT-002 envelope and benefits from TASK-PLAT-006 (chart cache makes the scan cheap). Blocks TASK-STRAT-002 (Scenario Comparison compares `TimingResult`s across options). When TASK-QMDG-007 (dung than by question type) lands, the palace filter tightens from all-palace to the dung than palace.
...
- Default granularity: QiMen changes per gio (2h) and per cuc; default gio-level scan, with an optional coarser day-level pre-filter for long ranges (perf). Confirm against p95 once TASK-QMDG-006 timing is known; TASK-PLAT-006 cache makes the scan cheap.
- Cross-engine timing: should the optimizer also consult LiuRen for a concrete yes/no on the chosen time? Deferred to TASK-STRAT-004 (cross-system validate); STRAT-001 is QiMen-only, which is its trach thoi strength (Claude-07 s1.1).
...
|---|---|---|
| STRAT recomputes a chart | optimizer implements casting itself | forbidden; must call TASK-QMDG-006; a test asserts the engine is invoked |
