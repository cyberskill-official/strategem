...
related_frs: [TASK-STRAT-001, TASK-STRAT-003, TASK-STRAT-004, TASK-QMDG-006, TASK-PLAT-006, TASK-WEB-007]
depends_on: [TASK-STRAT-001]
blocks: []
new_paths:
...
1. For each `Scenario`, call the TASK-STRAT-001 optimizer with its `TimingRequest` -> `TimingResult` (which casts via the engine, cache-keyed). Never cast or score here.
...
5. Determinism: the same `ScenarioSet` twice yields an identical comparison; overlapping instants hit the TASK-PLAT-006 cache.
...
Depends on TASK-STRAT-001 (the Timing Optimizer whose `TimingResult`s it composes; reuses its `TimingRequest` / `ScoredWindow` models). Benefits from TASK-PLAT-006 (the chart cache makes N scenarios cheap). Reads the TASK-QMDG-006 engine only transitively through STRAT-001. Feeds TASK-WEB-007 (the management / compare flow). Sibling to TASK-STRAT-003 and TASK-STRAT-004. Nothing depends on it (blocks empty).
