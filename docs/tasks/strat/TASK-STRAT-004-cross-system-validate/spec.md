...
related_frs: [TASK-QMDG-006, TASK-LN-006, TASK-TAT-006, TASK-RAG-003, TASK-API-001, TASK-STRAT-001]
...
1. For each system in the request, call its engine (TASK-QMDG-006, TASK-LN-006; TASK-TAT-006 when present) with the shared question plus that system's flags -> la so envelope. Never re-cast.
...
4. Wire `/calculate/all` in the API (TASK-API-001); tests with stub engines and golden agreement fixtures.

## §7 - Dependencies

Depends on TASK-QMDG-006 and TASK-LN-006 (the two engines it runs; TASK-TAT-006 joins at P2 for the third system, which is why this task is P2). Reads the TASK-PLAT-002 envelopes and, optionally, TASK-RAG-003 interpretations per system. Exposed via TASK-API-001 (`/calculate/all`). Sibling to TASK-STRAT-001/002/003. Nothing depends on it (blocks empty).
...
- Two engines vs all three at MVP. Default: two (Ky Mon + Luc Nham); the third (Thai At) activates at TASK-TAT-006 (P2), which is why this task is P2.
