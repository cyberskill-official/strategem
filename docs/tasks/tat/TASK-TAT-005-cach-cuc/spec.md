...
related_frs: [TASK-TAT-001, TASK-TAT-002, TASK-TAT-003, TASK-TAT-006, TASK-PLAT-002, TASK-RAG-003]
depends_on: [TASK-TAT-003]
blocks: [TASK-TAT-006]
new_paths:
...
This task reads the positional relationships of a cast Thai At chart into named cach and into the four deterministic criteria that a host-versus-guest reading rests on. It extends the `cyberos-thaiat` crate, consuming the tuong and toan of TASK-TAT-003 and the Thai At seat of TASK-TAT-002. It produces facts only; the victory verdict itself belongs to the cited AI layer (Claude-04 s6.3, tat module notes).
...
These are not extra placements; after every component is seated (TASK-TAT-002 / TASK-TAT-003), one pass reads them from the relative positions of Thai At and the tuong and records them.
...
def luan_bon_tieu_chi(cac_toan, cach_cuc, ban):
    ...
    truong_doan = (cac_toan.chu_truong_doan, cac_toan.khach_truong_doan)  # from TASK-TAT-003 (>=11 / <=9)
...
pub fn map_to_envelope_cach_cuc(cach: &[CachCucTat]) -> Vec<CachCuc>;   // TASK-PLAT-002 CachCuc
```
...
5. Each recognized cach maps to an envelope `cach_cuc` entry with a stable id, its Han name, the palace, a polarity, and a citation to Kim Kinh Thuc Kinh / Thong Tong Bao Giam; the envelope `cach_cuc` array validates against TASK-PLAT-002.
...
- Oracle: `tests/cachcuc_oracle.rs` loads `fixtures/cachcuc_kintaiyi.csv` (generated once from kintaiyi, per epoch, over many charts covering all seven cach and the compounds) and asserts the recognized cach set matches exactly. Feeds the TASK-TAT-006 gate for the cach portion.
...
1. `cachcuc.rs`: the `Cach` enum, the position predicates (same palace, before / after, opposed), the Kich noi / ngoai split, the compound-cach detection, and `nhan_dien_cach_cuc` as a read-only pass over TASK-TAT-002 / TASK-TAT-003 positions.
2. `thangbai.rs`: `tinh_tam_tai`, the four-criteria aggregation `luan_bon_tieu_chi` (facts only), and `map_to_envelope_cach_cuc` producing TASK-PLAT-002 `CachCuc` with ids, Han names, polarity, and citations.
...
4. Extend the `he = "thai_at"` `ban` with `tam_tai`; feed the `cach_cuc` list to the envelope (the envelope array is filled by TASK-TAT-006 assembly).
...
Depends on TASK-TAT-003 (the tuong positions and the toan with their truong / doan labels) and transitively on TASK-TAT-002 (the Thai At seat and the ring) and TASK-TAT-001. Soft-feeds TASK-TAT-006: the assembly's MUST path emits a valid envelope with an empty `cach_cuc` and no `tam_tai`; this task enriches it with the recognized cach and tam tai, so it blocks the cach / tam-tai portion of the TASK-TAT-006 gate, not the yearly assembly itself. Feeds TASK-RAG-003 (the interpretation branch reads these facts and never writes them) - the cited victory reading is built there, not here.
...
```json
{ "ban": {
    "tich": { "...": "from TASK-TAT-001" }, "thai_at_cung": 1, "thai_at_ring": 14,
    "thap_luc_than": { "...": "TASK-TAT-002" }, "bat_tuong": { "...": "TASK-TAT-003" },
...
(The `cach_cuc` array is the envelope-level list of TASK-PLAT-002 `CachCuc`; `tam_tai` lives inside `ban`. No "who wins" field appears - that reading is the AI layer's, cited and AIDisclosure-labeled.)
