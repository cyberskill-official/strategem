...
related_frs: [TASK-TAT-001, TASK-TAT-002, TASK-TAT-003, TASK-TAT-006, TASK-CORE-001, TASK-CORE-005, TASK-PLAT-002]
...
This task generalizes the Thai At chart from one plate per year to four plates keyed to four time granularities: nien ke (year), nguyet ke (month), nhat ke (day), thoi ke (hour). It extends the `cyberos-thaiat` crate, reusing the epoch / reduction machinery of TASK-TAT-001 and the ring seating of TASK-TAT-002; only the tich formula and the don anchor differ per level.

The module SHALL compute a tich for each of the four levels and reduce it mod seventy-two into a cuc (Claude-04 s5): nien ke tich = 10,153,917 + CE, cuc = tich nien mod 72 (the TASK-TAT-001 formula, identical); nguyet ke tich = tich nien times twelve with its own leap-month handling, cuc = tich nguyet mod 72; nhat ke anchored on Dong Chi with tich nhat = accumulated-days-from-the-Giap-Ty-anchor times 365.2425, cuc = (tich nhat mod 72) + 1 then advanced by day; thoi ke tich = tich nhat times twelve, cuc = (tich thoi mod 72) + 1 then advanced by hour. One hour SHALL be one cuc and six days SHALL close one seventy-two-cuc circuit.

The module SHALL take the don direction for nhat ke and thoi ke from the Dong Chi / Ha Chi anchor (Claude-04 s5.2): after Dong Chi duong don runs forward (thuan), after Ha Chi am don runs backward (nghich). The Dong Chi and Ha Chi instants SHALL come from the calendar core (TASK-CORE-001 via TASK-CORE-005), the same solstice instants TASK-TAT-001 keys its don off, so TaiYi and the calendar core MUST agree on the solstice to the second. The nguyet ke leap-month handling SHALL use a dedicated tich nguyet solve, not a naive month multiply.

Each level SHALL reduce to the same 1..=72 cuc contract as TASK-TAT-001 (a tich divisible by seventy-two maps to 72, not 0, before the level-specific "+1" where the source calls for it). The module SHALL feed the resulting cuc and don into TASK-TAT-002 seating (and, in the assembly, TASK-TAT-003 tuong) so that any of the four levels produces a full chart. The module SHALL extend the `dau_vao.cap` selector and stamp the chosen level; the oracle is kintaiyi, which supports all four levels.

## §2 - Why this design (rationale for humans)

Thai At is not a single yearly plate: the same machine casts at four resolutions so a reading can range from the multi-year backdrop (nien ke) down to the hour (thoi ke), and the user picks the level that matches the scope of the question (Claude-04 s5.2). Because all four reduce to a cuc mod seventy-two and then seat Thai At the same way, the right design is to reuse TASK-TAT-001's reduction and TASK-TAT-002's seating unchanged and vary only the tich formula and the don anchor - which is why this task is small (SHOULD, 8h) despite adding three whole new plates.

The nhat ke and thoi ke anchors are the reason this task touches the calendar core. Their tich is measured from Dong Chi, and the don flips at Dong Chi (forward) and Ha Chi (backward), so the solstice instant has to be the exact same instant TASK-TAT-001 and TASK-CORE-001 use; a disagreement of even a day at the anchor shifts the cuc and re-casts the plate (RISK-1 propagation). Stating the shared-anchor requirement here keeps the four levels consistent with the yearly plate and with QiMen / LiuRen, which read the same solstices.
...
### Nien ke worked example (Claude-04 s5.1, ties to TASK-TAT-001)

Year 2004: tich nien = 10,153,917 + 2,004 = 10,155,921; mod 60 = 21 (Giap Than); mod 72 = 33 (duong don, cuc 33). Identical to the TASK-TAT-001 golden numbers - nien ke IS TASK-TAT-001's path, listed here as the first of the four levels.

### Pseudocode

```
def tich_theo_cap(cap, dau_vao, epoch, lich):     # lich supplies Dong Chi / Ha Chi (TASK-CORE-005)
    if cap == "nien_ke":
        tich = tich_nien(dau_vao.nam_ce, epoch)                      # TASK-TAT-001
    ...
    return TichCap(cap, tich, cuc, don)                              # cuc + don then feed TASK-TAT-002 seating
```
...
1. `tich_theo_cap(NienKe, 2004, KimKinh, ...)` reproduces the s5.1 numbers exactly: tich 10,155,921, cuc 33, Giap Than, duong don - identical to TASK-TAT-001.
...
6. The Dong Chi / Ha Chi instant used here is byte-identical to the one TASK-TAT-001 and TASK-CORE-001 use; a cross-check test asserts the same anchor.
...
- Oracle: `tests/bonphep_oracle.rs` loads `fixtures/bonphep_kintaiyi.csv` (generated once from kintaiyi across all four levels and both epochs, spanning years and including day / hour probes) and asserts the tich, cuc, and don match exactly per level. This extends the TASK-TAT-006 gate from per-epoch to per-epoch-and-per-level.
...
1. `bonphep.rs`: the `Cap` enum, the four tich formulas, and `tich_theo_cap` reusing TASK-TAT-001's `tich_nien` and 1..=72 mapping.
2. Nguyet ke: the leap-month tich nguyet solve (consult TASK-CORE for the lunisolar month index); do not flat-multiply.
3. Nhat ke / thoi ke: take the Dong Chi anchor from `lich` (TASK-CORE-005), apply the `days x 365.2425` and `x 12` tich, the `+1` and advance-by-day / hour, and the after-Dong-Chi / after-Ha-Chi don.
4. Route the resulting `(cuc, don)` into TASK-TAT-002 seating so any level yields a full chart; keep nien ke bit-identical to TASK-TAT-001.
...
Depends on TASK-TAT-002 (seating the cuc / don on the ring) and transitively on TASK-TAT-001 (the tich nien formula, the 1..=72 mapping, the epoch flag). Needs the Dong Chi / Ha Chi instants and the lunisolar month index from the calendar core (TASK-CORE-001 via TASK-CORE-005), reached transitively through TASK-TAT-001's CORE-005 dependency. Soft-feeds TASK-TAT-006: the assembly's MUST path (nien ke) does not hard-require this task, but the SHOULD extension of the oracle gate to all four levels does - so this task blocks the per-level portion of the TASK-TAT-006 gate, not the yearly assembly itself. Emits into the TASK-PLAT-002 envelope via `dau_vao.cap`.
...
{ "dau_vao": { "nam_ce": 2004, "cap": "thoi_ke", "datetime": "2004-01-01T10:30:00", "tz": "+07:00", "epoch": "kim_kinh" },
  "ban": {
    ...
    "bat_tuong": { "...": "from TASK-TAT-003, cast at this level's cuc" },
    "cac_toan": { "...": "from TASK-TAT-003" }
  },
  "lich_phap": { "...": "supplies the Dong Chi / Ha Chi anchor (TASK-CORE-005)" } }
```
...
- Nguyet ke leap-month handling is stated only as "chinh thang nhuan rieng" (its own leap-month solve); the exact rule (which leap months count, how the month index maps to tich nguyet) is taken from TASK-CORE's lunisolar month module and confirmed against kintaiyi.
...
| Solstice anchor disagrees with CORE | TaiYi computes its own solstice | cross-check test asserts the same instant as TASK-TAT-001 / TASK-CORE-001 |

## §11 - Notes

This task is small because it reuses everything: TASK-TAT-001's reduction and 1..=72 mapping, TASK-TAT-002's seating, and (in the assembly) TASK-TAT-003's tuong. It varies only the tich formula and the don anchor per level, so the four plates are one code path parameterized by `Cap`. Its value is coverage: it lifts the TASK-TAT-006 oracle gate from per-epoch to per-epoch-and-per-time-level, which is the tat module's stated acceptance bar. The shared Dong Chi / Ha Chi anchor is the load-bearing detail - the same solstice instant TaiYi, QiMen, and LiuRen all read - so a divergence here is a RISK-1 calendar-core problem, not a TaiYi problem. Nien ke stays bit-identical to TASK-TAT-001; the other three are the SHOULD extension.
