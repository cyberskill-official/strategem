...
related_frs: [TASK-PLAT-004, TASK-PLAT-007, TASK-PLAT-008, TASK-API-001, TASK-API-003, TASK-RAG-007]
depends_on: [TASK-PLAT-004]
blocks: [TASK-PLAT-008]
new_paths:
...
This task is the observability stack: the metrics, dashboards, structured logs, error tracking, product analytics, and alerting that make the running platform legible. It extends the deployed environments TASK-PLAT-004 defines with a Prometheus + Grafana metrics pipeline, Sentry error tracking, Loki log aggregation, PostHog/Mixpanel product analytics, and Alertmanager/PagerDuty alert routing. It owns the metric catalog, the dashboards, and the alert rules; it does NOT own the behaviors those alerts observe - resilience (TASK-PLAT-008), rate limiting (TASK-API-003), or security controls (TASK-PLAT-007) - though it is the instrument that makes them measurable.
...
Histograms carry the buckets needed to compute p50/p95/p99. Chart-generation and LLM latency are measured at the orchestrator seam (TASK-API-001), so they reflect the user-visible cast, not an internal sub-step.
...
PostHog/Mixpanel events (`query_cast`, `report_viewed`, `signup`, `upgrade`) keyed by a pseudonymous id, gated on the analytics-consent flag (TASK-LEGAL-002), carrying no sensitive payload.
...
- Gates: `ruff check`, `ruff format --check`, `mypy tamthuc_api`, `pytest packages/tamthuc_api`; `promtool check rules`; the alert YAML and dashboards lint in the TASK-PLAT-004 pipeline.
...
Depends on TASK-PLAT-004 (the deployed staging/production environments this instruments, and the pipeline that lints the alert YAML and dashboards). Blocks TASK-PLAT-008 (resilience: the circuit-breaker and degradation transitions must be observable here, and PLAT-008's health signals feed these alerts). Instruments TASK-API-001 (the orchestrator seam where chart-gen and LLM latency are measured), TASK-API-003 (rate-limit and abuse events surface as metrics and alerts), and TASK-RAG-007 (LLM fallback state feeds the `LLMDowntime` signal). Coordinates with TASK-PLAT-007 (security events are audited and alertable) and TASK-LEGAL-002 (the analytics-consent flag).
...
- Analytics vendor: PostHog (self-hostable, open) vs Mixpanel (managed). Default: PostHog at MVP for data-residency control (VN PDPD context), keeping the event schema vendor-neutral so a swap is a client change, not a schema change. Confirm with TASK-LEGAL-002.
...
- Where the expert-validation pass rate is computed. Default: TASK-RAG-006's eval loop produces the number and this task exposes it as `tt_expert_validation_pass_ratio`; until RAG-006 lands, the gauge reads from the KB-002 validation set run.
...
This task is the instrument, not the machine: it measures the platform but changes no product behavior. Hold three disciplines - three metric families (business, technical, quality) so growth, health, and correctness are all visible; sensitive data never in telemetry (RISK-5); and no silent alerts, everything routes to PagerDuty with a runbook. The Python instrumentation lives in the `tamthuc_api` package and the IaC (Prometheus/Grafana/Loki/Alertmanager config) under `deploy/observability/`, reflecting the iac/python split. TASK-PLAT-008 builds directly on this: its circuit-breaker and degradation transitions are only useful if they are observable here.
