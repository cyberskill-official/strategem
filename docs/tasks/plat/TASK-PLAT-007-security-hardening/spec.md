...
related_frs: [TASK-PLAT-004, TASK-AUTH-001, TASK-AUTH-002, TASK-API-003, TASK-API-004, TASK-LEGAL-002]
depends_on: [TASK-PLAT-004, TASK-AUTH-002]
blocks: []
new_paths:
...
|---|---|---|
| Spoofing | JWT authentication + rate limiting | TASK-AUTH-001 tokens; TASK-API-003 limiter |
| Tampering | input validation + signed calculations | `security/validation.py`, `security/signing.py` |
| Repudiation | audit logging (append-only) | TASK-API-004 `audit_logs` |
| Information disclosure | AES-256 at rest + least-privilege access | TASK-AUTH-001 crypto; TASK-PLAT-003 RLS + DB grants |
| Denial of service | rate limiting + autoscaling + WAF | TASK-API-003; TASK-PLAT-010 HPA; `deploy/security/waf-rules.md` |
| Elevation of privilege | RBAC | TASK-AUTH-002 roles/capabilities |
...
Snyk + Dependabot + Trivy across the Rust/Python/Node manifests and the built images; a HIGH/CRITICAL finding fails the TASK-PLAT-004 build; suppression only via a checked-in allowlist with owner + reason.
...
4. A dependency with a known HIGH/CRITICAL CVE fails the dependency-scan workflow and the TASK-PLAT-004 build; adding an allowlist entry with owner + reason passes it (proof the gate bites and the override is explicit).
5. Input validation rejects malformed/oversized/injection-shaped input with the TASK-API-001 error envelope; a signed calculation detects a tampered chart (the signature check fails on a mutated envelope).
6. The OWASP Top 10 checklist is complete for the release; the pentest schedule is defined (annual + post-change); the incident-response playbook encodes the GDPR 72h and VN PDPD notification clocks (TASK-LEGAL-002).
...
- Gates: `ruff check`, `ruff format --check`, `mypy tamthuc_api`, `pytest packages/tamthuc_api`; the security workflows run in the TASK-PLAT-004 pipeline.

## §6 - Implementation skeleton

1. Author `docs/security/stride-threat-model.md` mapping each STRIDE class to its control and evidence; identify the gaps this task must fill.
...
4. `.github/workflows/dependency-scan.yml`: Snyk + Dependabot + Trivy; fail on HIGH/CRITICAL; the allowlist convention (extends the TASK-PLAT-004 security scan).
...
Depends on TASK-PLAT-004 (the pipeline that runs the dependency and secret scans and where the build fails on a finding) and TASK-AUTH-002 (the RBAC/tier model that is the Elevation-of-privilege and the authorization control this hardens). Composes TASK-AUTH-001 (JWT + AES-256 at rest), TASK-API-003 (rate limiting as the DoS/Spoofing control), TASK-API-004 (audit as the Repudiation control), and TASK-PLAT-003 (RLS + least-privilege DB grants). Coordinates with TASK-PLAT-010 (autoscaling as a DoS control) and TASK-LEGAL-002 (the breach-notification obligations and the consent/retention contracts the playbook references).
...
- WAF choice: a managed WAF (cloud provider) vs an app-layer WAF (e.g. Coraza/ModSecurity rules). Default: the managed edge WAF at MVP for DoS + common-attack coverage, with app-layer input validation as defense in depth; revisit under TASK-PLAT-010.
- Secret manager: cloud KMS/Secrets Manager vs a self-hosted vault. Default: align with the TASK-AUTH-001 master-key custody decision (envelope encryption works with either); the birth_data master key and the app secrets share the chosen manager.
...
| Vulnerable dependency ships | HIGH/CRITICAL finding ignored | dependency-scan fails the TASK-PLAT-004 build; only a reviewable allowlist entry passes it |
...
This task is where the platform's legal footing becomes a set of enforced controls rather than intentions. Hold three things: the STRIDE map is the contract (every threat class -> a control with evidence), secrets and vulnerable dependencies die at the TASK-PLAT-004 pipeline (never in the repo, never HIGH/CRITICAL past the gate), and the incident-response playbook carries the real regulatory clocks (GDPR 72h, VN PDPD). Most controls are composed from AUTH/API/PLAT tasks; this task proves the composition and adds TLS 1.3, secret custody, dependency scanning, the OWASP review, the pentest cadence, and the playbook. The Python controls live in `tamthuc_api/security`, the operational artifacts under `docs/security/` and `deploy/security/`, reflecting the iac/python split. Oracle libraries stay CI-only so their licenses never enter the shipped artifact (RISK-6).
