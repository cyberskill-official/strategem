...
|---|---|---|--:|---|---|---|
| PLAT-001 | MUST | P0 | 12 | - | [TASK-PLAT-001](TASK-PLAT-001-monorepo-workspace/spec.md) | Monorepo + hybrid workspace (cargo + uv + Next.js) + CI skeleton |
| PLAT-002 | MUST | P0 | 10 | PLAT-001 | [TASK-PLAT-002](TASK-PLAT-002-la-so-json-envelope/spec.md) | La so JSON envelope contract (Rust+Python shared types, versioned, contract test) |
| PLAT-003 | MUST | P0 | 12 | PLAT-001 | [TASK-PLAT-003](TASK-PLAT-003-db-schema-migrations/spec.md) | DB schema + migrations + RLS + indexes (users/queries/charts/patterns/reports/audit) |
| PLAT-004 | MUST | P0 | 10 | PLAT-001 | [TASK-PLAT-004](TASK-PLAT-004-cicd-pipeline/spec.md) | CI/CD pipeline (lint/type/test, docker, security scan, staging->prod gate) |
| PLAT-005 | MUST | P1 | 10 | PLAT-004 | [TASK-PLAT-005](TASK-PLAT-005-observability/spec.md) | Observability (Prometheus/Grafana, Sentry, structured logs, alerting) |
| PLAT-006 | SHOULD | P1 | 8 | PLAT-003 | [TASK-PLAT-006](TASK-PLAT-006-caching/spec.md) | Redis caching (chart cache 24h TTL, invalidation, warming) |
| PLAT-007 | MUST | P1 | 12 | PLAT-004, AUTH-002 | [TASK-PLAT-007](TASK-PLAT-007-security-hardening/spec.md) | Security hardening (STRIDE controls, TLS 1.3, secrets, dep scan) |
| PLAT-008 | MUST | P1 | 8 | PLAT-005 | [TASK-PLAT-008](TASK-PLAT-008-resilience/spec.md) | Resilience (circuit breaker, retry/backoff, graceful degradation) |
| PLAT-009 | SHOULD | P2 | 8 | PLAT-003 | [TASK-PLAT-009](TASK-PLAT-009-backup-dr/spec.md) | Backup + DR (daily backup, PITR, RPO 1h / RTO 4h, restore drill) |
| PLAT-010 | SHOULD | P2 | 10 | PLAT-004 | [TASK-PLAT-010](TASK-PLAT-010-iac/spec.md) | Infra as code (Terraform + K8s manifests, autoscaling) |
...
PLAT-001 is the root of the whole program (it depends on nothing). PLAT-002/003/004 are the three P0 pillars on top of it; the P1-P2 tasks harden the running system.

## Cross-module dependencies

- Blocks essentially everything: PLAT-001 hosts every crate, package, and app; PLAT-002 is (de)serialized by every engine and the interpretation branch (`blocks` every engine assembly task and RAG); PLAT-003 backs RULE-001, API-004, AUTH-001, and the cache; PLAT-004 is the train every deploy rides.
- The la so envelope (PLAT-002) is the one cross-language contract: the Rust engines (CORE, QMDG, LN, TAT) emit it and the Python branch (RULE consumers, RAG, REPORT, API) reads it. TASK-CORE-005's calendar output IS the `lich_phap` sub-object; the two are one contract seen from two sides.
- The DB (PLAT-003) is the data tier for AUTH (users), API (queries/charts/reports/audit persistence, TASK-API-004), and RULE/KB (`knowledge_patterns`), with the erasure/export contracts of TASK-LEGAL-002 operating on the same schema.
