...
related_frs: [TASK-PLAT-005, TASK-API-001, TASK-API-003, TASK-RAG-003, TASK-RAG-007]
depends_on: [TASK-PLAT-005]
blocks: []
new_paths:
...
| DB write fails (persist) | request fails transactionally (no silent 200) | 500 `INTERNAL` / 503 per TASK-API-004 |

### Error mapping (`resilience/errors.py`)

Maps `BreakerOpen`, timeout, and upstream errors to the TASK-API-001 envelope codes (`UPSTREAM_LLM`, `UPSTREAM_ENGINE`) with the correct HTTP status; never surfaces a raw exception as an opaque 500.
...
5. Every surfaced failure is the TASK-API-001 error envelope with the correct status: a dependency failure is 502/503 `UPSTREAM_*`, not an opaque 500; a test asserts the code/status mapping.
6. Breaker transitions and retries emit metrics/signals to TASK-PLAT-005 (an operator can see a dependency degrade and recover).
...
- Integration: the TASK-API-001 orchestrator uses these primitives on the engine and LLM calls; a fault-injection run confirms degrade-not-fail behavior end to end and the emitted PLAT-005 signals.
...
4. `resilience/errors.py`: map breaker/timeout/upstream failures to the TASK-API-001 envelope codes and HTTP statuses.
5. Wire the primitives into the TASK-API-001 orchestrator's engine and LLM calls; emit transitions to TASK-PLAT-005; expose the LLM-specific application to TASK-RAG-007.

## §7 - Dependencies

Depends on TASK-PLAT-005 (observability: breaker/retry/degradation transitions must be observable, and they feed the LLM-downtime and error-rate alerts). Consumes the TASK-API-001 error-envelope contract (this task maps failures onto it, it does not redefine it) and plugs into the TASK-API-001 orchestrator's engine and LLM calls. Provides the shared resilience primitives TASK-RAG-007 applies to the LLM caller (RAG-007 is the LLM-specific instance of this library). Coordinates with TASK-API-003 (the limiter's fail-safe posture is the same principle applied to the rate-limit datastore) and TASK-RAG-003 (the rule-based fallback reuses the detected patterns and their citations).
...
- Breaker granularity: one breaker per external dependency vs per provider+model. Default: per dependency at MVP (one LLM breaker, one engine-service breaker); split per provider when TASK-RAG-007 adds multi-provider fallback so a single provider's outage does not open the whole LLM path.
...
- Retry budget interaction with the TASK-API-003 rate limit. Default: retries are internal and do not consume the user's quota (the quota counts user requests, not internal attempts); a retried upstream call is one metered request.
...
| Opaque 500 on upstream failure | dependency error surfaced as generic 500 | mapped to 502/503 `UPSTREAM_*` in the TASK-API-001 envelope |

## §11 - Notes

This task keeps the platform useful when its external dependencies are not. The reusable primitives - circuit breaker, retry-with-backoff, degradation policy, error mapping - live here as one shared library so TASK-API-001 and TASK-RAG-007 apply them instead of reinventing them. Two disciplines matter most: degrade, do not fail (partial multi-engine results; a cited rule-based interpretation when the LLM is down), and the degraded answer stays labeled and cited (the fallback is safe precisely because it has no generative step to hallucinate, but it still carries AIDisclosure and citations on this legally sensitive surface). It depends on TASK-PLAT-005 because a breaker that trips invisibly is worse than none - every transition is observable and alertable.
