...
related_frs: [TASK-PLAT-002, TASK-PLAT-003, TASK-PLAT-004, TASK-CORE-001, TASK-AUTH-001, TASK-WEB-001]
depends_on: []
blocks: [TASK-PLAT-002, TASK-PLAT-003, TASK-PLAT-004, TASK-CORE-001, TASK-CORE-007, TASK-KB-001, TASK-KB-003, TASK-RAG-001, TASK-AUTH-001, TASK-WEB-001]
new_paths:
  - Cargo.toml
  - rust-toolchain.toml
  - clippy.toml
...
This task creates the Tam Thuc Strategem monorepo and the hybrid workspace that hosts all three toolchains (strategy 3.2, DEC-2). It is the first task built: every other task's `new_paths` land inside the tree this task establishes, so it MUST exist before any engine, package, or screen. Nothing here casts a chart or serves a request - it fixes where code lives, how the three languages coexist, and how the root CI gates the lot.

The repository SHALL be a single monorepo containing three coordinated workspaces:

- a Rust cargo workspace rooted at `Cargo.toml`, with member crates under `crates/` - the calendar core `cyberos-lichphap` (TASK-CORE-001), the engines `cyberos-qimen` (and later `cyberos-luc-nham`, `cyberos-thai-at`), the rule engine `cyberos-rule` (TASK-RULE-002), the shared boundary crate `laso-envelope` (TASK-PLAT-002), and the engine service binary crate(s) that expose the engines over the la so envelope to the Python branch;
- Python packages under `packages/`, managed by `uv` as a workspace - `tamthuc_api` (TASK-API-001), `tamthuc_rag` (TASK-RAG-001), `tamthuc_kb` (TASK-KB-001), and `tamthuc_auth` (TASK-AUTH-001);
- a Next.js 14+ application under `apps/web` (TASK-WEB-001) using Tailwind and shadcn/ui, styled later by the CyberSkill Design System v1.3.0.
...
The Rust engine crates compile to a service the Python API calls; the boundary between the deterministic branch and the interpretation branch is the TASK-PLAT-002 envelope (strategy 4.3). This task fixes the two branches' homes (`crates/` and `packages/`) and the seam between them; it does NOT define the envelope shape (TASK-PLAT-002), the DB (TASK-PLAT-003), or the deploy pipeline (TASK-PLAT-004), though it establishes the CI skeleton those extend.

## §2 - Why this design (rationale for humans)

The stack is deliberately hybrid, not uniform (strategy 3.2, DEC-2). The engines must match reference oracles to the digit and be cargo-testable like the rest of CyberSkill, so they are Rust; the AI, RAG, orchestration, and report layers live where the LLM and embedding ecosystem is and where iteration is fastest, so they are Python; the frontend is Next.js. A single monorepo is chosen over three repos because the three toolchains share one contract (the la so envelope) and one release train: a monorepo makes a cross-language contract change - Rust type plus Python model plus a TypeScript view - one reviewable pull request with one CI run, instead of three repos drifting out of lock-step. That is exactly the drift the envelope contract test (TASK-PLAT-002) is built to catch, and it can only catch it if both sides live and build together (RISK-8).

The root CI gating all three lanes from day one is the point of this task, not an afterthought. If the three-toolchain gate is added later, code accretes that was never held to `-D warnings`, `mypy`, or `next lint`, and retrofitting the gate becomes its own project. Establishing the skeleton now, while the tree is empty, means every subsequent task is born under the gate. The workspace layout mirrors the cyberos convention (DEC-1) so a later absorption into cyberos is mechanical rather than a port.
...
Member crates are added by their owning task (TASK-CORE-001 births `cyberos-lichphap`, TASK-PLAT-002 births `laso-envelope`, etc.); this task ships the workspace manifest, `rust-toolchain.toml`, and `clippy.toml` so the first crate lands under the gate.
...
Each package under `packages/` carries its own `pyproject.toml` (owned by its task) and is a uv workspace member; `uv sync` resolves the whole set from one lockfile.

### Web app (`apps/web`)

Next.js 14+ App Router, TypeScript strict, Tailwind, shadcn/ui. This task scaffolds the app and its config (`next.config.mjs`, `tsconfig.json` with `strict: true`, `tailwind.config.ts`); TASK-WEB-001 installs the Design System v1.3.0 tokens and the component library on top.
...
6. `docs/contracts/` exists and is the agreed home for cross-language JSON Schemas (the envelope schema TASK-PLAT-002 lands here).
...
- Gates (this task is the gate skeleton): the three lanes above; TASK-PLAT-004 extends this workflow with docker build, security scan, and the staging -> prod deploy gate.
...
Depends on nothing - this is the root task of the whole program. Blocks TASK-PLAT-002 (the envelope crate + package live in this workspace), TASK-PLAT-003 (migrations run against this repo's tooling), TASK-PLAT-004 (the deploy pipeline extends this CI skeleton), TASK-CORE-001 and TASK-CORE-007 (the first Rust crates), TASK-KB-001 / TASK-KB-003 / TASK-RAG-001 (the first Python packages), TASK-AUTH-001 (`tamthuc_auth`), and TASK-WEB-001 (`apps/web`). Effectively every task in the catalog roots here transitively; the `blocks` list names the direct dependents.

## §8 - Example payloads

Not an API task - no request/response payload. The "payload" is the layout and the CI matrix in §3. A developer's first interaction:
...
- Node package manager: pnpm vs npm vs bun for `apps/web`. Default: pnpm (workspace-friendly, fast); the choice is local to the web lane and does not affect the Rust/Python halves. Revisit only if the deploy image (TASK-PLAT-004) argues otherwise.
- Whether the Rust engine reaches Python as an HTTP service, a PyO3 binding, or WASM (strategy 4.1, DEC-2). Default: an out-of-process service behind a thin Python client (see TASK-API-001 §9), so the boundary stays a network contract the read-only-envelope assertion can guard; the binding is a later optimization. This task only reserves the service bin crate's home under `crates/`.
...
|---|---|---|
| Gate added late | CI skeleton deferred past first code | forbidden; this task ships the three-lane gate before any engine/package/screen lands |
