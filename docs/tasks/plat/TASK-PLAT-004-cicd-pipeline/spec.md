...
related_frs: [TASK-PLAT-001, TASK-PLAT-003, TASK-PLAT-005, TASK-PLAT-007, TASK-PLAT-008, TASK-PLAT-010]
depends_on: [TASK-PLAT-001]
blocks: [TASK-PLAT-005, TASK-PLAT-007, TASK-PLAT-008, TASK-PLAT-010]
new_paths:
...
|---|---|---|
| `ci.yml` (TASK-PLAT-001) | push, pull_request | rust / python / web lanes (lint, typecheck, unit test) |
| `security-scan.yml` (this task) | pull_request, schedule | Trivy (fs + image), Snyk (deps); fail on high/critical |
| `cd.yml` (this task) | pull_request (integration), push to main (deploy) | integration tests -> build+push images -> deploy staging -> [approval] -> deploy production |

### Pipeline stages (`cd.yml`)

```
1. integration    services: postgres (TASK-PLAT-003), redis (TASK-PLAT-006 when present)
...
6. No secret is read from the repository; the pipeline sources secrets from the CI secret store / environment (TASK-PLAT-007), verified by a scan for committed secrets in the security workflow.
...
- The integration stage is shown to run the TASK-PLAT-003 RLS isolation test against a service Postgres and to fail the pipeline if isolation regresses.
...
- Gates: this task is the pipeline; its own YAML is linted (`actionlint`) in the CI web/util lane; Dockerfiles are linted (`hadolint`).
...
3. `cd.yml` integration job: spin up postgres (+ redis when TASK-PLAT-006 lands), apply migrations, run the RLS isolation + API integration tests.
...
Depends on TASK-PLAT-001 (extends its CI skeleton and reuses the `just` gate recipes). Exercises TASK-PLAT-003 in the integration stage (migrations + RLS isolation as a pipeline test). Blocks TASK-PLAT-005 (observability wires into the deployed environments this task defines), TASK-PLAT-007 (the security controls are enforced through this pipeline's scan and secret sourcing), TASK-PLAT-008 (resilience is deployed and smoke-tested here), and TASK-PLAT-010 (Terraform/K8s replace the compose-based staging target this task bootstraps). Consumes the secret store defined by TASK-PLAT-007.
...
- Registry and deploy target at MVP: GHCR + a compose-based single host (the cyberos deploy pattern) vs a managed platform. Default: GHCR images + a compose staging target now, so the pipeline is real before TASK-PLAT-010 introduces Terraform/K8s; the deploy step is a thin script the IaC task later replaces without changing stages 1-4.
...
| RLS regression ships | isolation test not in the pipeline | the TASK-PLAT-003 isolation test runs in the integration stage and blocks merge/deploy on failure |
| Secret in repo | credential committed | committed-secret scan fails; secrets are sourced from the store, never the repo (TASK-PLAT-007) |
