...
related_frs: [TASK-REPORT-002, TASK-REPORT-003, TASK-RAG-003, TASK-PLAT-002, TASK-PLAT-003, TASK-API-004]
depends_on: [TASK-RAG-003]
blocks: [TASK-REPORT-002, TASK-WEB-005]
new_paths:
...
This task assembles the two branches of the platform into one durable artifact: it takes the la so envelope the deterministic engine cast (TASK-PLAT-002) and the structured interpretation the RAG branch produced (TASK-RAG-003) and composes a single `StructuredReport` object, then persists it. It is step 7 of the nine-step query flow (strategy 4.2).
...
class Citation(BaseModel):            # shape shared with TASK-RAG-003
...
`assemble` is pure and does no I/O. Persistence is `repo.persist(report)`, which writes the TASK-PLAT-003 `reports` row and the audit row. The read-only rule is a hard invariant: `assemble` reads the envelope's chart fields and never returns an object whose chart fields differ from the source.
...
- Persistence test against a TASK-PLAT-003 test database (or a fake repo): `assemble` then `persist` then load round-trips; the audit row exists.
...
2. `models.py`: the models above; import `Citation` and `AIDisclosure` from the TASK-RAG-003 schema so the two never drift (single shape).
...
4. `repo.py`: `persist(report)` - write the `reports` row + audit row per TASK-PLAT-003; `load(report_id)`.
...
Depends on TASK-RAG-003 (its structured output is the interpretation, recommendations, citations, confidence, and AIDisclosure source, and its `Citation` / `AIDisclosure` shapes are reused). Reads the TASK-PLAT-002 la so envelope for the chart summary and detected patterns. Persists per TASK-PLAT-003 (the `reports` and audit tables; TASK-API-004 owns query/chart persistence, REPORT owns the report row). Blocks TASK-REPORT-002 (PDF export renders this object) and TASK-WEB-005 (the report-view screen).
...
- Does REPORT store the full envelope or reference the chart by `query_id`? Default: store the report and reference the chart (charts are persisted by TASK-API-004), to avoid duplicating the chart. If audit needs an immutable snapshot, store the envelope hash alongside and revisit.
