...
Feature requests for report generation: the module that assembles the cast chart plus the cited interpretation into one structured, persisted report, then renders it to PDF. 3 tasks, ~26 engineering-hours, P1-P2. Rationale: `../../strategy/tam-thuc-unified-plan-2026-07-08.md` (strategy 4.2 step 7, strategy 5). Primary sources: Grok 33 (report generation) and Grok 08 (sample reports). Language is Python (DEC-2); everything lives in one package, `tamthuc_report`. Implementation order and trigger: `../IMPLEMENTATION_ORDER.md` + `../PROMPT.md`.
...
Three tasks, ~26 engineering-hours. One (REPORT-001, structured report assembly) is authored; the other two are authored. The spine is REPORT-001 (assemble + persist the structured report object) -> REPORT-002 (PDF export) with REPORT-003 (per-question sample templates) alongside.

## task list

| task | Pri | Phase | h | Title |
|---|---|---|--:|---|
| [REPORT-001](TASK-REPORT-001-structured-report/spec.md) | MUST | P1 | 10 | Structured report assembly (chart + patterns + interpretation + citations) |
...
- Depends on RAG: all three tasks stand on TASK-RAG-003 - its structured output (beginner / expert interpretation, recommendations, citations, confidence, AIDisclosure) is the interpretation half of the report. REPORT never re-prompts the LLM and never re-runs retrieval.
- Depends on PLAT: REPORT reads the TASK-PLAT-002 la so envelope for the chart summary and detected patterns, and persists per TASK-PLAT-003 (the `reports` table plus an audit row, strategy 4.2 step 9).
