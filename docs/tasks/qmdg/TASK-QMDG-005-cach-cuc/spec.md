...
related_frs: [TASK-QMDG-004, TASK-QMDG-006, TASK-RULE-002, TASK-PLAT-002]
depends_on: [TASK-QMDG-004, TASK-RULE-003]
blocks: [TASK-QMDG-006]
new_paths:
...
This task detects cach cuc (格局), the special configurations on a completed QiMen chart that carry condensed cat / hung meaning. It runs the rule engine (TASK-RULE-002) over the four-plate chart from TASK-QMDG-004 and emits the matches as `CachCuc` entries of the la so envelope (TASK-PLAT-002).
...
Each cach and each special state SHALL be a predicate expressed as data (a JSON condition evaluated by TASK-RULE-002), not imperative Rust; a palace MAY carry several at once. Every pattern SHALL be validated against the kinqimen oracle, and each emitted `CachCuc` SHALL carry its id, name, palace, polarity (cat / hung / trung), an optional score, and citations, per the envelope contract.
...
### Pattern shape (evaluated by TASK-RULE-002)

Patterns live in `patterns/qimen_cach_cuc.json` as data. A cat cach as a condition (illustrative shape; the authoritative operator set is TASK-RULE-002):
...
`CachCucHit` maps directly onto the envelope `CachCuc` type (TASK-PLAT-002 §3). Under `yin_yang_pan = am` the cach-cuc layer is intentionally light (s6.2); under `duong` it is the primary read-out.
...
4. Load and evaluate the patterns via TASK-RULE-002 against the completed chart plus CORE derived states (tuan khong, mo, hinh, xung).
5. Wire the pattern-file cross-check, the constructed-chart tests, and the kinqimen oracle test.

## §7 - Dependencies

Depends on TASK-QMDG-004 (the completed star / door / god rings and the sky/earth stems) and TASK-RULE-002 (the condition DSL and evaluator). Blocks TASK-QMDG-006 (assembly emits the detected cach cuc into the envelope). Consumes CORE derived states (tuan khong, tomb, hinh) via the `LichPhap` context from TASK-CORE-005 / TASK-CORE-004.

## §8 - Example payloads

The `cach_cuc` array as it appears in the envelope (TASK-PLAT-002 shape):
...
- Scoring: are the `score` values a fixed per-cach constant, or context-weighted? Default: a fixed per-cach constant seeded in the pattern file; revisit if the interpretation eval loop (TASK-RAG-006) shows ranking needs context weighting.
...
Cach cuc is where the QiMen engine meets the shared rule engine, so keep every pattern as data in `patterns/qimen_cach_cuc.json` and let TASK-RULE-002 evaluate it - no cach in imperative Rust. The engine's job ends at the matched fact; the cited explanation is the AI layer's job (s7.3), and because QiMen has many schools the interpretation must name which school it reads under. The oracle gate is set equality against kinqimen, so a spurious extra hit fails just as hard as a missing one.
