...
related_frs: [TASK-QMDG-002, TASK-QMDG-003, TASK-QMDG-006, TASK-CORE-001, TASK-CORE-005]
depends_on: [TASK-CORE-005]
blocks: [TASK-QMDG-002, TASK-QMDG-006]
new_paths:
...
The module SHALL stamp `dingju_method` into `co_truong_phai` of the la so envelope (TASK-PLAT-002) and SHALL emit its result as the `dinh_cuc` component of the QiMen `ban`. Two charts with the same instant but a different `dingju_method` MAY differ at boundary days; the stamp makes each reproducible.
...
s9.1 also names a fourth resolution, am duong thuan nghich (陰陽順逆), which places the cuc purely by don direction without a leap term; it belongs to the number-theory lineage and is reached through `yin_yang_pan` (TASK-QMDG-004), not through this flag. This flag stays a closed three-value enum (strategy RISK-7).
...
// ctx carries the tiet khi + tam nguyen + tu tru from TASK-CORE-005.
pub fn dinh_cuc(ctx: &LichPhap, flags: &QiMenFlags) -> Result<DinhCuc, QiMenError>;
```

`QiMenFlags` (defined in `flags.rs`, extended by later tasks) carries `dingju_method` here; TASK-QMDG-003/004 add `pan_method`, `yin_yang_pan`, `zhong_gong_ky`. Only the flags this step consumed are stamped by this step.
...
1. Create the `cyberos-qimen` crate; add `flags.rs` with `QiMenFlags` (only `dingju_method` populated at this task, other fields added by later tasks with their defaults).
...
Depends on TASK-CORE-005 (the calendar module API that supplies the in-force tiet khi, its tam nguyen marker, and the four pillars). Blocks TASK-QMDG-002 (bo dia ban starts from `so_cuc` and steps by `duong_don`) and TASK-QMDG-006 (assembly + oracle gate). The tam-nguyen boundary with CORE is the open question flagged in TASK-CORE-001 s9: CORE returns the raw term and instant, and this task computes the QiMen tam nguyen from the phu dau, since it is QiMen-specific.
...
- Should the fourth s9.1 resolution (am duong thuan nghich) be a fourth `dingju_method` value or stay coupled to `yin_yang_pan`? Decision: keep it under `yin_yang_pan` (TASK-QMDG-004), because it is the number-theory lineage's cuc rule, not a drift-absorption method. Revisit if kinqimen models it independently.
...
| Center cuc confusion | so_cuc 5 mishandled downstream | this task returns raw so_cuc; center-palace lodging is TASK-QMDG-003 |

## §11 - Notes

This is the highest-variance task in the QiMen engine (strategy RISK-7): the same instant can yield a different cuc under a different `dingju_method`, so the oracle gate MUST run per method, not just for the default. The crate name `cyberos-qimen` is shared with TASK-QMDG-002..006 - they extend this crate. Keep `flags.rs` the single home of `QiMenFlags` so the flag set grows in one place and the envelope stamp stays complete.
