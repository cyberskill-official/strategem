...
related_frs: [TASK-RULE-002, TASK-RULE-003, TASK-KB-002, TASK-KB-004, TASK-PLAT-003, TASK-QMDG-005]
depends_on: [TASK-PLAT-003]
blocks: [TASK-RULE-002, TASK-KB-002]
new_paths:
...
This task defines the pattern-as-data model that the whole rule engine stands on: a cach cuc (格局) or khoa the (課體) is a row of data, never a branch of code. The module SHALL express every detectable pattern as a `Pattern` record whose match logic lives in a JSON condition tree (the `conditions` field, whose grammar TASK-RULE-002 owns) and whose meaning lives in cited text. No pattern SHALL be hardcoded in Rust; adding, editing, or retiring a pattern SHALL be a data change, expert-reviewed and versioned, that ships without recompiling the engine (strategy 4.4, Grok-31).

This task owns three artifacts and one boundary. It owns (a) the canonical shape of the `knowledge_patterns` table - the column set, types, and enums; (b) the JSON Schema for a single pattern row at `docs/contracts/knowledge-pattern.schema.json`; and (c) a Rust `validate_pattern` function plus the seed-file format. It does not own the migration: TASK-PLAT-003 creates the physical table and RLS from this shape, and TASK-KB-002 fills the seed with the 150-200 real patterns. The boundary: this task shape-checks `conditions` only structurally (is it a JSON object with a recognized top-level form); the deep DSL grammar and the evaluator belong to TASK-RULE-002, which depends on this task.
...
| conditions | jsonb | condition DSL tree (TASK-RULE-002 grammar); shape-checked here only |
...
| citations | jsonb | array of citation ids into the classical corpus (TASK-KB-003) |
| version | int | monotonic per `id`, >= 1; bumped on any semantic edit |
| confidence | real | 0..1 expert prior; the base match score emitted by TASK-RULE-002 |
| status | text | enum: `active` \| `draft` \| `deprecated` (TASK-RULE-003 loads `active`) |
| reviewed_by | text | expert sign-off handle (TASK-KB-004); nullable while `draft` |
| created_at | timestamptz | audit |
| updated_at | timestamptz | audit |

Uniqueness is on `id` (the current row). Version history is retained per the TASK-KB-004 curation workflow; at MVP a single-current-row table plus an append-only `pattern_audit` row (TASK-PLAT-003) is sufficient, and `(id, version)` is the reproducibility key.
...
pub struct Pattern {
    ...
    pub conditions: serde_json::Value,   // parsed + deep-validated by TASK-RULE-002
...
`conditions` is `serde_json::Value` at this layer on purpose: the shared crate must not depend on the DSL evaluator (TASK-RULE-002 depends on this task, not the reverse). This task validates the pattern envelope and does a shallow structural check of `conditions`; TASK-RULE-002 parses and deep-validates it.
...
4. `conditions` is a non-empty JSON object whose top-level form is recognized: either a connective (`type` in {`and`, `or`, `not`}) or a leaf (has a `field` key). Depth and operator validity are deferred to TASK-RULE-002.
...
1. Create the `cyberos-rule` crate (this task owns its birth; TASK-RULE-002/003 add modules to it, mirroring how `cyberos-lichphap` is born in TASK-CORE-001).
...
5. `version.rs`: helpers for the `(id, version)` stamp and a `bump()` guard used by TASK-KB-004.
6. `seed.rs`: `load_seed(dir)`; commit a small `seed/qimen.json` (a handful of patterns) as the format exemplar and test fixture. TASK-KB-002 grows it.

## §7 - Dependencies

Depends on TASK-PLAT-003, which creates the physical `knowledge_patterns` table, indexes, and RLS from the shape this task defines. Blocks TASK-RULE-002 (the DSL parses the `conditions` field this task carries) and TASK-KB-002 (pattern seeding fills the seed-file format and the table this task shapes). TASK-QMDG-005 consumes detected patterns downstream through TASK-RULE-003.
...
- Full version history table vs single-current-row + audit. Default for MVP: one current row per `id` plus a `pattern_audit` append row in TASK-PLAT-003; a dedicated `pattern_versions` table is a TASK-KB-004 concern if experts need diffs across many revisions.
- Should `citations` be typed objects rather than string ids at this layer? Default: string ids that resolve against the TASK-KB-003 corpus, so this crate stays free of corpus types; the citation card is assembled in TASK-RAG-003. Revisit if the rule engine needs locator ranges.
- `confidence` provenance: expert-set constant now; whether TASK-RAG-006 evals ever feed a learned prior back into it is deferred, and would itself be a versioned edit.
...
| Silent pattern edit | `conditions`/meaning changed without version bump | TASK-KB-004 guard + reproducibility test (old stamp no longer matches text) fails |
| Uncited active pattern | `status = active`, `citations = []` | `validate_pattern` fails; DB check constraint (TASK-PLAT-003) rejects insert |
...
| Bad enum from DB | a legacy `system` string not in the enum | typed deserialize error, surfaced by TASK-RULE-003 loader, not silently skipped |

## §11 - Notes

This is the smallest P0 RULE task but it sets a hard rule for the entire product: patterns are data. Everything else in the module (TASK-RULE-002 evaluator, TASK-RULE-003 loader) and the KB seeding (TASK-KB-002) and curation (TASK-KB-004) assume this shape. The crate name `cyberos-rule` is shared with TASK-RULE-002/003; they extend this crate rather than create new ones, so the rule engine is one deterministic, cargo-testable unit shared by all three casting systems. Keep the schema under `docs/contracts/` beside `laso-envelope.schema.json` so the contract is reviewable independently of the build.
