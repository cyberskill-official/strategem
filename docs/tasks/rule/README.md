...
|---|---|---|--:|---|---|---|
| RULE-001 | MUST | P0 | 8 | PLAT-003 | [TASK-RULE-001](TASK-RULE-001-pattern-schema/spec.md) | Pattern-as-data schema + knowledge_patterns table + versioning |
| RULE-002 | MUST | P0 | 12 | RULE-001 | [TASK-RULE-002](TASK-RULE-002-condition-dsl/spec.md) | Condition DSL (AND/OR/NOT, field operators) + evaluator + scoring |
| RULE-003 | MUST | P0 | 6 | RULE-002 | [TASK-RULE-003](TASK-RULE-003-pattern-loader/spec.md) | Pattern loader + per-system filter + match API |
| RULE-004 | COULD | P2 | 8 | RULE-002 | [TASK-RULE-004](TASK-RULE-004-cross-system/spec.md) | Cross-system pattern support (nested, multi-system) |

Three P0 tasks are authored. RULE-004 (cross-system patterns) is P2 / COULD, authored; it extends the TASK-RULE-002 DSL and the TASK-RULE-003 matcher for multi-system, nested patterns and the reserved per-node `weight`.
...
- Depends on PLAT-003, which creates the physical `knowledge_patterns` table, indexes, and RLS from the schema shape TASK-RULE-001 owns. Depends on PLAT-002 for the la so envelope types (`LaSo`, `CachCuc`) that TASK-RULE-002 emits.
...
- Fed by KB-002 (pattern seeding, 150-200 patterns into the TASK-RULE-001 seed/table) and KB-004 (curation, expert review, versioning). Citations on each pattern resolve into the KB-003 classical corpus.
