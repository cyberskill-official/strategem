...
related_frs: [TASK-API-001, TASK-PLAT-003, TASK-AUTH-001, TASK-AUTH-002, TASK-RAG-004, TASK-REPORT-001, TASK-LEGAL-002]
depends_on: [TASK-API-001, TASK-PLAT-003]
blocks: []
new_paths:
...
This task implements step 9 of the query flow: persisting the query, the cast chart(s), the detected patterns, and the generated report, and writing an audit row for every sensitive action. It extends the `tamthuc_api` package. It owns the write path and the repository layer over the TASK-PLAT-003 tables; it does NOT own the table migrations or RLS (TASK-PLAT-003) nor the human-review queue (TASK-RAG-004), though its audit rows feed both.

Persistence SHALL write into the tables TASK-PLAT-003 provisions (Grok-46 schema): `queries(id, user_id, query_type, input_data, systems_used, created_at)`, `charts(id, query_id, system, chart_data, patterns_detected, created_at)`, and `reports(id, query_id, report_data, pdf_path, generated_at)`. The `charts.chart_data` column SHALL store the engine's la so JSON envelope (TASK-PLAT-002) exactly as cast, and `charts.patterns_detected` SHALL store the TASK-RULE-003 output; this task SHALL NOT re-derive or normalize any chart field, preserving the read-only invariant end to end (strategy 4.3, 4.4). A single query with multiple systems SHALL produce one `queries` row and one `charts` row per system, all sharing the `query_id`.

Auditing SHALL write an `audit_logs(id, user_id, action, details, created_at)` row for every sensitive action: any read or write of `birth_data`, a chart cast on behalf of a user, a report generation or download, a tier change, an authentication event of note (login, refresh, social link), a DSAR export or erasure (TASK-AUTH-004), and any abuse action (TASK-API-003). Audit rows SHALL be append-only and SHALL NOT contain plaintext `birth_data` or full question text - they record that an action occurred, by whom, and enough context to investigate, not the sensitive payload itself. Writes SHALL be transactional with the request outcome: a failed persistence SHALL NOT silently drop a served response, and a served response SHALL have its audit row (the audit write is part of completing the request, not a fire-and-forget afterthought).
...
### Tables written (TASK-PLAT-003 owns the DDL; this task writes them)

| Table | Columns (Grok-46) | This task writes |
|---|---|---|
...
`charts.chart_data` is the TASK-PLAT-002 envelope verbatim; `input_data` holds the `QueryRequest` minus any secret; `systems_used` mirrors the requested systems.
...
4. Persistence and the served response are transactional: an injected DB failure on the write does not return a 200 with lost data; the request fails with the TASK-API-001 error envelope and no partial rows remain.
5. `ReportRepo.get` honors row-level security (TASK-PLAT-003): a user cannot read another user's report; a cross-user read returns 404, not another user's data.
6. Audit rows are append-only: an attempt to update or delete an audit row is rejected (enforced by TASK-PLAT-003 grants); a test asserts the write-only contract at the repository layer.
...
- `tests/test_audit.py`: one row per sensitive action with the right `AuditAction`; the no-plaintext-sensitive-data assertion on `details`; the append-only contract; audit emitted for abuse actions (TASK-API-003) and auth events (TASK-AUTH-001).
...
1. `repositories.py`: `QueryRepo` / `ChartRepo` / `ReportRepo` protocols with a Postgres implementation over the TASK-PLAT-003 tables and an in-memory fake for tests.
...
4. Wire step 9 of the TASK-API-001 orchestrator to `persist_query_result` + `audit`; wire abuse and auth audit calls from TASK-API-003 / TASK-AUTH-001.

## §7 - Dependencies

Depends on TASK-API-001 (the orchestrator whose step 9 this completes, and the error envelope a failed write returns) and TASK-PLAT-003 (the physical `queries` / `charts` / `reports` / `audit_logs` tables, their indexes, and the RLS this write path relies on). Stores the TASK-PLAT-002 la so envelope verbatim and the TASK-RULE-003 patterns. Its audit rows feed TASK-RAG-004 (human-review context), TASK-AUTH-004 (DSAR export reads the user's persisted history; erasure removes it), and TASK-LEGAL-002 (retention and disclosure operate on exactly these tables). Auth and abuse events audited here originate in TASK-AUTH-001 and TASK-API-003.
...
- Whether `charts.patterns_detected` duplicates the envelope's `cach_cuc` or is stored separately. Default: store the TASK-RULE-003 match output in `patterns_detected` (it carries match scores and the `(id, version)` stamps), while `chart_data.cach_cuc` holds the engine's own list; a test asserts they reconcile. Revisit if the duplication proves noisy.
- Retention windows per table. Default: this task persists; TASK-LEGAL-002 sets the retention and the erasure contract over these tables, and TASK-AUTH-004 executes DSAR against them. Keep the windows in the LEGAL config, not hardcoded here.
- Audit sink: same Postgres vs a separate append-only store. Default: the `audit_logs` table in the shared Postgres with delete/update revoked (TASK-PLAT-003 grants); a dedicated immutable sink is a later hardening step (TASK-PLAT-007) that keeps the same `audit` interface.
...
| Mutable audit trail | an audit row updated or deleted | append-only; DB grants revoke update/delete (TASK-PLAT-003); repository exposes no mutator |
...
This task closes the query flow and makes the platform accountable: it stores what the engine produced (never a re-derivation) and records who touched sensitive data (never by copying it). The two disciplines to hold are verbatim chart storage and append-only, payload-free audit. It extends the same `tamthuc_api` app as TASK-API-001/002/003, writing into the TASK-PLAT-003 schema, so the gateway remains one installable, mypy-clean unit. Retention, export, and erasure over these tables belong to TASK-LEGAL-002 and TASK-AUTH-004; this task gives them a clean, audited store to operate on.
