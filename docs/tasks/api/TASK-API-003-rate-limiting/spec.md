...
related_frs: [TASK-API-001, TASK-AUTH-002, TASK-PLAT-006, TASK-PLAT-007, TASK-PLAT-008]
depends_on: [TASK-API-001, TASK-AUTH-002]
blocks: []
new_paths:
...
This task adds rate limiting and abuse detection to the gateway: it enforces the per-tier request quotas that TASK-AUTH-002 configures, and it detects and blunts abusive traffic patterns. It extends the `tamthuc_api` package. It owns the counting and the enforcement; it does NOT own the quota numbers (those are TASK-AUTH-002's single-source config) nor the tier resolution (TASK-AUTH-001/002).

The limiter SHALL enforce the quotas from `docs/contracts/rbac-tiers.json` via TASK-AUTH-002's `quota_for(principal)`: Free 100 requests/day, Premium 5000 requests/day, Enterprise the account's custom number, Admin unmetered. Counting SHALL be per principal (user id or Enterprise API-key account) over a rolling or calendar-day window, backed by Redis (TASK-PLAT-006) so the count is shared across gateway instances. When a principal exceeds its quota the gateway SHALL return HTTP 429 with the TASK-API-001 error envelope (`code = RATE_LIMITED`), a `Retry-After` header, and `details` carrying the limit and the reset time. Limiting SHALL be applied as middleware on the metered routes so no metered endpoint can be reached without passing the check, and Admin/unmetered principals SHALL bypass counting.

Abuse detection SHALL run alongside quota enforcement and cover at least: request-velocity spikes (a burst well above a principal's normal rate), credential-stuffing signatures on the auth routes (many failed logins across accounts from one source), and repeated malformed or probing requests. On a detected abuse signal the gateway SHALL apply a graduated response - short-window throttle, temporary lockout, and an audited flag - rather than a silent drop, and SHALL record the event for TASK-API-004 audit and TASK-PLAT-005 alerting. Rate-limit and abuse state SHALL fail safe: if Redis is unavailable the limiter SHALL degrade to a conservative local limit rather than failing open to unlimited traffic.

## §2 - Why this design (rationale for humans)

The calculate endpoints call an LLM, so an unmetered request is a direct cost and a direct abuse surface (Grok-12). Rate limiting is therefore not just a plan-enforcement nicety; it is the first line of cost control and the first line of denial-of-service defence (Grok-21). Reading the quota from TASK-AUTH-002's single config rather than hardcoding numbers here is what keeps the enforced limit equal to the advertised one - the same discipline stated from the enforcement side.
...
class RedisRateLimiter:     # default (TASK-PLAT-006 Redis); shared counter across instances
...
#  1. resolve principal (TASK-AUTH-001/002); Admin/unmetered -> pass through, no count
...
### 429 envelope (via TASK-API-001 error contract)
...
5. A burst well above a principal's baseline raises `velocity_spike` and a graduated `throttle`; many failed logins across accounts from one source raise `credential_stuffing` and a `lockout`; each produces an audited event for TASK-API-004 and an alert signal for TASK-PLAT-005.
...
4. Wire the middleware onto the metered routes in `app.py`; emit audit events to the TASK-API-004 seam and alert signals to TASK-PLAT-005.

## §7 - Dependencies

Depends on TASK-API-001 (the gateway, the metered routes, and the error envelope this returns) and TASK-AUTH-002 (the tier resolution and `quota_for` single-source config). Uses TASK-PLAT-006 Redis for the shared counters; audited events flow to TASK-API-004 and alerts to TASK-PLAT-005; the fail-safe posture aligns with TASK-PLAT-008 resilience. This task blocks nothing directly but is a gating control on the TASK-API-001 endpoints.

## §8 - Example payloads

```json
// audited abuse event (to TASK-API-004)
...
| Silent abuse drop | abusive traffic dropped with no record | graduated throttle/lockout/flag, each audited (TASK-API-004) and alerted (TASK-PLAT-005) |
...
This task is the cost and abuse control on an LLM-backed API, so its two rules are: read the quota from one source (never hardcode), and fail safe (never fail open). Keep the split with TASK-AUTH-002 clean - AUTH decides the number, API-003 does the counting and returns the 429. It extends the same `tamthuc_api` app as TASK-API-001/002/004; the limiter and abuse controls are middleware on the existing routes, so the gateway stays one installable, mypy-clean unit.
