...
related_frs: [TASK-API-001, TASK-PLAT-002, TASK-PLAT-004, TASK-WEB-006]
...
This task defines how the API evolves without breaking the clients that depend on it: URL versioning as the primary scheme, a header-versioning option, a deprecation policy with a support window, a changelog of breaking changes, and the invariant that separates what may change (interpretation) from what may not (calculation output). It extends the `tamthuc_api` package and wraps the TASK-API-001 routes. It owns the versioning scheme and the deprecation mechanics; it does NOT change the endpoint behavior TASK-API-001 defines - it governs how that behavior may change over time.
...
|---|---|---|
| calculation output (la so envelope, chart) | stable, reproducible | TASK-PLAT-002 `envelope_version` bump + migration note only |
...
1. All TASK-API-001 routes are served under a URL version prefix (`/api/v1/...`); a request to an unknown version returns the TASK-API-001 error envelope with the correct status, not a silent fallback.
...
5. The calculation output is stable across API versions - the la so envelope changes only via a TASK-PLAT-002 `envelope_version` bump, never as an unversioned API change - while the interpretation is allowed to vary; a test pins the chart and does not pin the interpretation prose.
...
- A stability test: the calculation output (envelope) is byte-stable across two API versions for a golden input, while the interpretation field is explicitly not asserted byte-stable; a change to the envelope shape without a TASK-PLAT-002 version bump fails.
...
- Gates: `ruff check`, `ruff format --check`, `mypy tamthuc_api`, `pytest packages/tamthuc_api`; the OpenAPI (TASK-API-001 `openapi-v1.md`) is emitted per version.

## §6 - Implementation skeleton

1. `versioning/router.py`: mount the TASK-API-001 routes under `/api/v{n}`; resolve the effective version (URL primary, header option, URL wins); reject unknown versions in the error envelope.
...
5. Confirm the calculation-stability invariant against TASK-PLAT-002 (the envelope changes only via its own version bump).

## §7 - Dependencies

Depends on TASK-API-001 (the routes, the error envelope, and the OpenAPI contract this task versions and wraps; unknown-version rejection uses the API-001 envelope). Enforces the calculation-stability half of the invariant against the TASK-PLAT-002 la so envelope contract (the chart changes only via `envelope_version`, never as an unversioned API change), while leaving the TASK-RAG-003 interpretation free to vary. Coordinates with TASK-PLAT-004 (the CI check for changelog + version bump on a breaking change runs in the pipeline) and TASK-WEB-006 (the frontend/i18n consumers pin to a version). Blocks nothing directly; it is a governance layer over the existing gateway.
...
| Unknown version guessed | a request to an unsupported version silently served | rejected in the TASK-API-001 error envelope, not a silent fallback |
| Calculation drift | the chart/envelope changes as an unversioned API change | forbidden; the calculation output changes only via a TASK-PLAT-002 `envelope_version` bump |
...
This task is governance, not new behavior: it lets the API-001 gateway evolve without breaking its clients. Hold the scheme (URL primary, header optional, URL wins), the window (a deprecated item lives >= 2-3 versions behind `Deprecation`/`Sunset` headers and a CHANGELOG entry), and above all the invariant that separates the two halves of the platform - calculation output is stable and reproducible, changing only through the TASK-PLAT-002 versioned envelope, while interpretation is free to improve. That split is the versioning face of the deterministic-engine / AI-layer boundary (strategy 4.3): clients pin to the chart, not to the prose. It extends the same `tamthuc_api` app as TASK-API-001/003/004, so the gateway stays one installable, mypy-clean unit.
