...
related_frs: [TASK-AUTH-002, TASK-AUTH-003, TASK-AUTH-004, TASK-API-001, TASK-API-004, TASK-PLAT-003, TASK-PLAT-007, TASK-LEGAL-002]
depends_on: [TASK-PLAT-001]
blocks: [TASK-AUTH-002, TASK-AUTH-003, TASK-AUTH-004, TASK-API-001, TASK-LEGAL-002]
new_paths:
...
| email_verified | boolean | default false; set by TASK-AUTH-003 |
| tier | text | enum Free/Premium/Enterprise/Admin (TASK-AUTH-002); default Free |
| created_at | timestamptz | |
| updated_at | timestamptz | |

This is the `users` table the master catalog assigns to TASK-PLAT-003; AUTH is its natural owner, so the DDL lives here and PLAT-003 runs it as part of the umbrella migration set (see section 7).
...
### Endpoints (`tamthuc_auth/routes.py`, mounted by TASK-API-001)
...
# verifies the access token, loads the user, exposes id + tier for downstream authorization (TASK-AUTH-002)
```
...
6. Auth failures return the TASK-API-001 error envelope with a generic message; login against a non-existent email and login with a wrong password are indistinguishable to the caller (no account enumeration).
...
1. Create the `tamthuc_auth` package (`pyproject.toml`, `uv`-managed per TASK-PLAT-001); this task owns its birth, TASK-AUTH-002/003/004 add modules.
...
Depends on TASK-PLAT-001 (the Python workspace). The `users` table DDL is owned here and applied through the PLAT migration runner into the shared Postgres; the master catalog lists `users` under TASK-PLAT-003, so PLAT-003 coordinates the umbrella migration set while AUTH owns the auth columns and the encryption contract - a reconciliation note, with the hard `depends_on` kept at PLAT-001 per the catalog. Blocks TASK-AUTH-002 (tiers and quotas key off this user and token), TASK-AUTH-003 (email verification / password reset extends this flow), TASK-AUTH-004 (DSAR reads and erases this profile), TASK-API-001 (`get_current_user` is the gateway's auth dependency), and TASK-LEGAL-002 (the PDPD/GDPR pack's retention and erasure contracts operate on this data). TLS in transit and secrets management are enforced by TASK-PLAT-007; the audit row for sensitive `birth_data` access is written by TASK-API-004.
...
- Master-key custody: cloud KMS vs an app-held master key in a secret manager. Default: envelope encryption with a wrapped per-record data key so either custody works; the KMS adapter is preferred in production, the secret-manager path is the MVP fallback. Decide with TASK-PLAT-007.
- Refresh-token storage: opaque token in a revocation table vs a stateless JWT with a `jti` denylist. Default: `jti`-based revocation with a short denylist window matched to the refresh TTL; revisit if multi-device session management (TASK-AUTH-003 scope) needs per-device records.
- Whether Apple's private-relay email counts as a verified email. Default: treat the provider-asserted email as verified for social accounts and skip the email-verification step for them; local email/password accounts still verify. Confirm in TASK-AUTH-003.
...
This task carries the product's highest data-sensitivity obligation, so treat the birth_data encryption and the account-enumeration-safe responses as non-negotiable, not polish. Keep the boundary clean: this task owns identity, tokens, and the at-rest crypto; TASK-AUTH-002 owns what a user is allowed to do and how much; TASK-AUTH-004 owns export and erasure; TASK-LEGAL-002 owns the consent, retention, and disclosure contracts around all of it. The package `tamthuc_auth` is shared with TASK-AUTH-002/003/004; they extend it, so auth is one installable, mypy-clean unit that the FastAPI gateway (TASK-API-001) mounts.
