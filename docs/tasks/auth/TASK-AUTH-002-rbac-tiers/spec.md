---
id: TASK-AUTH-002
title: "RBAC tiers - Free / Premium / Enterprise / Admin roles + per-tier capability and rate-limit quota config (Free 100/day, Premium 5000/day, Enterprise custom), the single source TASK-API-003 enforces, Enterprise API-key auth"
module: AUTH
priority: MUST
status: done
phase: P0
...
related_frs: [TASK-AUTH-001, TASK-API-001, TASK-API-003, TASK-PLAT-007]
depends_on: [TASK-AUTH-001]
blocks: [TASK-API-003, TASK-PLAT-007]
new_paths:
...
This task defines authorization: the four roles a user can hold, the capabilities each role grants, and the per-tier rate-limit quotas that TASK-API-003 enforces. It extends the `tamthuc_auth` package. It owns the tier and quota configuration as a single source of truth; it does NOT implement the counting or the 429 responses (that is TASK-API-003) nor issue tokens (TASK-AUTH-001).

The role set SHALL be the closed enum Free, Premium, Enterprise, Admin. Each role SHALL map to a capability set (which endpoints and features it may reach) and, for the request-serving tiers, to a rate-limit quota. The quotas SHALL be: Free 100 requests/day, Premium 5000 requests/day, Enterprise custom (a per-account override), with Admin unmetered for operational use. Tier and quota configuration SHALL live in one machine-readable artifact (`docs/contracts/rbac-tiers.json`) that both this package and TASK-API-003 read, so the limit a request is judged against and the limit the product advertises are the same number in one place.

Authorization SHALL be expressed as FastAPI dependencies (`require_tier`, `require_role`, `require_capability`) that read the `tier` claim from the TASK-AUTH-001 access token and reject an under-privileged request with a 403 in the TASK-API-001 error envelope. Enterprise clients SHALL additionally be able to authenticate machine-to-machine with an API key (distinct from a user JWT); an Enterprise API key SHALL resolve to an Enterprise-tier principal with its account's custom quota. A tier change SHALL take effect on the next token issuance or refresh, and SHALL be an auditable action (TASK-API-004).
...
### Tier quota config (`docs/contracts/rbac-tiers.json`, read by this package and TASK-API-003)
...
def quota_for(principal) -> int | Literal["unmetered"]: ...  # the number TASK-API-003 enforces
```
...
3. `quota_for` returns 100 for Free, 5000 for Premium, the account override for Enterprise, and `"unmetered"` for Admin; TASK-API-003 consumes exactly this.
...
6. A tier change is reflected on the next token issuance/refresh and produces an auditable action record consumed by TASK-API-004.
...
Depends on TASK-AUTH-001 (the user, the `tier` claim in the access token, and the token-issuance path a tier change flows through). Blocks TASK-API-003 (rate limiting reads `quota_for` and `rbac-tiers.json` to enforce the per-tier limit) and TASK-PLAT-007 (security hardening's authorization controls build on these roles). The authorization dependencies are consumed by TASK-API-001's protected routes; tier-change audit rows are written by TASK-API-004.

## §8 - Example payloads

```json
// a Free user calling /calculate/all
// require_capability(calculate_all) -> 403 in the TASK-API-001 envelope:
{ "error": { "code": "FORBIDDEN_TIER", "message": "This endpoint requires the Premium tier or higher.",
             "details": { "required": "Premium", "current": "Free" } } }
```

```json
// quota_for a principal, consumed by TASK-API-003
...
- Whether quotas are per-day only or also per-minute burst. Default: this task fixes the per-day quota (the documented plan number); TASK-API-003 may add a burst window on top, reading the same config. Keep the burst config in `rbac-tiers.json` if added, so it stays single-source.
...
This task is small but load-bearing twice over: it is the commercial plan ladder and the primary cost/abuse control on an LLM-backed endpoint. The one discipline that matters most is single-source config - the quota a request is judged against and the quota a pricing page shows must be the same JSON. Keep the split clean: AUTH-002 decides what a principal may do and how much; TASK-API-003 does the counting and returns the 429. The package `tamthuc_auth` is shared with TASK-AUTH-001/003/004; this task extends it with the authorization layer the FastAPI gateway leans on.
