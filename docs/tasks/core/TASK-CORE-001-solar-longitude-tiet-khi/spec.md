...
related_frs: [TASK-CORE-002, TASK-CORE-003, TASK-CORE-005, TASK-CORE-006]
depends_on: [TASK-PLAT-001]
blocks: [TASK-CORE-002, TASK-CORE-003, TASK-CORE-005, TASK-QMDG-001]
new_paths:
...
This task implements the astronomy layer (L1) of the shared calendar core: the apparent solar longitude function and the 24 solar terms (tiet khi) derived from it. It is the foundation of the entire platform - an error here propagates to all three engines at once (strategy RISK-1), so it carries the highest test density in the project.
...
The module SHALL distinguish the two interleaved kinds of term: the twelve jie (節, odd 15-degree marks) that govern month-pillar boundaries (consumed by TASK-CORE-003), and the twelve trung khi (中氣, even marks) that govern nguyet tuong changes in LiuRen (consumed by TASK-LN-001). Every returned term SHALL carry its kind, its exact instant in UTC, and the current tam nguyen (thuong/trung/ha) marker where applicable.

School and precision differences are flags, not hardcoded choices: `delta_t_model` (default `espenak_meeus`) and a `precision` selector (default `meeus_low`, with a `vsop87` slot reserved for TASK-CORE-006 follow-up).
...
1. Create the `cyberos-lichphap` crate (this task owns its birth; TASK-CORE-002/003/004 add modules to it).
...
Depends on TASK-PLAT-001 (cargo workspace). Blocks TASK-CORE-002 (true solar time reuses the equation-of-center terms), TASK-CORE-003 (month pillar needs jie boundaries), TASK-CORE-005 (module output), and TASK-QMDG-001 (dinh cuc keys off the current term + tam nguyen).
...
- Do we ever need VSOP87 in production, or only as an oracle? Default: Meeus low-order in production, VSOP87 only in the TASK-CORE-006 cross-check. Decide from the boundary-case count the oracle harness reports.
- Tam nguyen (thuong/trung/ha) assignment belongs partly here and partly in QMDG-001. Decision: this task returns the raw term + instant; the thuong/trung/ha nguyen for QiMen dinh cuc is computed in QMDG-001 from the phu dau, since it is QiMen-specific. Cross-check this boundary when QMDG-001 lands.
...
This is the highest-risk task in the project (RISK-1). Treat the oracle gate as non-negotiable: a term off by more than 60 seconds is a stop-ship. The crate name `cyberos-lichphap` is shared with TASK-CORE-002/003/004/005 - they extend this crate rather than create new ones, so the calendar core is one testable unit.
