...
related_frs: [TASK-CORE-003, TASK-CORE-005, TASK-CORE-007]
depends_on: [TASK-CORE-003]
blocks: [TASK-CORE-005]
new_paths:
...
All three outputs SHALL be derived from committed lookup tables, not recomputed heuristically, so that a reviewer can check each table against a classical source and the oracle harness (TASK-CORE-006) can diff them per school.
...
- Per-school cross-check: a manual fixture (hand-computed from a classical text) for tuan khong, vuong-suy, and both truong sinh schools, diffed by TASK-CORE-006 (this is the "per school" clause of the RISK-1 gate).
...
3. Reuse TASK-CORE-007 for `Can`/`Chi`/`NguHanh` and the sinh/khac relations used by vuong-suy.
4. Add the enumerated tests and the hand-computed per-school fixture.

## §7 - Dependencies

Depends on TASK-CORE-003 (the pillars supply the can-chi pairs and the season). Uses TASK-CORE-007 for phase/relation primitives (soft; joinable at integration). Blocks TASK-CORE-005 (the `phai_sinh` sub-object of the calendar output). The `truong_sinh_phai` flag it reads is defined in the canonical `LichFlags` owned by TASK-CORE-005.
...
- Do we always emit all three derived states, or only on request to save work? Decision: TASK-CORE-005 gates emission behind a `want_derived` request flag (they are not needed for pure timing scans); when emitted they are complete. Default on for interpretation paths.
...
| Flag unstamped | `truong_sinh_phai` missing from `co_lich_phap` | TASK-CORE-005 reproduction test diverges |

## §11 - Notes

Keep every table as committed `const` data with the classical Han in place - these are the kind of tables a domain reviewer reads directly, and normalizing 巳/子 or 囚/死 to ASCII would erase exactly the distinctions that matter. The per-school truong sinh diff is the clearest expression of the flag-and-stamp discipline in the calendar core. Same crate `cyberos-lichphap` - this task adds `derived.rs` and `data/truong_sinh.rs`.
