...
related_frs: [TASK-CORE-001, TASK-CORE-002, TASK-CORE-004, TASK-CORE-005, TASK-CORE-007]
depends_on: [TASK-CORE-001, TASK-CORE-002]
blocks: [TASK-CORE-004, TASK-CORE-005]
new_paths:
...
This task builds bon tru can chi (四柱干支, tu tru) - the year, month, day, and hour pillars, each a thien can paired with a dia chi - the calendar bedrock every engine reads. Each pillar follows a different rule with its own boundary and its own lookup table, and this is the most error-prone part of the calendar core (Claude-05 s4), so each is specified separately and pinned to a test vector.

The module SHALL compute the year pillar with its boundary at Lap Xuan (sun at 315 degrees), NOT at lunar new year and NOT at the Gregorian 1 January. The month pillar chi SHALL be fixed by the twelve jie (from TASK-CORE-001), and its can SHALL be derived from the year can by Ngu Ho Don (五虎遁). The day pillar SHALL be a continuous sexagenary count from the Julian day number, unbroken since antiquity. The hour pillar chi SHALL be assigned from the true solar hour (TASK-CORE-002), and its can derived from the day can by Ngu Thu Don (五鼠遁).
...
Boundary at Lap Xuan (sun 315 deg), from TASK-CORE-001. For a civil year `y`:
...
```rust
pub struct TruCanChi { pub can: Can, pub chi: Chi }          // Can/Chi from TASK-CORE-007
...
4. Ngu Thu Don is correct for all five day-can classes; the hour chi is taken from `gio_that` (TASK-CORE-002), not clock time.
...
6. All can/chi are returned as `TASK-CORE-007` `Can`/`Chi` values, serialized to their Han glyph in the envelope (e.g. 甲子).

## §5 - Verification

- `tests/pillars_oracle.rs` cross-checks bon tru against tyme4py over a long day span, including boundary cases: dates around Lap Xuan (each year 1950-2050), dates around midnight and around 23:00 for both zi flags, and the two day-pillar anchors. This feeds the TASK-CORE-006 gate.
...
1. `don_tables.rs`: the Ngu Ho Don and Ngu Thu Don tables as `const` arrays keyed by year-can / day-can class, plus the sixty-cycle helpers (reuse TASK-CORE-007 for can/chi indexing).
2. `pillars.rs`: `year_pillar` (with the Lap Xuan lookback via TASK-CORE-001), `month_pillar` (jie index -> chi, Ngu Ho Don -> can), `day_pillar` ((JDN-10) mod 60), `hour_pillar` (gio branch from `gio_that`, Ngu Thu Don -> can).
...
Depends on TASK-CORE-001 (Lap Xuan / jie boundaries for year and month) and TASK-CORE-002 (true solar hour for the hour-pillar branch). NOTE: the master catalog lists CORE-003 depends_on CORE-001 only; this task widens it to include CORE-002 because the hour-pillar chi is assigned from `gio_that`, which does not exist until CORE-002 is done - propose updating the catalog row to match. Uses TASK-CORE-007 for `Can`/`Chi` primitives (soft: the two can be built in parallel and joined at integration). Blocks TASK-CORE-004 (derived states key off the pillars) and TASK-CORE-005 (the `tu_tru` sub-object).
...
- Should `da_zi` also affect the year/month pillars in the rare 23:00-24:00 window at a year boundary? Decision: no - da_zi touches only the day pillar's stem-source and the hour stem; year and month still follow the solar longitude of the actual instant. Confirm against tyme4py boundary cases in TASK-CORE-006.
...
| Hour chi from clock, not sun | TASK-CORE-002 not applied first | boundary probe assigns wrong gio; ordering test fails |
| Zi flag ignored | 23:00-24:00 handled one fixed way | tao_zi vs da_zi probes identical -> flag not honoured; fails |
| Flags unstamped | zi flags missing from `co_lich_phap` | TASK-CORE-005 reproduction test diverges |

## §11 - Notes

The two don tables and the two ca quyet verses are the classical source of truth - keep them verbatim in `don_tables.rs` comments so a reviewer can check them against a text. The zi-hour logic is the single most contested boundary in the calendar core; treat its tests as non-negotiable, on par with the tiet khi oracle. Same crate `cyberos-lichphap` - this task adds `pillars.rs` and `don_tables.rs`.
