...
related_frs: [TASK-CORE-001, TASK-CORE-003, TASK-CORE-005, TASK-CORE-006]
depends_on: [TASK-CORE-001]
blocks: [TASK-CORE-003, TASK-CORE-005]
new_paths:
...
This task implements chan thai duong thoi (真太陽時), the true (apparent) solar time at the place of observation, on top of the astronomy layer built in TASK-CORE-001. Clock time is mean zone time; the Eastern hour boundaries (gio Ty, gio Suu, ...) are defined by the real position of the sun, not by the clock. This module converts one from the other so the hour pillar (TASK-CORE-003) keys off the correct branch.

The module SHALL compute true solar time as `clock_time + longitude_correction + equation_of_time`, where every term is expressed in the same signed convention fixed in §3. It SHALL compute the equation of time (均時差) by the Jean Meeus method (Astronomical Algorithms ch. 28), reusing the apparent-longitude and obliquity routines from TASK-CORE-001 so the two layers never diverge on the sun's position. It SHALL compute the longitude correction as four minutes per degree of offset from the timezone standard meridian; for Vietnam (UTC+07) the standard meridian is 105 degrees East.
...
### Equation of time (Meeus ch. 28, reusing TASK-CORE-001)
...
The classical figures are usually quoted as "clock ahead of sundial" (`mean - apparent`); this task's `E` is the negative of that. Both columns are locked as oracle targets.
...
- Cross-check the EoT curve against the `equation_of_time` produced by an independent library (skyfield or the sxwnl mean-time delta) at monthly samples over one decade, within a few seconds (feeds TASK-CORE-006).
- Gates: `cargo fmt --check`, `cargo clippy -p cyberos-lichphap -- -D warnings`, `cargo test -p cyberos-lichphap`.

## §6 - Implementation skeleton

1. `eot.rs`: `equation_of_time_minutes(jde)`, computing `alpha` from TASK-CORE-001's `kinh_do_mat_troi` (lambda) plus obliquity, then `4*(L0 - 0.0057183 - alpha)`.
...
3. Expose `hieu_chinh_phut = longitude + E` so TASK-CORE-005 can stamp a single number in the envelope.
4. Add the golden-triple and extrema fixtures; wire the property test.
5. Leave a `precision` hook so TASK-CORE-006 can swap in a higher-order EoT if the boundary-case count demands it.

## §7 - Dependencies

Depends on TASK-CORE-001 (reuses apparent longitude, obliquity, and the delta-T / JDE plumbing). Blocks TASK-CORE-003 (the hour pillar assigns its branch from `gio_that`, so it cannot be correct until true solar time is) and TASK-CORE-005 (the `chan_thai_duong` sub-object of the calendar output). This is why the catalog's CORE-003 dependency is widened to include CORE-002 (see TASK-CORE-003 §7).
...
- Is the low-order EoT enough at the hour boundary, or do the rare within-a-minute-of-boundary cases need the higher-order sun? Deferred to TASK-CORE-006: the oracle harness counts boundary cases and decides whether a `precision` bump is ever needed.
...
| Correction applied after hour-branching | ordering bug in TASK-CORE-003 wiring | boundary probe assigns wrong gio; caught by TASK-CORE-003 test |
| Flag not stamped | `use_true_solar_time` omitted from `co_lich_phap` | reproduction test in TASK-CORE-005 diverges |

## §11 - Notes

Keep the EoT anchored to TASK-CORE-001's sun rather than a standalone day-of-year series: one source of truth for the sun's position is worth more than a few lines saved. The single stamped number `hieu_chinh_phut` (longitude + E) is what a reader needs to see how far the chart's hour was shifted; the two components are kept alongside for audit. Same crate `cyberos-lichphap` as TASK-CORE-001 - this task adds `truesolar.rs` and `eot.rs`, it does not create a new crate.
