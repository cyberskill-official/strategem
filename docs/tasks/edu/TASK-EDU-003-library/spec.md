---
id: TASK-EDU-003
title: "Bilingual classical library - search and cite over the TASK-KB-003 three-layer store; the learner reads original Han beside bach thoai and translation, with citations, to reach primary sources; a citation id resolves to the exact passage"
module: EDU
priority: SHOULD
status: done
phase: P3
...
related_frs: [TASK-KB-003, TASK-EDU-001, TASK-EDU-002, TASK-RULE-001, TASK-RAG-003, TASK-WEB-001, TASK-WEB-006]
depends_on: [TASK-KB-003]
blocks: []
new_paths:
...
This task builds the bilingual classical library: the learning flow's search-and-read surface over the TASK-KB-003 three-layer classical-text store, so a learner reads the original Han (chu Han) beside the bach thoai gloss and the translation, always with a citation, and reaches the primary source rather than a second-hand summary (Claude-07 s3.3, s4.3).

The module SHALL present each retrieved unit as a `Passage` carrying all three layers - `han`, `bach_thoai`, `dich` - never dropping the original Han (strategy 7, the cultural-respect rule). It SHALL expose search over the store (`searchLibrary`) and single-passage retrieval by id (`getPassage`), both backed by a library endpoint over the TASK-KB-003 store; this task owns the client, the view, and the request / result contract, not the store or the retrieval engine (those are TASK-KB-003 and the RAG retriever). It SHALL resolve a citation id (`resolveCitation`): given the citation string a pattern (TASK-RULE-001) or an AI interpretation (TASK-RAG-003) carries, it SHALL deep-link to the exact passage, so a citation shown anywhere in the product is a live link into the source text.

Every rendered passage SHALL show its citation (work, and locator where present) and SHALL keep the Han visible at all breakpoints (the dau chong / diacritic-clipping rule from the design system applies to the mixed Han + Vietnamese text). The surface SHALL render inside the TASK-WEB-001 shell using Design System v1.3.0 components with no new tokens, and SHALL be i18n-ready (TASK-WEB-006) for its own chrome while the three text layers are content, not translatable labels.
...
export interface Citation {
  id: string;              // e.g. "yba_thien_can_khac_ung_12"; resolves into the TASK-KB-003 corpus
...
export interface Passage {
  id: string;              // chunk id in the TASK-KB-003 store
  work: string;
  work_han?: string;
  han: string;             // original Han (always present)
  bach_thoai: string;      // vernacular gloss
  dich: string;            // translation (VN; EN mirror via TASK-WEB-006 later)
  citation: Citation;
...
```ts
// Search the three-layer store (backed by TASK-KB-003 through a library endpoint).
...
The three functions call the library endpoint; this task does not implement retrieval (TASK-KB-003 owns the store; the endpoint may reuse the TASK-RAG-002 retriever's index). `resolveCitation` is `getPassage(c.id)` with a graceful fallback to a `work` + `locator` lookup when an id is not directly addressable, so a citation never dead-ends.
...
6. The library chrome is i18n-ready (TASK-WEB-006) while the three text layers are treated as content, not passed through label translation.

## §5 - Verification

- Vitest (`library.test.ts`): a fixture corpus of a few passages (Han + bach thoai + dich + citation) mirroring the TASK-KB-003 shape. Search-and-filter cases; `getPassage` hit and typed miss; `resolveCitation` for a real pattern citation id (shared with the TASK-RULE-001 seed) and the `work` + `locator` fallback.
- Contract test: the `Passage` shape matches the TASK-KB-003 store's export (field parity), so a store change surfaces here.
...
2. `api.ts`: `searchLibrary`, `getPassage`, `resolveCitation` over the library endpoint (TASK-KB-003-backed).
...
Depends on TASK-KB-003 (the three-layer classical-text store - Han / bach thoai / dich - and its chunking); this task is the search-and-cite surface over it, not the store. Consumes citation ids emitted by TASK-RULE-001 patterns and TASK-RAG-003 interpretations (it is where those citations resolve to readable passages). Renders in the TASK-WEB-001 shell and is i18n-ready via TASK-WEB-006. Related to TASK-EDU-001 (the curriculum links out to library passages) and TASK-EDU-002 (a wrong practice step can link to the relevant source passage).
...
```ts
// a pattern's citation id (TASK-RULE-001) becomes a readable passage
...
- Search backend: does the library reuse the TASK-RAG-002 vector index or a lighter lexical index over the TASK-KB-003 store? Default: reuse the TASK-RAG-002 retriever's index through the library endpoint so Han, bach thoai, and translation are all searchable; a dedicated index is a later optimization if learner search differs from interpretation retrieval.
- Layer for EN users: the `dich` layer is Vietnamese at MVP; the EN translation is deferred to TASK-WEB-006. Default: keep `dich` as authored and add an `en` layer through the i18n content pipeline, never machine-translating the classical text inline.
...
| Store drift | TASK-KB-003 changes the passage shape | `Passage` field-parity contract test fails |
