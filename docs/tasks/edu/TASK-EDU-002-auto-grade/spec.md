...
related_frs: [TASK-EDU-001, TASK-EDU-003, TASK-QMDG-006, TASK-LN-006, TASK-PLAT-002, TASK-API-001, TASK-CHART-001, TASK-CHART-002, TASK-WEB-001]
depends_on: [TASK-QMDG-006, TASK-LN-006]
blocks: []
new_paths:
...
```ts
import type { LaSo } from "@/lib/laso";   // TASK-PLAT-002 envelope
...
export interface CastStep {
  ...
  criterionId?: string;    // the TASK-EDU-001 criterion this step contributes to
...
// (TASK-API-001 -> TASK-QMDG-006 / TASK-LN-006; the engine chart is the ground truth.)
...
1. The QiMen and LiuRen ladders exist as ordered `CastStep[]`, each step addressing real `ban` (or root `cach_cuc`) paths of the TASK-PLAT-002 envelope; a test asserts every `refPath` resolves on a golden la so.
...
6. The practice surface renders in the TASK-WEB-001 shell with DS v1.3.0 components, reuses the TASK-CHART-001 / TASK-CHART-002 views for input, and introduces no new design tokens; it never displays an interpretive verdict (grades casting only).

## §5 - Verification

- Vitest (`grader.test.ts`): golden-la-so fixtures for one QiMen and one LiuRen chart (emitted by the engines or built to the TASK-PLAT-002 schema). Correct-answer case (no diffs); each canonical error case (an_khoa, tam_truyen, an_sao) with the expected message; the cascade case where a wrong dinh cuc yields one root and several `blocked` downstream steps.
...
2. `steps.ts`: the `qimenLadder` and `lucNhamLadder`, each step tied to `ban` paths and (where applicable) an TASK-EDU-001 `criterionId`.
3. `grader.ts`: `gradeStep` (per-cell diff + message), `gradeCast` (root-cause + `blockedBy`), `referenceChart` (cast via TASK-API-001).
...
Depends on TASK-QMDG-006 and TASK-LN-006 - the oracle-gated QiMen and LiuRen engines whose charts are the answer key; the practice surface reaches them through the normal casting path (TASK-API-001), never re-deriving a chart in TypeScript. Consumes the TASK-PLAT-002 envelope (the la so it diffs against) and the TASK-EDU-001 criteria (the steps carry `criterionId`s for the engine-graded transitions). Renders in the TASK-WEB-001 shell and reuses TASK-CHART-001 / TASK-CHART-002 for chart input. Related to TASK-EDU-003 (a wrong step can link out to the relevant library passage).
...
- Partial credit within a step: is a step all-or-nothing, or scored by fraction of correct cells? Default: report every `CellDiff` (so the learner sees each mistake) and treat the step as `incorrect` if any cell diverges; a numeric score is an TASK-EDU-001 progression concern, not the grader's.
...
- TaiYi practice: the ladder is defined for QiMen and LiuRen (the P3 engines that are built); the TaiYi ladder is added when TASK-TAT-006 lands and is out of scope here.
