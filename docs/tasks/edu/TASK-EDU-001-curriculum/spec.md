...
related_frs: [TASK-EDU-002, TASK-EDU-003, TASK-EDU-004, TASK-WEB-001, TASK-WEB-006, TASK-KB-003]
...
This task defines the training platform's spine: the four-level Tam Thuc curriculum and its measurable progression criteria (Tam-Thuc-07 s3.1-3.2). It is the P3 learning flow's structural foundation; TASK-EDU-002 wires the auto-grader onto it and TASK-EDU-003 supplies the bilingual library it links to.
...
A learner's current level SHALL be a pure function of which criteria they have met: a level cannot be entered while a prerequisite's criterion is unmet. Criteria that Tam-Thuc-07 s3.3 grades with the deterministic engine SHALL be marked so (measure `auto_graded`, `gradedBy: "engine"`), so TASK-EDU-002 knows exactly which criteria it grades by diffing the learner's chart against the engine's. The curriculum content SHALL render inside the TASK-WEB-001 app shell using the existing Design System v1.3.0 components, introducing no new tokens, and SHALL be bilingual-ready (an optional `han` field on domain modules; stable ids on criteria so TASK-WEB-006 can translate by id).
...
Keeping the criteria as machine-checkable records with stable ids, rather than prose in a lesson, is what lets TASK-EDU-002 grade them, TASK-WEB-006 translate them, and this task test them against the source table. The curriculum is data, not copy.
...
- L1 - nen can chi + ngu hanh: thien can dia chi, ngu hanh sinh khac, chi hop/xung/hinh/hai, tu tru, tiet khi, chan thai duong thoi. The shared foundation of all three engines (the TASK-CORE content).
...
The 1->2 (pillars + tiet khi) and 2->3 (LiuRen chart) criteria are `auto_graded` / `gradedBy: "engine"`: TASK-EDU-002 casts the learner's answer and diffs it against the engine. The 3->4 criterion is `auto_graded` for the chart-casting half and `manual`/`quiz` for the "nhan dien cach cuc co ban" half. The L4 criterion ("chon dung he ... giai thich duoc cach doc") is `quiz` / `manual` - it is interpretive, so the engine does not grade it.
...
5. Domain modules carry an optional `han` field and every `Criterion` has a stable `id`; the curriculum renders in the TASK-WEB-001 shell with no new design tokens.
...
- Grader-hook test: every `gradedBy: "engine"` criterion has a non-empty `systems` list (so TASK-EDU-002 knows what to cast); the L4 criterion has none.
...
6. Render the levels + criteria + lock state in the TASK-WEB-001 learning flow (list view, no new tokens).

## §7 - Dependencies

Depends on TASK-WEB-001 (the app shell and Design System v1.3.0 components; the curriculum renders in the learning flow, Tam-Thuc-07 s6). Blocks TASK-EDU-002 (auto-graded practice consumes the criteria marked `gradedBy: "engine"` and needs TASK-QMDG-006 + TASK-LN-006 to cast the reference charts) and informs TASK-EDU-004 (onboarding). Related to TASK-KB-003 / TASK-EDU-003 (the bilingual classical library the curriculum links out to) and TASK-WEB-006 (i18n translates the criteria by id).
...
- Localization: the s3.2 table is Vietnamese; the EN mirror is deferred to TASK-WEB-006. This task keeps the VN source text and a stable id per criterion so WEB-006 can translate by id without re-authoring.
