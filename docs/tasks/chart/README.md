...
The interactive chart views that render each Tam Thuc la so: the 9-palace QiMen chart (the product's signature visual) first, then the LiuRen and TaiYi views, plus a shared export/accessibility pass. 4 tasks, ~48 engineering-hours, P0 flagship then P1-P2. Source of rationale: `../../strategy/tam-thuc-unified-plan-2026-07-08.md` (sections 4.3, 4.4) and Grok 34 (the chart) + 51 (the wireframes) + Claude 07 (the chart screen). Language is Next.js / TypeScript (DEC-2), rendered inside the TASK-WEB-001 shell. Implementation order and trigger: `../IMPLEMENTATION_ORDER.md` + `../PROMPT.md`.
...
|---|---|---|--:|---|---|---|
| CHART-001 | MUST | P0 | 16 | WEB-001, QMDG-006 | [TASK-CHART-001](TASK-CHART-001-nine-palace-qimen/spec.md) | Interactive 9-palace QiMen chart (4 layers, hover/click, cat/hung color, export) |
| CHART-002 | MUST | P1 | 12 | CHART-001, LN-006 | [TASK-CHART-002](TASK-CHART-002-liuren-view/spec.md) | LiuRen chart view (thien dia ban, tu khoa, tam truyen, thien tuong) |
| CHART-003 | SHOULD | P2 | 12 | CHART-001, TAT-006 | [TASK-CHART-003](TASK-CHART-003-taiyi-view/spec.md) | TaiYi chart view (cuu cung, 16 than, tuong) |
| CHART-004 | SHOULD | P1 | 8 | CHART-001 | [TASK-CHART-004](TASK-CHART-004-export-a11y/spec.md) | Chart export (PNG/SVG/print) + accessibility (dau chong test, screen reader) |

One P0 task is authored: CHART-001, the interactive 9-palace QiMen chart. Three are authored: CHART-002 (the LiuRen chart view - thien dia ban / tu khoa / tam truyen / thien tuong, P1, needs LN-006), CHART-003 (the TaiYi chart view - cuu cung / 16 than / tuong, P2, needs TAT-006), and CHART-004 (the shared PNG/SVG/print export + the full accessibility pass including the dau chong stacked-diacritics test and screen-reader support, P1).
...
- Depends on WEB: every chart renders inside the TASK-WEB-001 shell and uses its tokens and the cat/hung color+icon+text convention. Depends on the engines: CHART-001 reads the TASK-QMDG-006 `KyMonBan` (`he = "ky_mon"`), CHART-002 reads the TASK-LN-006 LiuRen `ban`, CHART-003 reads the TASK-TAT-006 TaiYi `ban` - each is a pure reader of its engine's output.
- Blocks WEB: TASK-WEB-003 (the results screen) embeds CHART-001; the report view (TASK-WEB-005) and export (TASK-REPORT-002) build on the chart export seam CHART-004 completes.
- Reads the TASK-PLAT-002 envelope read-only: a chart never writes `ban`, `cach_cuc`, `lich_phap`, or `co_truong_phai`. The `ban` shape each view reads is pinned to its engine's assembly task via a shared golden fixture, so a chart change is a reviewed, versioned envelope change (PLAT-002), not a silent view edit.
