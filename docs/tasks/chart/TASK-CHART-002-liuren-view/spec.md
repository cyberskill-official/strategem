...
related_frs: [TASK-WEB-001, TASK-CHART-001, TASK-WEB-003, TASK-CHART-004, TASK-PLAT-002, TASK-LN-006, TASK-STRAT-004]
depends_on: [TASK-CHART-001, TASK-LN-006]
blocks: []
new_paths:
...
This task builds the interactive LiuRen (Dai Luc Nham) chart view - the second chart type, rendering the la so the LiuRen engine cast (TASK-LN-006). Like the 9-palace QiMen chart (TASK-CHART-001), it is a pure view over the engine's output: it reads the la so envelope and never computes or mutates it (strategy 4.3). It inherits the TASK-CHART-001 pattern - read the engine's `ban`, render the domain's native notation, encode no rule of its own. It owns the LiuRen rendering and interaction; it does NOT cast the chart (TASK-LN-006) and does NOT own the results/cross-system composition (TASK-WEB-003, TASK-STRAT-004) that embeds it.

The component SHALL read the la so envelope (TASK-PLAT-002) for `he = "luc_nham"`, taking the `ban` slot as the LiuRen `ban` (TASK-LN-006) and the top-level `cach_cuc` for polarity. It SHALL render, in the LiuRen native notation: (1) the thien dia ban - the fixed dia ban 12-branch ring (地盤, the twelve dia chi in their fixed positions) with the rotating thien ban ring (天盤) placed over it by the nguyet tuong; (2) the tu khoa (四課, four lessons), each as its upper/lower (thuong / ha) can-chi pair with the khac direction (thuong khac 上剋 / ha khac 下剋) marked; (3) the tam truyen (三傳, three transmissions) - so truyen (初傳), trung truyen (中傳), mat truyen (末傳) - each with its chi, its thien tuong, and its luc than; and (4) the twelve thien tuong (十二天將, the heavenly generals) placed around the board thuan/nghich from the khoi quy nhan. Where a slot is empty (e.g. the degenerate phuc ngam / phan ngam cases the engine produced), it SHALL render empty, not fabricated.

Hover or click on a board cell, a lesson, a transmission, or a general SHALL reveal a detail view (its full content and role). Elements carrying a cach cuc / polarity SHALL be marked cat / hung / trung by color AND icon AND text - never color alone (TASK-WEB-001). The chart SHALL be responsive and SHALL expose the export seam (TASK-CHART-004 completes export/print/accessibility) and meet baseline accessibility (keyboard-reachable cells, text alternatives). It SHALL treat the envelope as read-only and SHALL NOT write `ban`, `cach_cuc`, `lich_phap`, or `co_truong_phai`.

## §2 - Why this design (rationale for humans)

A LiuRen reading is read in a fixed order through a fixed notation: the caster looks at the thien dia ban, reads the four lessons, follows the three transmissions, and weighs the twelve generals (Claude-02). That order and that notation are the domain's language, so getting the fixed dia ban ring, the thien-ban rotation, the four-lesson layout, or the transmission order wrong makes the chart unreadable to anyone who practices the art. Rendering exactly the `ban` the engine produced - including the degenerate phuc ngam / phan ngam cases as the engine cast them, empty where empty - keeps the chart an honest picture of a deterministic computation the kinliuren oracle verified (TASK-LN-006), not an illustration that smooths over an awkward cast.

Making the view a pure reader is the same architectural boundary the QiMen chart rests on (strategy 4.3, TASK-CHART-001 s2). LiuRen has the most explicit, least school-contested rules of the three engines - the nine tong mon are a decision tree, the tam truyen derivation is the engine's - so the temptation to "help" by re-deriving a transmission in the view is real and must be refused: the moment the chart re-computes a truyen, it asserts something the engine did not, and the determinism guarantee leaks into the UI. cat/hung by color plus icon plus text is the accessibility floor applied where a favorable/unfavorable general or lesson is easy to over-read as a verdict, so it always carries an explicit icon and word, and survives grayscale print.

## §3 - Contract (data / layout / interaction)

### Input (reads the TASK-PLAT-002 envelope, he = luc_nham)

```ts
// the LiuRen ban shape (TASK-LN-006), read-only
type LucNhamBan = {
  thien_dia_ban: {
...
Hover/click a board cell, a lesson, a transmission, or a general -> a detail showing its full content and role. An element referenced by a `cach_cuc` is marked cat / hung / trung as color + icon + text together (semantic tokens from TASK-WEB-001; never color alone). Cells, lessons, transmissions, and generals are keyboard-focusable and the detail is keyboard-reachable.
...
- Contract: `LucNhamBan` is checked against the TASK-LN-006 shape via a shared golden fixture; a drift fails the test.
...
5. `liuren-chart.tsx`: compose the board, lessons, transmissions, and generals (SVG); responsive sizing; cat/hung marking from `cach_cuc` with color+icon+text; the TASK-CHART-001 export seam.
6. `tests/liuren-chart.test.tsx` with the TASK-LN-006 golden fixture.

## §7 - Dependencies

Depends on TASK-CHART-001 (the "pure reader of its engine's `ban`" pattern, the export seam, and the cat/hung color+icon+text convention) and TASK-LN-006 (the LiuRen `ban` and the `he = "luc_nham"` envelope it reads). Uses TASK-WEB-001 (tokens, semantic markers, the shell it renders inside). Embedded by TASK-WEB-003 (results) and TASK-STRAT-004 (cross-system validate renders LiuRen alongside QiMen). Completed by TASK-CHART-004 (export/print/accessibility builds on the export seam). Reads the TASK-PLAT-002 envelope read-only (strategy 4.3); the `LucNhamBan` contract is pinned to TASK-LN-006 via a shared golden fixture.
...
| Envelope-shape drift | `LucNhamBan` differs from TASK-LN-006 | the shared golden-fixture contract test fails; treat any `ban` change as a versioned TASK-PLAT-002 change |

## §11 - Notes

The LiuRen view is the second chart type and it inherits the TASK-CHART-001 discipline exactly: faithful notation (the fixed dia ban ring, the thien-ban rotation, the four lessons with khac, the three transmissions in order, the twelve generals on their branches, empty where empty) and no computation (it reads `ban` and `cach_cuc` and writes nothing - strategy 4.3, asserted by a read-only test). cat/hung is color + icon + text, never color alone. The export seam is exposed here and completed by TASK-CHART-004, and the `LucNhamBan` contract stays pinned to TASK-LN-006 via a shared golden fixture, so a chart change is a reviewed, versioned envelope change, not a silent view edit.
