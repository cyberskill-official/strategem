...
related_frs: [TASK-LN-001, TASK-LN-002, TASK-LN-003, TASK-LN-005, TASK-LN-006, TASK-PLAT-002]
...
This task places the muoi hai thien tuong (十二天將), the twelve generals, over the board, giving each chi its second layer of meaning. It extends the `cyberos-luchnham` crate.
...
6. The emitted `ban.thien_tuong` round-trips through the la so envelope (TASK-PLAT-002) under `he = "luc_nham"`, `co_truong_phai` carries `khoi_quy_nhan` (the resolved tru/da) and `quy_nhan_variant`, and the whole arrangement matches kinliuren across day stem x day/night x variant.
...
6. Extend `flags.rs` (TASK-LN-001) to carry `quy_nhan_variant`; stamp `khoi_quy_nhan` (resolved tru/da) and `quy_nhan_variant` into `co_truong_phai`; emit `AnThienTuong` into `ban.thien_tuong` for `he = "luc_nham"`.
...
Depends on TASK-LN-002 for crate sequencing; the hard data input is TASK-LN-001's `ThienDiaBan` (the generals ride the thien ban) plus the day stem and gio chiem from TASK-CORE-005. Extends the TASK-LN-001 `flags.rs` flag set with `quy_nhan_variant`. Blocks TASK-LN-005 (khoa the read the general a spirit rides) and TASK-LN-006 (assembly). Emits into the TASK-PLAT-002 envelope.
...
- Placement anchor: this task reads the "Quy Nhan palace" as the dia ban palace of the computed noble chi (Claude-02 s5.2, "vi tri quy nhan tren dia ban"), then walks palaces; an alternative anchors on the palace whose thien ban chi equals the noble. Confirm the anchor against kinliuren before locking, since the two can differ and it changes the whole ring. This is the single highest-risk ambiguity in this slice.
- Day/night threshold: default is the fixed Mao..Than window; a sunrise/sunset school is a parameter. Decide whether the platform ever needs the astronomical boundary (it would consume TASK-CORE-002 true solar time) or whether the fixed window suffices for MVP.
