...
Six tasks, all P1, ~70 engineering-hours. One (LN-001, thien dia ban + nguyet tuong) is authored; the other five are authored. The pipeline is a straight chain - thien dia ban -> tu khoa -> {tam truyen, thien tuong} -> khoa the / luc than -> assembly - and LN-006 emits the la so JSON envelope (TASK-PLAT-002, he = "luc_nham") and gates the whole engine 100% against the kinliuren oracle for every flag combination.

## task list

| task | Pri | Phase | h | Title |
|---|---|---|--:|---|
| [LN-001](TASK-LN-001-thien-dia-ban-nguyet-tuong/spec.md) | MUST | P1 | 12 | Thien dia ban + nguyet tuong (gia nguyet tuong, thien can ky cung) |
...
- Depends on CORE: every task stands on TASK-CORE-005 (the calendar module API - tu tru plus the current trung khi that sets nguyet tuong), and LN-006 additionally on TASK-CORE-006 (the oracle harness must be green before LiuRen inherits the context). LN specifically consumes CORE-001's jie/trung split: nguyet tuong changes at trung khi, never at jie, so LN reads the TrungKhi-kind term, not the month-pillar jie.
- Depends on PLAT: every engine task emits the TASK-PLAT-002 la so envelope and stamps `co_truong_phai`; LN-006 owns the assembly and the cache key. CORE-007 ganzhi primitives (ngu hanh sinh/khac, chi relations) feed the khac/tac test in tu khoa and the luc than mapping.
...
- Crate: `cyberos-luchnham` (Rust). TASK-LN-001 owns the crate birth; TASK-LN-002..006 extend it, so the engine is one cargo-testable unit. The LN flag set lives in `flags.rs` as its single home.
