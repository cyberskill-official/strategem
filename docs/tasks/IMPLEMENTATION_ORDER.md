...
Single source of truth for task status and eligibility. Task id = task id; the spec is the task in the module folders in this directory. Rationale is the strategy report `../strategy/tam-thuc-unified-plan-2026-07-08.md`.

Status: `draft` (task body not yet authored) | `ready_to_implement` | `blocked` (unmet deps) | `implementing` | `in_review` | `done` | `superseded`. Only a human sets `done`. Eligibility: task is `ready_to_implement`, status not `done`, every `depends_on` is `done`. Pick order: phase, then dependency spine, then id.

At this snapshot (2026-07-08) nothing is `done`, so the only immediately eligible task is the root PLAT-001; everything else is `blocked` until its deps clear. All 87 task bodies are authored.
...
| STRAT-003 | Chu-khach decision framework | SHOULD | 8 | RAG-003 | yes | done |
...
Status lives in `backlog.yaml`; this file is sequencing only. The task is the spec.
