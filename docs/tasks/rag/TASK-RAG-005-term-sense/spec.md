---
id: TASK-RAG-005
title: "Term-sense expansion - expand a query across the layered senses of a classical term (ban nghia base / dan than extended / gia ta phonetic-loan / dien tich allusion) via a curated glossary, weighted and bounded, so retrieval does not miss relevant classical text; sits upstream of TASK-RAG-002"
module: RAG
priority: SHOULD
status: done
phase: P2
...
related_frs: [TASK-RAG-002, TASK-RAG-001, TASK-KB-003, TASK-RAG-003, TASK-RAG-006]
...
class TermSense(BaseModel):
    ...
    citations: list[str] = []    # optional unit(s) documenting this sense (TASK-KB-003)
...
class ExpandedQuery(BaseModel):
    ...
    def enriched_text(self) -> str: ...       # original + weighted added terms, for TASK-RAG-002 to embed
...
### The TASK-RAG-002 seam

TASK-RAG-002's `RetrievalRequest` carries `query: str`. The default integration is single-query: the caller sets `request.query = expanded.enriched_text()` (original plus weighted added terms) and TASK-RAG-002 embeds it as one vector - TASK-RAG-002 is unchanged. A recall-sensitive mode uses `expanded.query_variants()`: the caller retrieves for each variant and unions the hits into TASK-RAG-002's fusion. The default is single-query (cheapest, keeps the interface intact); multi-query is a flag evaluated by TASK-RAG-006.
...
5. `enriched_text()` returns the original query plus the weighted expansions in a stable order, and feeding it to a stub TASK-RAG-002 retrieves a passage that the bare query missed on the fixture (recall lift shown).
...
4. Wire the TASK-RAG-002 seam: `enriched_text()` for single-query (default) and `query_variants()` for the multi-query flag.
5. Commit `fixtures/term_sense_sample.json` (a few terms across systems, with a deliberate gia_ta case) as the test exemplar.

## §7 - Dependencies

Depends on TASK-RAG-002 (this task widens the query TASK-RAG-002 retrieves on; the default single-query integration keeps TASK-RAG-002's interface intact). Reads no la so - it operates on the query string and the system only. Related to TASK-RAG-001 (the same three-layer chunks its expanded query retrieves over) and TASK-KB-003 (the corpus a sense's optional `citations` resolve into, and a natural source for authoring the glossary's senses). Blocks nothing; it is an optional recall enhancement in front of retrieval. Its value is scored by TASK-RAG-006, which measures whether expansion lifts recall without hurting faithfulness or precision. The RAG-branch invariant holds trivially here: this task never produces an interpretation and never touches `ban`/`cach_cuc`/`lich_phap`/`co_truong_phai`; the citation and AIDisclosure discipline lives downstream in TASK-RAG-003 and is untouched by widening retrieval.
...
- Single enriched query vs multi-query retrieval. Default: single enriched query (keeps TASK-RAG-002 unchanged, cheapest); multi-query behind a flag for recall-critical cases. Decide from TASK-RAG-006 recall/precision numbers.
- Whether the glossary is hand-authored or bootstrapped from a classical dictionary. Default: hand-author the flagship QiMen terms with a curator, cite senses into TASK-KB-003 where a source documents them; bootstrap wider coverage later under TASK-KB-004 review.
...
| Recall lift unverified | expansion shipped on faith | TASK-RAG-006 scores recall vs precision/faithfulness; expansion tuned from the number |

## §11 - Notes

This task closes a recall gap specific to văn ngôn văn: a query phrased in one sense of a term must still reach classical text that uses another sense. Its discipline is entirely in the weighting and the bound - widen recall, never blur intent - and its safety is that it is claim-free, so it changes what is retrieved and never what is asserted. Author the QiMen glossary first to match the P0/P1 flagship, keep giả tá off unless a curator vouches for a term, and let TASK-RAG-006 decide the layer weights from measured recall and faithfulness rather than intuition. The package `tamthuc_rag` is shared with TASK-RAG-001/002/003/004/006/007; this task adds `expand.py` and `glossary.py`, so the RAG branch stays one installable, mypy-clean unit.
