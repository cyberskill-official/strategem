...
related_frs: [TASK-RAG-003, TASK-KB-004, TASK-WEB-001, TASK-WEB-003, TASK-AUTH-002, TASK-PLAT-003, TASK-API-004]
...
This task is the runtime human-in-the-loop for interpretation: a review queue that intercepts AI interpretations flagged important or low-confidence, holds them for an expert to approve or reject with a reason, and gates them before they reach the user. It is the operational form of the third anti-hallucination layer (human-in-the-loop) that TASK-RAG-003 declares (Claude-06 s4.3; strategy 4.4). It consumes the `requires_human_review` flag and the `AIDisclosure` that TASK-RAG-003 produces, and it owns the review policy that TASK-RAG-003 consults to set that flag.

When an `Interpretation` has `requires_human_review = false`, the gate SHALL release it, set `disclosure.review_status = "not_required"`, and let it reach the user unchanged. When `requires_human_review = true`, the gate SHALL NOT release the interpretation to the user; it SHALL enqueue a `ReviewTicket`, set `disclosure.review_status = "pending"`, and return a withheld view that carries no confident uncited claims. A reviewer holding the review role (TASK-AUTH-002) SHALL approve or reject each ticket with a non-empty `reason`; the decision SHALL be recorded immutably with the reviewer id and timestamp (an audit row). On approve, the gate SHALL set `review_status = "approved"` and release the interpretation (with the reviewer's optional edits to the interpretation text and recommendations only); on reject, it SHALL set `review_status = "rejected"`, withhold the interpretation, and surface a safe under-review message instead of the raw AI text.

The gate SHALL own the `ReviewPolicy` - the confidence threshold and the high-stakes question-type set - that TASK-RAG-003's guard reads to decide `requires_human_review`, so the flag and the queue share one policy. The gate SHALL NOT touch the la so envelope: it operates only on the `Interpretation` object and the review record, and it SHALL NEVER write `ban`, `cach_cuc`, `lich_phap`, or `co_truong_phai` (strategy 4.3). A reviewer edit SHALL change only the interpretation text, recommendations, and citations shown, never the chart the engine cast, and the `AIDisclosure` SHALL be preserved with only `review_status` updated. Every interpretation that reaches the user SHALL carry an `AIDisclosure` whose `review_status` is one of `not_required` or `approved`; a `pending` or `rejected` interpretation SHALL never be presented as a finished reading.
...
TASK-RAG-003's `guard.enforce` calls `ReviewPolicy.requires_review(...)` to set `requires_human_review`; this task is the single owner of the policy so the flag and the queue cannot disagree.
...
class ReviewTicket(BaseModel):
    ...
    id: str
    query_id: str
    interpretation_id: str              # the persisted interpretation (TASK-API-004)
    persona_level: str
    confidence: float
    reason_flagged: str                 # "low_confidence" | "high_stakes:<type>"
    status: ReviewStatus
    interpretation: dict                # snapshot for the reviewer (TASK-RAG-003 Interpretation)
...
class ReviewOutcome(BaseModel):
    model_config = ConfigDict(extra="forbid")
    ticket_id: str
    reviewer_id: str                    # holds the review role (TASK-AUTH-002)
...
`under_review_view` strips confident claims and shows a neutral "awaiting expert review" state; it is what TASK-WEB-001's `HumanReviewGate` component renders. `apply_outcome` mutates only the interpretation's text, recommendations, and citations on an approved edit; it never mutates the chart fields, and it refreshes only `disclosure.review_status`.
...
6. `ReviewPolicy.requires_review` drives TASK-RAG-003's flag: a low-confidence or high-stakes case flags and queues; a confident low-stakes case does not - the flag and the queue agree by construction.
...
1. `review/policy.py`: the `ReviewPolicy` (threshold + high-stakes types) that TASK-RAG-003 consults; a default config plus an override hook.
...
5. Wire the role check to TASK-AUTH-002 and the `withheld_view` shape to TASK-WEB-001's `HumanReviewGate` component.

## §7 - Dependencies

Depends on TASK-RAG-003 (which produces the `Interpretation`, the `requires_human_review` flag, and the `AIDisclosure` this gate consumes; this task in turn owns the `ReviewPolicy` TASK-RAG-003 reads). Uses TASK-AUTH-002 for the reviewer role, TASK-PLAT-003 for the `review_ticket` / `review_outcome` tables and audit rows (soft edge; applied through the PLAT runner), and TASK-API-004 for the persisted interpretation the ticket references. Surfaced by TASK-WEB-001 (the `HumanReviewGate` component) and TASK-WEB-003 (the results screen, which shows the under-review state instead of a pending reading). Blocks nothing in the catalog, but it is the runtime gate every AI-facing surface passes flagged output through. Sibling to TASK-KB-004: same human-in-the-loop principle at a different stage (AI interpretations at query time here, knowledge content at curation time there).
...
- The exact confidence threshold and the high-stakes question-type set. Default: a conservative threshold (0.55) plus a health/legal/large-financial/major-life allowlist; tune from review outcomes and TASK-RAG-006 evidence, keeping the policy in one place so TASK-RAG-003's flag follows.
...
- Whether reject offers the user a rule-based fallback reading. Default: show the safe under-review message at MVP; wiring the TASK-RAG-007 degraded reading in on reject is a follow-up once both exist.
...
| Unauthorized reviewer | a non-review role decides | rejected; only the TASK-AUTH-002 review role may decide |
...
This task makes the third anti-hallucination layer operational, and its whole reason to exist is that a consequential reading must not reach a user unattended. Two invariants are non-negotiable and mirror TASK-RAG-003: the gate reads the interpretation and never writes the la so, and no interpretation reaches the user without a truthful `review_status` on its `AIDisclosure`. Keep the `ReviewPolicy` here as the single source so the flag TASK-RAG-003 sets and the queue this task runs can never drift. It is a deliberate sibling of TASK-KB-004, not a merge - different objects, different review focus, same principle (strategy 4.4). The package `tamthuc_rag` is shared with TASK-RAG-001/002/003/005/006/007; this task adds the `review/` module, so the RAG branch stays one installable, mypy-clean unit.
