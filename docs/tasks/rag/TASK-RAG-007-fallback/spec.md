...
related_frs: [TASK-RAG-003, TASK-PLAT-008, TASK-RULE-003, TASK-KB-002, TASK-RAG-004]
...
class ResilientLLM:                                  # implements TASK-RAG-003's LLM protocol
...
`ResilientLLM` is a drop-in for the `LLM` TASK-RAG-003 already accepts; it reuses TASK-PLAT-008's breaker/retry primitives where present, else a local implementation.
...
def rule_based_interpretation(la_so: LaSo, persona: str,
    ...
    for cc in detected:                              # ordered by score/polarity
        p = patterns.get(cc.id)                      # TASK-RULE-003 / TASK-KB-002: meaning + citations
        lines.append(render(cc, p, persona))         # fixed template; NO model text
        cites += p.citations
    cards = citation_cards(cites, retrieved)          # TASK-KB-003-resolved cards for the used patterns
...
def interpret_resilient(la_so, request, persona, retriever, llm, patterns,
    ...
    try:
        return interpret(la_so, request, persona, retriever, ResilientLLM(llm, breaker))  # TASK-RAG-003
...
`interpret_resilient` wraps TASK-RAG-003's `interpret` unchanged: it injects the `ResilientLLM` and catches `LLMUnavailable` to produce the degraded reading. The degraded templates stitch only the cited pattern meanings; they contain no free-form slots the model would fill.

### Disclosure extension (coordinated with TASK-RAG-003)

This task adds one field to the shared contract: `AIDisclosure.degraded: bool = False`. It is additive and defaulted, so every existing TASK-RAG-003 output is unaffected; the JSON Schema (`docs/contracts/interpretation.schema.json`) bumps a minor version. The degraded marker is the truthful signal TASK-WEB-003 renders as a reduced-reading banner.

## §4 - Acceptance criteria

1. A healthy LLM path returns TASK-RAG-003's normal interpretation unchanged, with `disclosure.degraded = false`.
...
1. `resilience.py`: `CircuitBreaker` (fail threshold, cooldown, half-open probe) and `ResilientLLM` (timeout + bounded retry/backoff + breaker), reusing TASK-PLAT-008 primitives where present; `LLMUnavailable`.
2. `fallback.py`: `rule_based_interpretation` (assemble from `la_so.cach_cuc` + cited pattern meanings, read-only) and `interpret_resilient` (wrap TASK-RAG-003's `interpret`, catch `LLMUnavailable`).
...
4. Add `AIDisclosure.degraded: bool = False` to the shared TASK-RAG-003 contract; bump `docs/contracts/interpretation.schema.json` a minor version.
5. Wire `patterns` to TASK-RULE-003 (the pattern loader serving active TASK-KB-002 patterns with meanings + citations) and `citation_cards` to TASK-KB-003 resolution.
...
Depends on TASK-RAG-003 (it wraps that task's LLM caller and `interpret`, and extends its `AIDisclosure` with the additive `degraded` field). Uses TASK-PLAT-008 resilience primitives (circuit breaker, retry/backoff, graceful degradation) where available - this task is their interpretation-branch application. Reads the detected cách cục from the la so and their cited meanings via TASK-RULE-003 (serving active TASK-KB-002 patterns) and TASK-KB-003 (resolving the citation cards). Composes with TASK-RAG-004: a degraded reading is conservative and `requires_human_review = true`, so it also passes the HumanReviewGate; wiring the degraded reading in as the reject-time fallback is a natural follow-up. Blocks nothing in the catalog, but it is the resilience wrapper every production interpretation call SHOULD go through. The RAG-branch invariant is central here and enforced: the fallback reads the la so read-only, asserts only cited pattern meanings, and keeps the citation + AIDisclosure discipline with the degraded marker set.
...
| Schema break from the new field | `degraded` field breaks existing outputs | additive + defaulted `false`; minor schema bump; existing TASK-RAG-003 outputs unaffected |

## §11 - Notes

This task is what lets the interpretation branch stay both available and honest under an LLM outage: the deterministic, cited layers beneath the model make a true degraded reading possible, so resilience never becomes an excuse to hallucinate. Two invariants carry over from TASK-RAG-003 unchanged - the fallback reads the la so and never writes it, and it makes no claim without a citation - and one label is added: degraded output is always marked degraded, low-confidence, and review-required, because a reduced reading must announce itself. Keep the degraded templates free of any model-fillable slot so the path can never drift into generation, and coordinate the single additive `AIDisclosure.degraded` field with TASK-RAG-003 as a reviewed schema bump. The package `tamthuc_rag` is shared with TASK-RAG-001/002/003/004/005/006; this task adds `fallback.py`, `resilience.py`, and the degraded templates, so the RAG branch stays one installable, mypy-clean unit.
