...
related_frs: [TASK-KB-003, TASK-RAG-002, TASK-KB-005, TASK-RAG-005, TASK-PLAT-001, TASK-PLAT-003]
depends_on: [TASK-KB-003, TASK-PLAT-001]
blocks: [TASK-RAG-002]
new_paths:
...
```python
def ingest(source_rows: Iterable[dict],        # from TASK-KB-003's three-layer store
           chunker: Chunker,
           embedder: Embedder,
           store: VectorStore,
...
`Chunker` (`chunk.py`) splits TASK-KB-003 units by `unit_type` and emits one `Chunk` per present layer, preserving `unit_id`/`citation_id` linkage. `config.py` selects the embedder and store from environment/config (`RAG_EMBEDDER=bge-m3`, `RAG_VECTOR_BACKEND=pgvector`) and records `model`+`dim` into the index metadata.
...
1. Create the `tamthuc_rag` package (`pyproject.toml`, `uv`-managed per TASK-PLAT-001); this task owns its birth, TASK-RAG-002/003 add modules.
2. `models.py`: `Chunk`, enums, `StoredChunk`, `Hit`.
3. `chunk.py`: `Chunker` splitting TASK-KB-003 units by natural unit into per-layer chunks.
...
Depends on TASK-KB-003 (the three-layer classical-text store this task reads and indexes; its chunk units define `unit_id`/`citation_id`) and TASK-PLAT-001 (the Python workspace and, for pgvector, the shared Postgres from TASK-PLAT-003). Blocks TASK-RAG-002, which queries this vector store as the semantic arm of hybrid retrieval. TASK-RAG-005 (term-sense expansion) later builds on the same three-layer chunks.

## §8 - Example payloads

```json
// one source unit from TASK-KB-003 (input)
...
- bge-m3 vs a hosted embedder for production. Default: bge-m3 local (cost, privacy of sensitive queries per strategy 4.4); the OpenAI adapter is the fallback/benchmark. Decide from the TASK-RAG-006 eval loop's retrieval quality.
...
This task is the foundation of retrieval-grounded interpretation: no citation card, no anti-hallucination check, and no hybrid retrieval exists without an index over the classical text. Keep the boundary with TASK-KB-003 clean - that task owns the canonical three-layer text and its chunk units; this task owns the embeddings and the searchable vector index over them. The package `tamthuc_rag` is shared with TASK-RAG-002/003; they add the retriever and the interpreter to it, so the RAG branch is one installable, mypy-clean unit.
