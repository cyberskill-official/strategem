...
related_frs: [TASK-AUTH-001, TASK-AUTH-004, TASK-PLAT-003, TASK-API-004, TASK-LEGAL-001, TASK-LEGAL-004]
depends_on: [TASK-AUTH-001]
blocks: [TASK-AUTH-004]
new_paths:
...
This task defines the data-protection compliance pack: consent capture at signup, a retention schedule per data class, and the right-to-erasure and data-export contracts that TASK-AUTH-004 (DSAR self-service) implements. Its legal basis is the Vietnamese PDPD (Nghi dinh 13/2023/ND-CP on personal data protection) and the GDPR (strategy 4.4, RISK-5). It is a python/doc task: it owns the contracts, the schedule, and the consent copy as reviewable artifacts; it does NOT implement the DSAR runtime endpoints (TASK-AUTH-004) nor the encryption at rest (TASK-AUTH-001).
...
This task provides the contract and the schedule; TASK-AUTH-004 implements the export and erasure endpoints against them, and the named statutes and final wording are reviewed at TASK-LEGAL-004 before launch.
...
`retention.py` exposes the schedule as typed data so TASK-AUTH-004 and TASK-PLAT-003 read one source, not two copies.

### Consent capture (`consent.md`)

Consent is captured at signup: granular per purpose, revocable, stored with `consent_version` + timestamp + scope, with a withdrawal path. User-facing consent copy is keyed via TASK-LEGAL-001 `copy-keys.yaml`, never inlined.
...
TASK-AUTH-004 implements `ExportContract` and `ErasureContract`; this task owns their shape and the JSON Schema. `erase` is idempotent: a second call returns the same receipt shape and changes nothing.
...
2. The DSAR contracts (`ExportContract`, `ErasureContract`) exist as typed interfaces plus a JSON Schema; a conformance test asserts the shape TASK-AUTH-004 must implement.
...
- Retention completeness: a test asserts every TASK-PLAT-003 table holding personal data maps to a data class in the schedule (no unclassified sensitive store).
- Doc review: the pack is marked for counsel review at TASK-LEGAL-004.
...
3. `consent.md`: consent capture spec (granular, revocable, versioned); copy keyed via TASK-LEGAL-001.
...
6. Hand the statute mapping to TASK-LEGAL-004; mark the pack `counsel_review: pending`.

## §7 - Dependencies

Depends on TASK-AUTH-001 (birth-data AES-256 encryption + profile; crypto-shred destroys the keys AUTH-001 manages). Blocks TASK-AUTH-004 (DSAR self-service implements the export and erasure contracts). Aligns with TASK-PLAT-003 (the tables the schedule classifies and the audit rows it exempts) and TASK-API-004 (persistence + audit rows). User-facing consent copy is keyed via TASK-LEGAL-001; the statutes and final wording are reviewed at TASK-LEGAL-004.
...
- Audit retention duration (12 vs 24 months). Default: 12 months minimum, extend per statute; counsel confirms at TASK-LEGAL-004.
...
Package `tamthuc_compliance` (Python, DEC-2). This is the RISK-5 mitigation as a reviewable artifact: two regimes (VN PDPD Nghi dinh 13/2023/ND-CP + GDPR) met as one pack at the stricter bar. Crypto-shred is the erasure mechanism that is actually complete across backups, because the AES-256 key - not the ciphertext - is the thing destroyed (TASK-AUTH-001 owns the keys). LEGAL owns the contract and the schedule; TASK-AUTH-004 implements the endpoints against them; the audit exemption is deliberate and lawful. Reviewed at TASK-LEGAL-004 before launch. refs Claude-07 s4.2, Grok-25.
