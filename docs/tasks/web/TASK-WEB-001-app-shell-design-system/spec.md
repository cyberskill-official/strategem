...
related_frs: [TASK-PLAT-001, TASK-WEB-002, TASK-WEB-003, TASK-WEB-006, TASK-CHART-001, TASK-LEGAL-001, TASK-RAG-003, TASK-RAG-004]
depends_on: [TASK-PLAT-001]
blocks: [TASK-WEB-002, TASK-WEB-003, TASK-WEB-004, TASK-WEB-006, TASK-CHART-001, TASK-LEGAL-001, TASK-EDU-001, TASK-EDU-004]
new_paths:
...
This task builds the frontend foundation: the Next.js app shell, the CyberSkill Design System v1.3.0 as design tokens, and the shared component library every screen composes from - including the two domain components that make the deterministic-engine || AI boundary visible in the interface (strategy 4.4): `AIDisclosureBadge` and `HumanReviewGate`. It owns the shell, the tokens, and the component library; it does NOT build the query or results screens (TASK-WEB-002, TASK-WEB-003) nor the chart (TASK-CHART-001), which consume this library.
...
The chart-casting layout is two columns: a left input panel and a right chart-plus-interpretation panel (TASK-WEB-002/003 fill them). The top bar is Umber ground with the Ochre brand mark on the left and the system tabs (LiuRen / QiMen / TaiYi) as navigation. The shell hosts the theme (light/dark), the density attribute, and the locale (TASK-WEB-006), and reserves a persistent slot for the in-product disclaimer copy (TASK-LEGAL-001).
...
Anatomy (Claude-07 s5): rendered in the info color `#2C5F8A`, fully round (`--radius-full`), with an info icon and short text (never color alone). It is a link/button - activating it opens a popover that names (a) the model used, (b) the interpretation limits (heritage education / decision support, not a certain future - copy from TASK-LEGAL-001), and (c) the citation sources backing the interpretation. It is a link to explanation, not decoration.
...
type AIDisclosureBadgeProps = {
  ...
  limitsCopyId: string;          // key into the TASK-LEGAL-001 copy deck
};
```

The badge is mandatory on any surface carrying AI output; it reads the `AIDisclosure` block TASK-RAG-003 produces and reflects `reviewStatus`.
...
The gate is the UI half of TASK-RAG-004's review pipeline; this task ships the component and its accessibility contract, TASK-RAG-004 wires the queue and audit behind it.
...
- Gates: `pnpm --filter web lint`, `pnpm --filter web test`, `next build` (TASK-PLAT-001 web lane).
...
2. `app-shell.tsx` + `top-bar.tsx`: the two-column shell, the Umber top bar with the Ochre brand mark and the system tabs, theme + density + locale hosting, and the disclaimer slot (TASK-LEGAL-001).
3. `button.tsx`: the variant/size matrix with the xs-never-primary rule and the Ochre focus ring.
4. `ai-disclosure-badge.tsx`: the info-color round badge + popover (model, limits copy, citation sources); read the TASK-RAG-003 `AIDisclosure` shape.
...
Depends on TASK-PLAT-001 (the `apps/web` scaffold, Tailwind, shadcn/ui). Blocks TASK-WEB-002 (query input composes this library), TASK-WEB-003 (results screen composes the chart + the two domain components), TASK-WEB-004 (dashboard), TASK-WEB-006 (i18n plugs into the shell's locale slot), TASK-CHART-001 (the chart is rendered inside this shell and uses the tokens and the cat/hung icon+text convention), TASK-LEGAL-001 (the disclaimer copy fills the shell slot and the badge popover), and the EDU learner surfaces. Reads the `AIDisclosure` contract from TASK-RAG-003 and pairs with TASK-RAG-004 behind `HumanReviewGate`.

## §8 - Example payloads

```tsx
// AIDisclosureBadge fed by the TASK-RAG-003 AIDisclosure block
<AIDisclosureBadge
  model="gpt-4o-mini"
  citationSources={interpretation.citations}
  reviewStatus={interpretation.disclosure.review_status}   // "not_required"
  limitsCopyId="ai.limits.heritage_decision_support"       // TASK-LEGAL-001 copy key
/>
...
- Where the AIDisclosure "interpretation limits" copy lives. Default: the TASK-LEGAL-001 copy deck keyed by id (`limitsCopyId`), so legal owns the words and the component owns the affordance - the badge never hard-codes legal language.
