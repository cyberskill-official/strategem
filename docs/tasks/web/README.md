...
|---|---|---|--:|---|---|---|
| WEB-001 | MUST | P0 | 18 | PLAT-001 | [TASK-WEB-001](TASK-WEB-001-app-shell-design-system/spec.md) | App shell + Design System v1.3.0 tokens + component library (incl. AIDisclosureBadge, HumanReviewGate) |
| WEB-002 | MUST | P0 | 12 | WEB-001, API-001 | [TASK-WEB-002](TASK-WEB-002-query-input-screen/spec.md) | Query input screen (datetime, place, question type, system tabs) |
| WEB-003 | MUST | P0 | 14 | WEB-002, CHART-001, RAG-003 | [TASK-WEB-003](TASK-WEB-003-results-screen/spec.md) | Results screen (chart + patterns + cited interpretation + AIDisclosure) |
| WEB-004 | SHOULD | P1 | 8 | WEB-001 | [TASK-WEB-004](TASK-WEB-004-dashboard/spec.md) | Dashboard |
| WEB-005 | SHOULD | P1 | 8 | WEB-003, REPORT-001 | [TASK-WEB-005](TASK-WEB-005-report-view/spec.md) | Report view screen |
| WEB-006 | MUST | P1 | 10 | WEB-001 | [TASK-WEB-006](TASK-WEB-006-i18n/spec.md) | i18n (VN + EN, next-intl, content/label split) |
| WEB-007 | SHOULD | P2 | 12 | WEB-003 | [TASK-WEB-007](TASK-WEB-007-management-flow/spec.md) | Management flow (history, school-flag config, share/export) |
| WEB-008 | COULD | P3 | 10 | WEB-006 | [TASK-WEB-008](TASK-WEB-008-zh-i18n/spec.md) | Chinese i18n + RTL-ready |

Three P0 tasks are authored (WEB-001..003, the app-shell -> query -> results spine). Five are authored: WEB-004 (dashboard, P1), WEB-005 (report view, P1), WEB-006 (VN+EN i18n, P1), WEB-007 (the management flow - history, school-flag config, share/export, P2), and WEB-008 (Chinese i18n + RTL-ready, P3).
...
- Depends on PLAT: PLAT-001 (the `apps/web` scaffold, Tailwind, shadcn/ui). Depends on API: WEB-002 calls TASK-API-001 `/calculate/{system}` and renders its structured error envelope; the frontend never casts a chart itself (strategy 4.2).
- Depends on CHART + RAG: WEB-003 embeds the TASK-CHART-001 9-palace chart and renders the TASK-RAG-003 `Interpretation` + `AIDisclosure`; it pairs with TASK-RAG-004 behind the `HumanReviewGate`.
- Depends on LEGAL: the TASK-LEGAL-001 copy deck fills the shell disclaimer slot and the `AIDisclosureBadge` popover (legal owns the words, the component owns the affordance).
- Feeds REPORT + STRAT surfaces: WEB-005 renders TASK-REPORT-001 output; the strategic tools (Timing Optimizer, Scenario Comparison) surface through later screens.
