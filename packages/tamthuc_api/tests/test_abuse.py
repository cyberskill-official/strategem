...
def test_velocity_throttle() -> None:
    ...
    for _ in range(3):
        assert d.evaluate("p1", "9.9.9.9", RequestEvent(path="/api/v1/calculate/qimen")).action == "allow"
...
def test_probing_flag() -> None:
    ...
    for _ in range(2):
        assert d.evaluate(None, "8.8.8.8", RequestEvent(path="/x", malformed=True)).action == "allow"
